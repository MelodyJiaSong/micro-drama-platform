...
- **四个视图**（follow-up 162）：**总览**（KPI + 象限散点：对数横轴＝播放中位，纵轴＝点赞率中位，
  两条中位线切四象限，点大小＝翻拍易度，填色＝你标的状态）/ **系列**（六维筛选、五种排序、
  卡内定状态与评分、每卡 10 条样本的播放柱＋赞率点）/ **样本库**（100 条样本的可查询表，
  列排序、播放量背景条、赞率前十分位高亮、页脚实时统计）/ **对比**（最多 4 列的转置矩阵，
  数值行标「最佳」）。点任意系列打开**详情抽屉**（概览 / 翻拍流程 / 实测样本 / 风险与变现 / 我的笔记）。
- **你的决定会被保存**：状态（未定 / 短名单 / 在做 / 已弃）、0–5 星评分、系列笔记、
  样本书签与备注，写在 `ai_videos/_research/{dataset}.workspace.json`——
  **与数据集分文件**，所以重跑调研（重新生成 `{dataset}.json`）不会覆盖你的判断；
  工作区按 slug / video_id 引用，重排名次也仍然对得上。
- **接口**：`GET /api/research/datasets`、`/api/research/dataset/{dataset}`、
  `/api/research/dataset/{dataset}/series/{slug}`、`/api/research/dataset/{dataset}/workspace`；
  写入走 `PUT .../series/{slug}/mark` 与 `PUT .../video/{video_id}/mark`。
- **刷新**：重跑调研即可，例如
...
- **Downloads 导入 · 系列共享资产 (series-shared import)**: on a series member (`ai_videos/{series}/{ep}`), `DownloadsImporter.import_drama` also collects `characters/` / `scenes/` / `props/` from `ai_videos/{series}/_series/` (flat or `2_世界观人设/` layout; resolved in `libs/common/series_shared.py`). They route exactly like the episode's own assets (`_classify` + `asset_key.view_key_in`, plus the any-scene subject/plate and any-prop view fallbacks), so `c2-1_…` lands at `_series/characters/c2_…/c2-1.png`. Series assets live **only** in `_series/`: when an episode folder duplicates a shared one (same routing prefix such as `c1`, or identical folder name), a download that would land in either is **not moved** — it stays in Downloads and `errors` reports `series_key_conflict: {episode folder} <-> {series folder}` (repo-relative). Other keys still import; flat dramas are unaffected. Tests: `tests/test_series_shared_asset_import.py`.
...
- **双语字幕烧录 (bilingual subtitle burn-in)**: each shot render mp4 (`shots/shot{NN}/…`) tile shows 「📝 生成台词」 (`POST /api/scaffold-subtitles` → seeds a bilingual `shots/shot{NN}/subtitles.md`, each cue `起-止 中文 || English` with English left blank) plus **three burn buttons** 「💬中文 / 💬EN / 💬中英」 (`POST /api/burn-subtitles` `{path, lang}`, `lang ∈ zh|en|both`). The chosen language is rendered to ASS (中文 = 系统中文字体，按 OS 解析：macOS `PingFang SC` / Windows `微软雅黑` / Linux `Noto Sans CJK SC`，可用环境变量 `AI_VIDEO_MGMT_SUBTITLE_FONT_ZH` 覆盖——字体名按 OS 解析才能在 Windows/Mac 都正确渲染中文；英文 Arial; `both` stacks 中 above 英; L/R safe margin 120px, bottom 170px) and ffmpeg-burned into a stable per-shot master `shots/shot{NN}/shot{NN}_{zh|en|zhen}.mp4` in the shot-folder **root** (so multiple imported takes can coexist under `renders/`; burning any take overwrites the language master). Original renders are kept. Aggregate: `subtitle__{route,command,dto,mapper}` + `domain/value_objects/subtitle__valueobject.py` (parse + per-lang ASS) + `infrastructure/writers/subtitle__writer.py` (`SubtitleBurner`).
