...
└── episodes/
    └── epNN/
        ...
        │   └── shotNN/         # ⚠ folder-per-shot per rule #12.9 (NOT a flat shotNN.md). Folder holds prompt .md + renders/ media. (subtitles.md 默认不生成 — rule 11c 全流程默认关闭字幕烧录)
...
5. **shot prompts**: `ai_videos/{name}/episodes/epNN/shots/shotNN/shotNN.md` — derived from shotlist + script + chapter excerpt. **⚠ folder-per-shot (rule #12.9): each shot is a same-named folder `shots/shotNN/` containing `shotNN.md` (+ `renders/` media; `subtitles.md` 默认不生成 per rule 11c), NEVER a flat `shots/shotNN.md`.** The webapp display + render-import (`renders/` routing) contracts depend on this folder. （台词 burn-in `subtitles.md` 为 opt-in，默认不产出。） A flat `shotNN.md` is a stage-6 structural blocker.
...
- **道具可见性 / 连续性铁律（2026-06-21 教训：EP3 shot5 玉佩没掏就贴出来、玉佩一近胸口自动挂上脖子无任何挂的动作）**：道具的**可见状态**（藏衣内不可见／握在手中／佩戴颈上／取出细看／收回）是一条**跨镜连续轴**，每一次状态改变都必须由画面内一个 motivating 动作 beat 演出来。两条硬约束：
  - **(a) ref/描述符 ↔ 上画可见 双向绑定**：道具 `{道具名}=>` ref + 锁定描述符**只进它本镜真正上画可见的 shot**；本镜道具**藏起来/不外露/不上画**的 shot **禁带该 ref、禁写完整外观描述符**（`情节:` 只点名提及，如"衣内玉佩骤热·不外露"）。给了生成器 ref/描述符＝命令它把道具画进画面，与"藏起来"自相矛盾 → 生成器径直画出、没动作就"凭空贴上来"。
  - **(b) 状态改变须动作驱动**：道具从不可见→可见、或佩戴状态改变（取出／戴上／收回／递出），必须写出对应动作 beat（取出＝"探手入怀捏红绳顺衣领掏出"；佩戴＝"提红绳套过头颈理顺垂胸前"；收回＝"拢回衣襟隔衣按贴胸口"）。**禁道具凭空出现 / 自动佩戴 / 自动入手 / 自动消失**；`负面词` 可补"道具凭空出现/自动佩戴"兜底。
  - 机械校验归 `ai_videos__格式契约` K20；动作落地归 `ai_videos__动作表演` A9。

### 4c. Image-first **object** pipeline — 核心物件多角度参考盘（2026-08-29 follow-up duikang_shangzeng/002）

rule 4 锁人物、rule 4b 锁复用道具，两者都只给**一张**正面立绘。
但当项目的**核心主体是一个物件**（汽车广告的车、器物短片的器物、手办/产品片的产品），
一张图锁不住：**物件的身份分布在多个视角上**——车的前四分之三看肩线、正侧看比例与轴距、
后四分之三看轮拱与尾翼。少给一个角度，生成器就会自己编一个，且每镜编得不一样。

因此核心物件 MUST 建一套 **多角度参考盘**，与人物 7s turntable（rule 12.5）同构、参数加密：

| | 人物（rule 12.5） | **核心物件（本规则）** |
|---|---|---|
| 静止落点 | 3 个：0° / 90° / 180° | **5 个：0° / 45° / 90° / 135° / 180°** |
| 样片时长 | 7s 单 take | **10s 单 take** |
| 谁在动 | 镜头绕、人不动 | 镜头绕、物件不动 |
| 附加角度 | 无 | **顶视 + 贴地四分之三**（转盘绕不到，单独出） |
| 台词锚点 | 有（一/二/三 + 标志台词） | **无**（物件不说话，只靠静止落点定时） |

**10s 转盘时刻表（抽帧时间戳＝静止落点中点，下游按此切 object sheet）：**

| 段 | 时段 | 方位角 | 抽帧 t | 用途 |
|---|---|---|---|---|
| 静止 | 0.0–1.5s | 0° | **0.75s** | 正面 |
| 旋转 | 1.5–2.2s | →45° | — | |
| 静止 | 2.2–3.5s | 45° | **2.85s** | 前四分之三 ← **主参考** |
| 旋转 | 3.5–4.2s | →90° | — | |
| 静止 | 4.2–5.5s | 90° | **4.85s** | 正侧 |
| 旋转 | 5.5–6.2s | →135° | — | |
| 静止 | 6.2–7.5s | 135° | **6.85s** | 后四分之三 |
| 旋转 | 7.5–8.2s | →180° | — | |
| 静止 | 8.2–10.0s | 180° | **9.10s** | 正后 |

**双轨制**——结构与外观分开锁，两轨互为输入：

> ⚠ **A 轨先行的顺序已被 rule 4d 取代（2026-08-30）。** 现行顺序是**图先行**：
> 先出无参考的锚点图，派生图挂锚点图，白模反过来由图 image-to-3D 生成。
> 理由见 4d。本节的**转盘时刻表、framing 恒定要求、身份包整包上传**三项仍然有效，
> 只是 A 轨的白模改为在 B 轨之后由图生成。

- **A 轨 · 结构**：物件白模 `.blend` → `tools/render_object_turntable.py` 渲
  7 张定角静帧 + 10s 转盘 MP4。几何**精确**（不是生成猜的），跨角度 framing 恒定。
- **B 轨 · 外观**：对每个角度，用 A 轨该角度的白模渲染当**结构参考图** + 锁定描述符，
  经 Seedream 生成同角度的**写实静帧**。
- **合**：B 轨写实图 + A 轨转盘 MP4 = 该物件的「参考包」。
  ⚠ **每个用到它的 shot 整包随附**（rule 4d §A3）——早先「上传一次建立对象、
  此后每镜只带一张」的写法基于旧模型的槽位限制，已作废。

**跨角度 framing 恒定是这套盘能当 reference 用的前提**：同焦距、同相机距离、同主体居中、
同光位，**只有方位角在变**。任何一项跟着角度变，模型会把「变化」也学进去。
（恒定距离意味着正面看起来比正侧小——这是**对的**，真实转盘就是这样。）

**适用判定**：项目的**核心主体**是物件时必建（汽车广告的车＝S 级资产）。
普通复用道具仍走 rule 4b 的单图 ref 即可；一次性道具不必。

*(来源：duikang_shangzeng follow-up 002 —— 用户要求「参考生成人物的流程，给一套生成多角度
object 的统一流程，正面照/侧面照/背面照，upload 到 Seedance 建立对象」。
实测发现白模渲的定角静帧比纯生成的多角度图一致性高一个量级，因为几何是同一个源。)*

### 4d. 对象建立的 5-prompt 结构 + 参考图 prompt 字段契约（2026-08-30 follow-up duikang_shangzeng/004+005）

rule 4c 建立了「核心物件要一套多角度盘」，但没规定**这套图怎么生、prompt 长什么样**。
本规则补上这两件事，**全仓库所有剧共用**。

#### A. 一个 object ＝ 恰好 5 个 prompt

**object 的定义**：一个能被生成器当作**单一身份**反复复用的主体。
判定标准是「靠什么识别」：**靠剪影与整体比例识别的**（车的外观、一件器物）是一个 object；
**靠部件相对位置识别的**（车的内饰、一个房间内部）是**另一个** object。
两者混进同一个对象里，两边都锁不住——必须拆开、各走一遍下面五步。
...
| 3 | 侧面 | **图1** | 出图模型 |
| 4 | 背面 | **图1** | 出图模型 |
| 5 | **由图生白模** | **图1**（＋2/3/4 若已出） | **Claude（MCP），不是出片模型** |

**三条硬约束：**

1. **图先行，白模后置。** 锚点图是纯文字自由生成的，质量上限最高；白模是它的**下游产物**。
   反过来做（先手工 blockout、拿白模渲染当结构参考）会把写实图的质量上限拉低到 blockout 的水平
   —— 这是 duikang_shangzeng 实测踩出来的坑，见本条来源。
2. **星形派生，不是链式。** 图 2/3/4 各自挂**图1**，图3 绝不参考图2。链式会让误差逐张累积。
3. **图1 过完验收清单才动 2/3/4**；一个 object 五步全部做完、对象建立完成，**才**开下一个 object。
   锚点图是该对象唯一的单点瓶颈，它错一点，后面四个全错。

**Prompt 5 是给 Claude 的任务书，不是给出片模型的**：正文即 image-to-3D 的 MCP 调用
（`mcp__blender__generate_hyper3d_model_via_images` → `poll_rodin_job_status` →
`import_generated_asset`）+ 归一化四步（朝向 / 真实包围盒尺寸 / 原点落底面中心 / 清材质留灰模）
+ 下游重跑命令 + 验收项。白模只承载几何，长相与材质一律交给出片模型。

##### A1b. 白模闸门：归一化与验收是代码，不是散文（2026-08-30）

**Prompt 5 的「归一化四步 + 验收项」不再由人按文字手动执行**——改由
`tools/whitemodel_normalize.py` 执行，输入是每个 object 的 `props/{object}/object.toml`。

```bash
blender -b --factory-startup --python tools/whitemodel_normalize.py --     --src <raw.glb|obj|fbx|blend>  --spec props/{object}/object.toml     --out props/{object}/{object}.blend
# 体检已有白模（不改不写盘）：--qc-only
```

**闸门是 vendor 无关的，这是它的全部意义。** 白模来源可换——image-to-3D
（Rodin / Hunyuan3D 本地 / Tripo…）、素材市场买的高模、脚本 blockout、手工建的。
pipeline **不在这些来源之间二选一，它接受任何一个，只要过同一道闸门**；
genericity 来自闸门，不来自来源。哪个 object 刚好买得到好模型就直接丢进去，
买不到就走 image-to-3D，验收标准完全一致。

为什么值得代码化：质量不稳的真正来源从来不是「选了哪家 vendor」，而是
「拿到网格之后那一段」——四步归一化过去写在每个 object 的 Prompt 5 散文里、
由人逐条手动做，漏一步没人发现；验收清单靠人眼看。两件事现在都是机器做。

**`object.toml` 的两类验收探针**（区域坐标一律归一化到包围盒 0..1，
与真实尺寸和摆放解耦，故同一份清单能复用到重生成的任何一版网格上）：

| 类型 | 测什么 | 典型用途 |
|---|---|---|
| `占位` | 声明区域内是否有几何（`最少顶点`） | 尾翼在不在、扩散器在不在、某部件有没有被生成丢掉 |
| `截面对比` | 两个带之间的截面尺寸关系（`测量轴`/`分带轴`/`带A`/`带B`/`关系`/`最小差`/`限定`） | 外鼓、收窄这类**没有硬边**、占位测不到的曲面特征 |

两条实测踩出来的硬约束，写进契约：

1. **`截面对比` 必须带 `限定`（第三轴范围）**，否则突出于主体的附件会把主体自身的
   信号淹掉——量「后轮拱是否外鼓于车身中段」时，不限定 Z 的话两个带量到的
   都是外凸的**轮子**而非车身，读数双双逼近全车宽。
2. **空带判失败，不判通过。** 带内无几何时 extent＝0，`A > B` 会平凡成立 → 假通过。
   空带说明带范围划错、或该结构根本不存在，两者都必须报出来。

**阈值需要一次校准，这是诚实的前提**：首个被你验收接受的网格落地后跑一遍闸门，
微调带范围与阈值使其通过，**那一版就成为此后所有重生成的回归基线**。
拿一个已知不合格的网格去调阈值使其通过，是把闸门调废——方向反了。

**薄结构**（尾翼、扩散器叶片、器物细部）是 image-to-3D 最不稳的地方，换哪家都一样。
闸门治不好生成，但能**发现**；挂了之后的两条路是重生成、或脚本补件后合并再过一遍闸门
（混合网格走同一道闸门，这也是 generic 的，不是给某个 object 开的后门）。

**缩放默认等比（`--fit uniform`）而非逐轴拉伸**：等比会把「生成网格的比例不对」
留成一个可被包围盒验收发现的问题；逐轴拉伸会把这个缺陷抹平成永远通过。

**探针的能力边界，必须写进每份 object.toml**（2026-08-30 实测）。这些探针都是
**体量测量**，能判定的是：归一化做没做对（包围盒/原点/材质）、某结构**在不在**
（占位）、大尺度**比例关系**（截面对比）。判定不了的是**表面质量与拓扑**——
轮子是嵌在轮拱里还是与一块同样宽的车身面齐平，两者宽度读数完全相同；
曲面是圆顺的还是多面体拼的，体量测量同样无从分辨。
**所以「全部通过」≠「这是个好白模」**，白模的型面好坏仍须看渲染。
不写明这条，一个绿色的验收报告会直接误导下一个人。

**占位探针用面积占比，不用顶点数。** 顶点数是密度相关的：320 顶点的脚本 blockout
与十万顶点的 image-to-3D 网格描述同一个结构时差三个数量级，绝无可能共用绝对阈值——
而闸门的意义正是「同一份验收清单验任何来源的网格」。面积占比密度无关：
2 个三角形描述的扩散器底板与 2000 个三角形描述的同一块板，面积占比相同。

**截面对比需要上界关系（`最大差`），不只下界。** 有些特征要判的不是「A 明显大于 B」
而是「A 不该比 B 大太多」——这类只有上界能表达。只给下界的探针表达不了「包覆」类要求。

#### A2. 落盘结构与路由键（导入功能靠这两样把图放对地方）

object 的每个视角**各占一个 `v{N}_{视角}` 子文件夹**，与场景的 `bg{N}_{方位}_{描述}` 板同构：

```
props/{object}/
├── {object}.md              卡片（锁定描述符 / 5 prompt / 验收清单）
├── v1_{视角1}/              ← 锚点全景的落点（一个文件夹一张权威图）
├── v2_{视角2}/              ← 正面
├── v3_{视角3}/              ← 侧面
├── v4_{视角4}/              ← 背面
├── {object}.blend           prompt 5 产物（白模母版）
└── whitemodel/              渲染脚本产物（**rename-excluded**，见下）
```

**路由键 ＝ `{视角}` 本身，必须是中文且全剧唯一。**
出图工具把下载文件名截到 prompt 首行的前 ~9 个字符：拼音开头的键
（`f80_ferrari_全景` → `f80_ferra`）会把视角整个截没，四张图全部塌到同一个文件上。
中文视角词（`车外全景`）无论怎么截都在。因此：

- prompt **首行只写 `{视角}`**（如 `车外全景`），不写拼音 object 名；
- 视角词跨 object 也要唯一（`车外全景` / `车内全景`），否则导入判为歧义、
  **报 unmatched 而不猜**（宁可不导，不可错导）；
- 一个 `v{N}_` 文件夹只放**一张**权威图，重导即**覆盖**——重跑锚点图是常态。

**脚本产物必须与下载产物分开放。** 渲染/previz 工具写出的文件名是下游契约
（`whitemodel/angles/{name}_{tag}.png`），而导入后的重命名扫描会把任意文件夹里的媒体
统一改名成 `{文件夹名}{N}.ext`。故这类文件夹名登记在 `GENERATED_DIR_NAMES`
（`whitemodel` / `previz`）并被排除；**新增脚本产物文件夹时必须同步登记**，
否则第一次导入就会把它们改名改烂。

*(实现：`downloads__writer.py` 的 `_prop_view_token` / `_match_prop_view` /
`_match_view_any_prop`，测试 `tests/test_downloads_import_prop_views.py`。)*

#### A3. 参考是 per-generation 的，不是「建立对象」（2026-08-30 实证修订）

**产品侧有持久化的「主体」，API 侧是 per-request —— 以产品侧为准**（2026-08-30 用户实测修正）。
即梦的主体 ＝ **一组图片 + 一个 description 文本字段**，存下来后在 prompt 里用 `@` 引用；
ByteDance 的 API 博客只描述 per-request referencing，那是同一件事的下层实现，
不代表工作面上没有主体。**凡产品 UI 与 API 文档口径冲突，以用户在 UI 里看到的为准。**

容量（官方稳定性建议）：单次 **30 图 + 10 视频 + 10 音频**；
**一次生成引用 1–8 个主体效果最好，9–12 个稳定性下降**——所以主体要「少而全」，
不是「多而碎」：把一个地方/一个物件做成**一个**主体，而不是拆成十几个零散参考。

**description 字段就是锁定描述符的家。** 该资产卡 §一 的锁定串**原样粘进 description**，
prompt 里则只写 `@主体名`——这样跨镜措辞不可能漂，因为压根不再重复书写。
...
2. **~~槽位不再稀缺，一镜可以同时带 object 主锚图 + 本镜视角图 + 场景方位板 + 角色图~~**
   ...
   **主体已经持有整套图集**，逐镜再传视角图、场景板、白模渲图全是冗余，
   而冗余参考会稀释主体的权重、增加多主体互认的出错面。

   **shot `参考:` 行的最终契约 —— 只有三类，其余一律淘汰：**

   **写法沿用 §B 第 2 条的 `名称(类型)=>@`** —— `@` 后**留空**，那是用户上传时手填的占位；
   `名称(类型)` 由 Claude 写，作用是让模型知道这一位是什么。**绝不代填槽位号**（`@图片1` 是错的）。

   | 位 | 内容 | 管什么 |
   |---|---|---|
   | `previz_shotNN(previz灰模视频)=>@` | previz 灰模视频 | **形状 · 动作 · 走位 · 机位**（权限最高） |
   | `{主体名}(人物主体/物件主体/场景主体)=>@` | 对应主体 | 各自的**长相** |
   | *(仅承接镜)* `本镜首帧(上一镜末帧)=>@` | 上一镜末帧 | 跨镜首帧承接，**这不是参考图，是首帧槽** |

   `参考:` 行列上传位，`角色:` 行用 `@主体名` 调用主体——两者写法不同是因为职责不同，别混。

   **被淘汰的**：逐镜视角图（主体已含）、场景方位板图（→场景主体）、
   **白模定角渲图**（previz 视频完全覆盖且更强）、色板图（→写进主体 description 或 style_guide）。

   连带后果：**`角色:` 字段不再重贴锁定描述符**，写成 `@主体名 — 本镜状态`。
   锁定串住在主体的 description 里（见上），逐镜重写只会制造漂移机会。
   **状态变体是另一个主体**（青铜像态 / 活马态各一个）。

**白模是官方支持的参考模态**：2.5 明确支持用无材质 3D 模型（clay render）指定
「spatial structure / character poses / motion paths / **camera angles**」。
故 previz 视频不是变通，是**控制机位与走位的正规通道**——也是 §B 第 3 条
`参考用法:` 行里权限最高的那一位（文字与它冲突时以视频为准）。

**已知弱区**（官方自陈）：复杂运动的物理合理性、**多主体互动场景的稳定性**。
凡一镜内有两个以上主体互动（并驾 / 对打 / 交接物件），把 previz 视频列为硬约束，
并在 `参考用法:` 行写明「走位与相对位置一律以预演视频为准」。

#### B. 参考图 prompt 的字段契约（`ai_videos/` 下任何 ref-image prompt 适用）

结构承 `xianjian_yi_mv` 的成熟实践，抽象为通用契约：

1. **首行＝路由键**，与产物文件名一致（下载导入按它落盘）。
2. **`参考:` 行**用 `` `名称(类型)=>@, 名称(类型)=>@` `` 写法，**`@` 是用户上传图片的占位**。
   无参考的锚点图**明写「无 —— 本图是本对象的锚点图，不挂任何参考图」**，不得省略该行。
3. **`参考用法:` 行必写** —— 写死**权限归属**：参考图管什么、本 prompt 的文字管什么、
   **冲突时以谁为准**。这一行是跨图一致性的真正保险，比在正文里堆形容词有效一个量级。
   派生图的标准表述是「参考图是唯一标准，文字只负责把相机搬到新方位角，不一致时以参考图为准」。
4. **字段化**：每个字段独占一行、值用反引号包住；不写成散文段落。
5. **锁定描述符 byte-identical 复用**：同一 object 各图的 `主体:` 行逐字相同，
   且与该资产卡 §一 的锁定描述符、与每个 shot 的 `角色:` 行是**同一串**。
6. **`恒定项:` 行**（派生图专用）—— 逐项列出必须与参考图相同的拍摄参数
   （背景 / 地面 / 阴影 / 光位 / 焦段 / 光圈 / **相机到主体的距离** / 主体在画面中的位置）。
   **相机距离恒定是这套盘能当 reference 用的前提**（同 4c）；正面因此看起来比正侧小，这是对的。
7. **`本视角要点:` 行**（派生图专用）—— 这一张相对其它张**独有**的可读信息，
   告诉模型这一张的存在价值（如正侧是唯一能读出完整轴距与离地高度的一张）。
8. **负向词单独一块**，块前注明「粘进平台的反向提示词输入框，**不要并进正向 prompt**」；
   同一 object 各图**共用同一串**。
9. **合规 fallback**：正文含真实品牌 / IP 专名而平台可能拒绝时，在卡片里预先写好去品牌变体
   ——删掉专名，**首行路由键与其余所有字段一字不改**（几何特征本身足以锁住主体）。见 §16。
10. **≤ 2000 中文字符**（全局 2000 硬顶 / 格式契约 K10），生成时自检，不留给下游校验器。

**物件参考图的标准字段序**（人物走 rule 4 / 12.5，本序专用于物件与空间）：

```
首行路由键 → 参考 → 参考用法 → 主体 → 形体规格 → 识别特征 → 材质细节
→ 姿态 → 机位 → 本视角要点 → 场景 → 光线 → 镜头 → 恒定项 → 构图 → 渲染样式 → 比例
```

**每个 object 的卡片必须带一张验收清单表**（一行一条可证伪的检查项 + 「不过的样子」），
逐条过完才算锚点图定稿。含糊的「看起来对不对」不是验收项。

*(来源：duikang_shangzeng follow-up 004「一个 object 恰好 5 个 prompt，prompt 5 是给 Claude 的」
＋ 005「每个 prompt 要 structured、高质量，参考 xianjian_yi_mv 的实践：要有参考行、要有 @ 占位，
把好的 practice 拿过来并加到 repo root 让将来的剧自动 apply」。
被此规则取代的做法：4c 的 A 轨先行；「派生角度补齐到 6 个」；把内饰当车外对象的一个角度。)*

### 4e. 场景主体：一套场景无关的固定视图集（2026-08-30 follow-up duikang_shangzeng/007+008）

rule 4d 管 **object**（车、器物——**有界刚体**，一圈转台就穷尽了它）。
**场景是无界的**：没有「全部」可拍，镜头背后的东西在任何参考图里都不存在。
所以场景**不能指望参考图自己转方向**——那不是插值，是每镜重新编造。

但这**不等于**要按分镜表去逐个方向出板。**主体是流程资产，不是本片资产**：
同一套规格将来要用在草地、城市、古镇、室内上。**凡「读本片分镜表才能定」的东西，
一律不属于主体**——它属于 shot。

> ⚠ **被废止的做法（2026-08-30，用户纠正）**：早先版本写「方位板数量由分镜表推导，
> 37 镜 → 12 张板」。那是把一次性的分镜表焊进了本该复用的流程里——换一部戏，
> 那 12 个方向一个都用不上。**主体的视图集必须是固定的、场景无关的。**

#### 场景主体 ＝ 固定视图集 + description

**核心四张（任何场景必出）：**

| # | 视图 | 管什么 | 为什么任何场景都需要 |
|---|---|---|---|
| 1 | **平面**·正交俯视 | 空间布局：什么在哪、相对位置 | 草地有地形、古镇有街巷，都有「布局」 |
| 2 | **正向**·眼平广角 | 主look：材质·色板·光·氛围 | 建立镜的那一眼 |
| 3 | **反向**·眼平广角（与②严格 180°） | 镜头背后是什么 | **最省事的一张**：反打/回望不必再编 |
| 4 | **材质**·地面与主表面近景 | 质感锚：草叶/沥青积灰/青石板 | 远景糊掉的东西，近景才立得住 |

**选配两张（按场景性质加，不是按分镜加）：**

| # | 视图 | 什么时候加 |
|---|---|---|
| 5 | **斜瞰**·四五度俯角 | 该场景有空中视点，或体量本身是看点（城市天际线、山谷） |
| 6 | **侧向**·眼平广角（垂直主轴 90°） | 场景不是单轴主导的（广场、庭院、开阔地）；单轴场景（街、隧道、走廊）②③ 已够 |

**description 字段** ＝ 该场景卡 §锁定描述符 原样粘入。prompt 里只写 `@场景主体名`。

**判定「一个主体」的边界**：靠**同一套材质与光**能不能读成一个地方。
`entropy_city` 这样含广场/长街/高架/隧道的，是一个**世界**不是一个场景——
四个地方各建一个主体（长相互不相同，一个主体表达不了）；一次生成 `@` 用到的那一个。
官方稳定性建议是**一次引用 1–8 个主体**，所以主体要少而全。

#### 图先行，3D 在后——**绝不拿 blockout 当出图参考**（2026-08-30 duikang_shangzeng/016 · 二次纠正）

**这条被踩过两次，写死在这里。**

```
① Seedance 自由生成锚点图   ← 无任何参考图，细节/材质/氛围做满，质量上限最高
② 用①出其余角度            ← 同样精致，长相照搬①
③ 用这些图 → Claude 建 3D   ← 3D 是图的下游
④ 从图里挑 → 建 Seedance 主体
```

**`.blend` 永远不是出图的参考源**，它只为 previz（运镜 / 走位 / 时刻）服务。

**为什么不能拿 blockout 渲图当结构参考**：手工 blockout 就是一堆方盒子，
拿它当参考等于把生成图的质量上限锁死在方盒子水平——这与
「rule 4d 图先行」废止「白模渲图当结构参考」是**同一个坑**。
平面图 / 灰模透视图的正确用途只有两个：**人看的空间理解**、**建模与 previz 的依据**。

**布局怎么约束**：写进锚点 prompt 的 `场景` 文字里（几米见方、什么在哪一侧、哪边有开口），
**不靠参考图**。3D 模型日后照生成出来的锚点图去建——契约来自图，不是来自 blend。

**已有 blend 的老项目**：blend 不作废，但降级为 previz 专用；
出图仍从 ① 重新开始，出完再按新图校正 blend 的比例与布局。

#### 落盘顺序 ＝ 依赖顺序（用 `bg{N}` 的 N 编层，不编地点）

`bg{N}_` 的 N **表示依赖层级**，这样目录按名排序就是该做的顺序：

```
bg0_  平面图（渲染）      bg1_  世界锚点      bg2_  地点锚点
bg3_  派生视图            bg4_  状态变体
```

routing 仍只看 `bg\d+_` 之后的视图键，改 N 不影响导入。

#### 一个世界只有一张自由生成图（2026-08-30 duikang_shangzeng/014）

一个 World 下有多个 Place 时，**不要让每个 Place 的锚点各自纯文字生成**——
它们之间没有任何图片纽带，只靠共用的光色/样式文字碰运气，**很容易出成 N 个世界的调子**。
...
平面图（渲染，每地点一张）
        │
        ▼
世界锚点  ← 全片唯一的自由生成图（长相纯文字，无图片参考）
        │
        ├─► 地点锚点 B   ref = 世界锚点(世界基调) + B 平面图(布局)
        ├─► 地点锚点 C   ref = 世界锚点 + C 平面图
        └─► …
                │
                └─► 各地点的派生视图  ref = 本地点锚点 + 本地点平面图
```

- **世界锚点选哪个**：镜头最多、且最能代表世界基调的那个地点。
- **地点锚点的 `参考用法`**：「世界锚点图定**世界基调**（做旧程度、色的冷暖、光的性质、
  微粒密度、胶片质感）一律照搬；平面图定本地点布局；本地点长什么样由文字写，
  但必须落在同一个世界的基调里。」
- **地点锚点不重述光色**——世界锚点图已经带着它了，只写一行「与世界锚点图同一套光色」。

**自由生成的特权只给链条起点那一张。** 它之后的每一张都该被上游约束——
这不是降低质量上限，而是「同一个世界」本身就是约束。

#### 方向从哪来（主体不提供）

| 来源 | 给什么 |
|---|---|
| 场景主体的固定视图集 | 「这是同一个地方」——材质·色板·光·氛围 |
| **previz clay render** | **「从哪个方向、什么高度、第几秒到哪」**——权限最高 |
| shot prompt 文字 | 补视频拍不出来的表演与情绪 |

2.5 官方支持无材质 3D 模型指定 spatial structure / motion paths / **camera angles**，
并有 Blender 插件可直接调起。**逐镜方向是 previz 的活，不是主体的活**——
这条分工正是主体得以场景无关的原因。

#### previz 白模必须把「型面」渲得可读（2026-08-30，duikang_shangzeng 实测）

分工是 **Blender 管几何与走位，Seedance 管颜色·材质·细节·特效**。但这条分工有个
容易漏掉的推论：**几何这一半必须真的被渲进画面，否则它在渲染这一步就丢了，
根本到不了 Seedance 手里。**

为什么要紧：参考图只覆盖有限的几个固定角度（rule 4d 的五步＝锚点＋正/侧/背）。
其余绝大多数机位不在这些角度上——贴地低角、俯拍、跟车侧滑、绕物环移。
**这些角度上「它是什么形状」，模型只能从 previz clay render 里读；读不到就自己编，
编出来就是逐镜形状漂移**（duikang_shangzeng `benchmark_teardown.md` 点名的竞品头号缺陷）。

默认渲染设置会主动抹掉型面信息，三个常见成因：

1. **平光** —— 主光与补光能量接近（≤3:1），灰模在平光下是一片均匀的灰，
   曲面往哪拐完全读不出。主:补拉到 **≈5:1** 并加一盏背侧轮廓光。
2. **无 AO** —— 凹陷、缝隙、轮拱内侧、叶片之间没有暗部。EEVEE Next 开
   `use_raytracing` + `use_fast_gi` + `fast_gi_method="AMBIENT_OCCLUSION_ONLY"`，
   `fast_gi_distance` 取被摄体尺度的量级（车约 0.6m）。
3. **背景与模型同明度** —— 剪影融进背景。背景压到明显低于模型（模型 0.45 → 背景 0.22）。

再加 **Freestyle 描边**（silhouette + border + crease + ridge_valley）：这是灰模上
读转折最有效的一招，且不引入任何美术，不违反「previz 不得携带美术」。
`ridge_valley` 尤其关键——曲面的脊与谷（座舱收窄、轮拱外鼓这类**没有硬边的鼓凹**）
只有它能显形。

**推论：往白模里堆网格细节之前，先确认渲染能读出来。** 渲染侧不修，
换上百万面高模与现有 blockout 在 previz 里的可读性差别有限——先修渲染是
零成本的，而且修完才**看得见**现有白模离够用还差多少，换模型的决策才有依据。

落地：`tools/previz/build_previz.py` §形状可读性 与 `tools/render_object_turntable.py`
两处配置保持一致（同一套 AO / Freestyle / 三点光），否则两处白模观感不一致。

**细节的边界不变**：面缝、格栅网、轮辐、卡钳、材质纹理**不该由白模承载**——
那是 Seedance 从参考图与锁定描述符里拿的。白模要扛的是到**型面语言**为止
（剪影地标 + 曲面走向），再往下是浪费。

#### S1 平面图：先于建模与出图，且是一次免费体检

**无论有没有 `.blend`，第一张图都是纯文字自由生成的锚点图。**
有 blend 时它也只用来（a）帮人理解布局、（b）做 previz——**不进出图的参考位**。

```
① 锚点图 ← 纯文字、无任何参考图      ★ 全片视觉源头
② 其余角度 ← ref = ①
③ Claude 照 ①② 建 / 校正 3D 模型
④ 从 ①② 里挑图 + description ──► 建 Seedance 主体
```

**鸡生蛋是被 Blender 断开的**：链条上第一个不需要参考的环节要么是纯文字（新场景的 ①），
要么是渲染（已有 blend 的平面图）——生成图永远不是链条起点之外的东西的参考源。

平面图渲出来一眼能看出 blend 缺什么。duikang_shangzeng 实测：平面图立刻暴露
「城市」其实只有一个广场 + 一条街，而两个无人机 hero 镜需要真的城市肌理；
补 266 个体块（keep-out 让开全部走廊，既有 previz 构图零影响）才成立。
**建模阶段必跑一次平面图渲染并肉眼过一遍。** 工具：`tools/render_scene_plan.py`。

#### 落盘与路由

沿用既有 scene-plate 约定 `bg{N}_{视图键}_{描述}/`，**视图键全剧唯一**
（rule 4d §A2 同一条理由：下载文件名被截断后只剩它）。
多地点的世界，视图键带上地点：`bg1_广场正向_眼平` / `bg2_主街正向_眼平`。

**变化态**（同几何不同色彩状态，如「褪色态 → 复原态」）跨度小的写进 shot 的
`光线/色彩:` 字段；跨度大到「饱和度近零 → 满色域」的，该场景**另建一个变化态主体**
（同一套固定视图集重出一遍），而不是在原主体里混两种长相。

*(来源：duikang_shangzeng follow-up 007 + 008。008 纠正了 007 的「按分镜表推导方位板」，
理由：主体是可复用的流程资产，凡需读本片分镜表才能定的都属于 shot 而非主体。)*

### 4f. 场景层级模型：World / Place / Look 三种切法（2026-08-30 follow-up duikang_shangzeng/009）

rule 4e 定了**一个**场景主体长什么样；本规则定**一共该有几个主体、它们怎么分层**。
适用于所有剧的所有场景——草地、城市、古镇、室内一视同仁。

#### 核心认识：三样东西被三种不同的不变量切开

它们**不是一棵树的上下级，是同一个现实的三种切法**：

| | 被什么切开 | 承载什么 |
|---|---|---|
| **Blender `.blend`** | **空间连续性**——这些位置之间有没有可比的距离 | **只有几何**（仓库既定分工：3D 只做几何、不碰长相） |
| **Seedance 主体** | **外观不变量**——这组图能否被读成同一个长相 | 材质·光·色·氛围·特效 |
| **Scene 卡** | **叙事地点**——大纲里写「广场」时指哪儿 | 人怎么称呼这个地方 |

证明它们确实是三种切法而非上下级的**不对称**：

> **光影变了 → 新主体，Blender 一动不动**（Blender 不承载光）。
> **被炸毁了 → 新主体，且 Blender 要加几何变体。**

#### 四层

```
L0  世界观风格（纯文字）  被下面各层继承，不产出图
L1  World  = 一个 .blend  切法：空间连续性断了才切
L2  Place  = scene 卡     切法：叙事地点；world 内用 collection 分区
L3  Look   = 主体         ★ 主体 = Place × State
```

**主体 description ＝ 继承链拼出来**：`L0 世界观串 + L2 地点串 + State 串`。
世界观只写一次，不在每个主体里重抄。

#### 判定一：什么时候是**新主体**（外观不变量断了）

> **同一主体内允许的差异上限 ＝ 调色师能调回来的差异。**

- **能后期调回来**（曝光 / 色温 / 对比的轻微差异）→ 同一主体，差异写进 shot 的 `光线/色彩:`。
- **调不回来**（几何变了 / 材质变了 / **光的方向或性质变了** / 增删了物体）→ **新主体**。

据此：阴天→稍暗＝同主体；**日→夜＝新主体**（光的性质变了、路灯亮了）；
下雪＝新主体（材质变了）；炸毁＝新主体。

#### 判定二：什么时候 Blender 要建**几何变体**

> **问一句：这个变化会不会让相机或主体走不了原路？**

会（塔楼倒塌横在路中，挡视线、改走位）→ 在 `.blend` 里加一个 state collection；
不会（墙面焚黑、碎玻璃、地面积灰）→ **不建**，全交给主体的材质层。
**几何变体只为「挡镜头」而建，不为「看起来不同」而建。**

#### 判定三：什么时候**建**这个主体

> **用到才建，但不允许建一半。**

某个 `地点 × 状态` 第一次被镜头用到时才建，但一建就按 rule 4e 的**固定视图集建完整**。
剧本里从未出现的组合（如「隧道 × 夜」）不建。
**注意分工**：*视图集规格*是场景无关的固定一套（rule 4e）；
*有哪些状态*是本剧的故事事实——后者可以读剧本，前者不可以。
...
完损: 完好 | 受损 | 废墟
季节: 春 | 夏 | 秋 | 冬
剧情态: <自由文本>        ← 本剧特有，如「熵增 / 复原」
```

**状态是相对基线的稀疏差量**：每个 Place 在卡里声明自己的**基线态**，
一个 State 只写与基线不同的那几个轴。基线主体的视图键**不带状态标**；
变体主体带一个 **≤2 字的状态短标**（`广场正向` vs `广场复原正向`）。

**变体主体继承基线的平面图**——布局没变，`平面` 视图不重出，只重出 look 类视图。

#### 变体主体怎么写：结构继承，外观整套换

变体主体**不能靠「继承基线文本 + 改几个字段」拼出来**。字段分两类：

| 类 | 字段 | 变体怎么处理 |
|---|---|---|
| **结构** | 机位 / 构图 / 恒定项 / 参考图 | **继承基线**，一寸不改 |
| **外观** | 本地点 / 光线 / 色彩 / **负面词** | **整套换掉** |

实测踩坑（duikang_shangzeng 010）：复原态变体沿用了基线负面词，
里面禁着「彩色灯光 / 鲜艳保持明亮的广告牌」——而复原态的全部意义就是颜色回来了；
`本地点:` 也还写着「积着薄薄一层灰」，与同一份 prompt 的 `色彩:` 行直接打架。
**模型收到互相打架的字段会各挑一半**，产出既不像基线也不像变体。
**负面词最容易被漏**，因为它在 prompt 主体之外、看起来像"全局共用"的东西。

#### 参考图 prompt 的精简三律（2026-08-30 duikang_shangzeng/013）

写完一份参考图 prompt，按这三条自查——都是实测踩出来的：

1. **一个事实只说一次。** 若「无方向主光 / 灰烬色 / 微粒下落」同时出现在 `场景`、`光线`、
   `色彩` 三个字段里，那不是密度、是 word-stacking（CLAUDE.md 明禁）。
   合并同类字段：`光线`+`色彩` → **`光色`**；`机位`+`镜头` → **`机位`**（位置朝向 + 焦段一行写完）。
2. **共用的世界串不得夹带别处的细节。** 实测：共用世界串把「广告屏」带进了**广场**的 prompt，
   而广告屏在主街——不只是冗余，是**有害**，模型可能真给广场画上广告屏。
   **`场景` 字段必须按地点写，只写这个地点有的东西。**
3. **派生图不重述长相。** 长相由参考图携带、`参考用法` 已写明听它的；再写一遍只是
   增加与参考图自相矛盾的机会。派生 prompt 只留：`参考` / `参考用法` / `机位` /
   `恒定` / `构图` / `样式`——**删掉 `场景` 与 `光色`**。

变体（状态改写）用 **`只改:` / `不改:`** 两行取代分散在 `光线`/`色彩`/`恒定项` 里的表述——
一眼就能核对「改的是不是只有该改的」。

实测收效：26 份 prompt 平均 314 字（原 ~850），字段数 8 / 7 / 8（锚点 / 派生 / 变体），
信息一条未丢。**短不是目的，不自相矛盾才是**——重复的字段就是矛盾的温床。

#### 视图集要多小：先做锚点，派生视图按需补（2026-08-30 duikang_shangzeng/017）

**方向由 previz 给，不由参考图给**——这条在 rule 4e 已经写明，但视图集的规模必须照它推论：

- **必做 ＝ 每个 `Place × State` 一张锚点。** 它给「这是同一个地方」。
- **反向 / 侧向 / 斜瞰 / 材质是「该角度的材质保真度」，不是方向机制。**
  它们是锦上添花，**默认不做**——等实拍真在该角度出现漂移了再补（rule 4f「用到才建」）。

实测：entropy_city 5 个地点 + 3 个复原态 ＝ **8 张必做**，
而非「每地点固定 5 视图」的 26 张。**先出 8 张跑通全流程，再按证据补。**

**锚点 prompt 的篇幅下限：1500 字**（硬顶仍是 2000）。锚点是全片视觉源头、且是唯一
零参考的自由生成——它的信息量直接决定全片的细节天花板，写短了后面所有图一起降级。
分层写：**地面 / 主体物 / 围合建筑 / 出入口与远景 / 纵深层次 / 边界高差 /
补丁与年代感 / 家具与遗留物**，每层只给新信息，不与 光色 / 机位 / 构图 / 样式 重叠。
派生与变体不需要这个下限——它们的长相由参考图携带。

#### 落盘结构：一个目录 ＝ 一个主体（2026-08-30 duikang_shangzeng/019）

**别把每个视图各建一个目录**——那样目录数等于视图数，主体的边界在文件系统里看不见。
正确的形状是**目录即主体**：

```
scenes/{world}/
├── _blender/              建模指令 + `.blend` + 校验产物，**不参与出图**
│   ├── blender_build.md   ← 给 Claude 的建模 prompt（引用锚点图），用户贴给 Claude
│   ├── {world}.blend      ← **Claude 建在这里**；previz 按此路径解析
│   ├── plan.png           校验产物：正俯视平面图
│   └── check_{地点}.png   校验产物：与锚点图并排比对的灰模透视
├── bg1_{地点}/            ← 一个 Seedance 主体
│   ├── bg1_{地点}.md      该主体全部视图的 prompt（按依赖顺序排，附出图顺序表）
│   ├── {地点}正向.png     ← 导入按视图名落位，同目录多图互不覆盖
│   ├── {地点}反向.png
│   └── {地点}材质.png
├── bg2_{地点2}/ …
└── bg6_{地点}复原/        ← 状态变体是另一个主体，另一个目录
```

- **`bg{N}` 的 N 编主体的出场顺序**（世界锚点所在的主体排 1）。
- **一个目录一份 md**，装该主体全部视图的 prompt + 出图顺序表 + 每视图的用途与验收 +
  **一份共用的反向提示词**（原先每个视图目录各存一份，重复 26 遍）。
- **description 写在 md 顶部**，建主体时整段粘进去。

**导入端配套**（`downloads__writer` + `media__writer`，2026-08-30 实现）：
主体目录里的下载按**视图键**命名落位（`_view_key()` 取下载名的首个空白分隔词，
须以该目录的某个 token 开头），**重导只覆盖同名那一张**，不再清空整个目录；
重命名扫描对**多视图主体目录**放行（单图目录 `v{N}_…` / `bg{N}_{方位}_{描述}` 仍照旧
重命名为目录名）。这同时修好了长期红着的「同一 prop 多形态变体互相覆盖」。

#### `.blend` 怎么建：一份给 Claude 的建模 prompt（2026-08-30 duikang_shangzeng/020）

`_blender/` 里**不放任何"给生成模型看"的东西**。它只放两类：

1. **`blender_build.md` —— 给 Claude 的建模指令**，是这一步的核心。结构：
   - **前置**：列出必须先出好的锚点图（缺一张就别开始——那个地点的几何只能靠编）
   - **现状**：已有 `.blend` 时说明它的来历与已有内容，让 Claude **校正**而非从零重建
   - **可粘贴的 prompt 块**：`参考:` 行挂锚点图 + `参考用法:` 写死「图是唯一标准、只做几何」
     + 必须落实项（真实米制与尺度校准基准 / 空间契约铁律 / 通行净宽 / 按图校正的比例项 /
     collection 分区与命名 / **只建镜头相关体量，装饰不建**）
     + 逐条验收 + 完成后的存盘与下游重跑
2. **`{world}.blend` 本身**——Claude 照指令建在这个目录里，不放在场景根。
   `build_previz.py` 按 `scenes/{场景名}/_blender/{场景名}.blend` 解析（旧布局作兜底）。
3. **校验产物**：平面图、与锚点图并排比对的灰模透视。
   它们只回答「几何对不对」，**不进任何 prompt 的 `参考:` 行**。

**场景根目录下只留主体目录、`_blender/` 与该场景的流程 md**——
`.blend` / `.blend1` / 渲染图散在根目录会被重命名扫描误伤（实测：根目录的平面图
被改名成 `{场景名}.png`，看着像一张场景立绘，其实是张素模俯视图）。

**"装饰不建"是有理由的**：窗框、招牌、空调外机由出图模型画；建了既拖慢 previz，
又诱使人按「好不好看」去评判 previz——而 previz 只该被按「体量与位置对不对」评判。

#### `参考:` 行写**完整路径**，不写裸名（2026-08-30 duikang_shangzeng/021）

`参考: \`广场正向(长相标准)=>@\`` 是不够的——用户看不出这张图在哪个目录，
上传前得先翻一遍树。**handle 必须与图片在盘上的路径逐字一致**：

```
参考: `bg1_广场/广场正向.png(长相标准)=>@`
```

md 里每个 prompt 块上方再补两行，把「传什么」「存哪」分开写清：

```
**上传**：bg1_广场/广场正向.png(长相标准)
**出图后存**：`bg1_广场/广场正向.png`（导入自动落位）
```

无参考的锚点写 `**上传**：无——本图不挂任何参考图`，不要留空。

**可机检**：把全部 `参考:` handle 与全部「出图后存」路径求差集，
差集必须为空——否则说明某个 prompt 引用了一张这个流程根本不会产出的图。

#### 路由键必须「任意位置可搜 + 落盘只用键」（2026-08-30 duikang_shangzeng/024）

出图工具会把 prompt 派生的名字**包在自己的前缀与时间戳后缀里**。实测：
...
1. **键在文件名任意位置搜索**（`re.search`），**绝不锚定开头**。
   锚定会静默回落到「用目录名命名」，产出看似合理实则错误的文件名。
2. **落盘名只用键**（`bg{N}-{M}.png`），**不从下载文本里提取任何东西**——
   这才是对「工具想怎么装饰就怎么装饰」免疫的做法。
   人可读的中文名住在主体 md 里，不进文件名。

键的形状：`bg{N}-{M}` —— 纯 ASCII+数字、极短（5 字符）、`{N}` 全剧唯一。
prompt 首行写 `bg{N}-{M}_{视图名}`（给人看），工具据此命名，导入只认其中的 `bg{N}-{M}`。

#### 文件名可能带**两个**路由键——最左匹配胜出（2026-08-30 duikang_shangzeng/025）

出图工具会把 prompt 的**前若干字符** inline 进文件名，而其中包含 `参考:` 行，
`参考:` 的 handle 里又含另一个视图的键。实测：
...
**取最左匹配**——这是构造上的保证而非运气：路由键是 prompt 的**首行**，
必然排在下方 `参考:` 行引用的任何东西之前。写 prompt 时因此有一条硬约束：
**路由键必须是第一行，`参考:` 永远排在它下面。**

#### 反模式（明确禁止）

- **为了「更多细节 / 光影 / 特效」而拆更细的主体。** 细节靠**给主体加视图**解决
  （加 `材质` 近景、加特效视图），不靠拆主体。拆碎的代价：① 官方建议一次引用 1–8 个主体，
  很快撞上限；② 一个地方没有唯一权威主体，每镜都要选，选错就漂。
  **拆主体的唯一理由是外观不变量断了。**
- **把「sub-scene」当成一个独立层级。** 商铺内景该独立，不是因为它是主街的子场景，
  而是因为它的外观不变量与主街断了——L2 的地点切法已经把它切出来了。
  目录上嵌套是导航方便，不是语义层级。

#### 变更传播表

| 发生了什么 | L1 Blender | L2 Place 卡 | L3 主体 | previz |
|---|---|---|---|---|
| 光影 / 天气 / 时间变 | **不动** | 加一个 State 条目 | **新主体** | 不动 |
| 破坏且挡镜头 | **加 state collection** | 加 State | **新主体** | 受影响镜重渲 |
| 破坏但不挡镜头 | **不动** | 加 State | **新主体** | 不动 |
| 同世界里新地点 | 不动 | **新 Place 卡** | **新主体** | 新镜才需要 |
| 新世界 | **新 .blend** | 新 Place ×N | 新主体 ×N | 全新 |
| 只是想要更多细节 | 不动 | 不动 | **不新建——给现有主体加视图** | 不动 |

*(来源：duikang_shangzeng follow-up 009。三个判定由用户拍板：几何变体只建挡镜头的、
主体用到才建但规格固定、State 用封闭枚举加自由扩展位。)*

### 4g. 环境资产：3D 承载几何，主体承载长相（2026-08-30 会话沉淀 · 苏州古镇用例）

面向**多镜头共用同一座城/镇/村**、且含**长镜头穿行**的项目。rule 4d 管 object，
4e 管场景主体，4f 管层级；本条管**环境的几何该怎么建、一致性靠什么保证**。

#### A. object 与 environment 是两类，规则不同

| | object | environment |
|---|---|---|
| 定义 | **有界刚体**，一圈转台就穷尽它 | **你在它内部**，没有单一剪影，转不了台 |
| 例 | 车、玉佩、器物、乌篷船、石狮 | 古镇、街区、庭院 |
| 建法 | rule 4d 五步 + image-to-3D + 闸门 | **本条**：体块 + 构件拼装 + 场景主体 |

**把 environment 丢给 image-to-3D 是错的**，且是系统性的错：生成模型给不出直角、
给不出统一地平面、给不出重复开间节奏，一片建筑群会出来一坨有起伏的疙瘩。

#### B. 什么时候才该用生成式 3D —— 四条判别，按顺序问

1. **有没有现成资产可买/可下？** → 买。质量最高、最便宜
2. **它是不是直线／模块／精确尺寸的？** → **建模或脚本**（墙、屋顶、街面、台座）
3. **它是不是有机曲面的、有界的、且只以图片形式存在？** → **这才是 image-to-3D**
4. **它是不是一个环境？** → **永远不用 image-to-3D**

生成式 3D 在建筑上的四个失败模式（与汽车正相反，2026-08 实测与文献一致）：
**平面会翘**（车的曲面藏得住起伏，墙藏不住）、**重复模块会融化**（每个窗略有不同＝melted）、
**尺寸不可靠**（视觉可信但不能据以施工）、**光照被烤进资产**（参考图的阴影高光可能被当成几何刻进去）。

#### C. 环境的正解：**生成构件，不生成建筑**

中国传统建筑本身是模块化的（材分制／斗口模数），这给了一条干净的分界：

| 走 image-to-3D（有机、繁复、有界的小件） | 走脚本／建模（平直、重复、精确） |
|---|---|
| 门楼 · 木格窗 · 花窗 · 斗拱 · 雀替 · 撑拱 · 柱础 · 瓦当滴水 · 屋脊兽 · 桥栏望柱 · 系船石 · 乌篷船 · 石狮 · 幌子 | 墙体 · 楼板 · 屋顶坡面 · 马头墙阶梯轮廓 · 街面 · 驳岸 · 桥拱 · 台阶 |

**依据就是上表的失败模式**：生成式擅长有机繁复，脚本擅长平直重复，各干各的强项。

收益：① 构件是小的有界物体＝image-to-3D 甜区，与 rule 4d 同一套闸门**原样复用**；
② 生成一次无限复用，**重复的窗户是 byte-identical 的，不会融化**；
③ 墙由脚本建，所以永远平、直、垂直；④ **等于用 AI 造自己的资产包**。

先定**模数**（如开间 3.3m），所有构件按模数出，拼装时自然对齐——这解决共墙／贴巷／临水的贴合问题。
**重复感**在拼装层解决（逐实例旋转/缩放微抖 + 材质变化），不在生成层解决。

#### D. 一致性有四层，各有各的保证人

| 层 | 内容 | 谁保证 |
|---|---|---|
| **几何** | 楼在哪、多高、巷多宽、桥在哪、转角看到什么 | **只有 3D**。场景主体做不到 |
| **长相** | 色调、材质、年代、风化 | 场景主体 + 锁定描述符（rule 4e） |
| **光照** | 时辰、光向、天气 | 3D 灯光 + prompt `光线:` 字段 |
| **衔接** | 相邻镜首末帧承接 | 跨镜首帧承接机制 |

**跨镜漂移几乎全在第一层，而第一层只能靠 3D。** 场景主体给的是**风格一致**，
不是**几何一致**——长镜头会看到固定视图覆盖不到的角度，那些角度模型只能编，
这镜编一个样、下镜编另一个样。这是车型漂移的场景版，且因内部角度无穷而更严重。

**一致性来自「只有一份几何」，不来自模型的记忆。** 全项目一份 `{world}.blend`，
按 rule 4f 的 place 前缀分区，所有镜头的 previz 从同一份渲。
这条是整个架构的地基：不是教模型记住这座镇，而是每一镜都把同一座镇的正确视图递给它。

#### E. 几何精度按**镜头接近度**分级，不按「是不是主角」

| 级 | 谁 | 做到什么程度 |
|---|---|---|
| **A · 贴身而过** | 长镜头沿街走过的那几栋 | 真立面：墙 + 窗 + 门 + 檐口 + 马头墙轮廓，构件实例化 |
| **B · 中景** | 街对面、水对岸 | 体块 + 屋顶形制 + 开窗位置，不用构件细节 |
| **C · 远景剪影** | 天际线上的 | 纯体块 |
| **D** | 镜头永远看不到的 | **不建** |

**镜头动得越多，需要的几何越实。** 固定机位短镜可以靠场景主体；
**一镜到底穿行是 Seedance 最弱、3D 最强的地方，全程几何必须实。**

#### F. 镜头先于几何

**先画长镜头路径（走哪条街、经过哪几栋、在哪转弯），再定几何精度。**
反过来先建镇再想拍什么，会在错的地方花力气。
建模阶段仍须跑一次平面图渲染并肉眼过一遍（`tools/render_scene_plan.py`，见 §S1 平面图）。

#### G. ID 色索引契约（多主体同框时的身份绑定）

**上色 / 灰模的分界线**——依据是「形状由谁提供」：

| | 处理 | 理由 |
|---|---|---|
| 人物 / 可动道具 | **ID 色 + 自发光** | 身份要分辨，**形状由参考图提供**，型面被抹平无所谓 |
| 建筑 / 车 / 器物 | **纯灰模，不上色** | **形状必须由 previz 传递**，参考图覆盖不到其余角度 |

⚠ **ID 色必须正反向成对声明，否则会被当成戏服渲进画面。** 正向写明性质：

```
角色索引: previz 视频中的纯色人形为身份标记，非服装、非灯光。
          绿色标记 = @人物1，蓝色标记 = @人物2。最终画面中不得出现这些标记色。
```

反向必须追加：`绿色衣服、蓝色衣服、纯色人形、色块人物、荧光色服装、参考视频的标记色出现在画面中、发光的人`

**再加一层剪影差异**（高瘦长袍 / 矮壮短打）：颜色可能被误读，**剪影不会**，
且服装体积本来就是布料运动所需的信息。
最终是**四重冗余**：主体名（`@`）、ID 色、剪影、方位——任一层失效其余三层仍在。

#### H. 一镜内主体数 ≤ 8 —— 这是**分镜阶段**的硬约束

官方稳定性建议一次引用 1–8 个主体，9–12 下降（rule 4e）。所以：
**一整条街做成一个场景主体，不要拆成十几个建筑主体**；
只有剧情要点名的建筑（"她住的那间绣楼"）才单独建主体。
一个镜头需要被点名区分的实体超过 8 个，**该拆镜**——写分镜时就守住，不是出片时才发现。

#### I. 人物代理：3D 给什么、不给什么

脸交给主体参考图（多角度照）+ Seedance。**3D 层不做 MetaHuman、不做面部绑定、不做布料解算。**

| 3D 必须给对 | 为什么 AI 补不了 |
|---|---|
| 身高与比例 | 代理矮了，渲出来就是矮的 |
| 走位与时刻 | 谁第几秒走到哪、何时转身——这是叙事不是渲染 |
| 遮挡与接触 | 脚踩地、手扶栏、被柱子挡住。**AI 编不出正确的遮挡** |
| **服装剪影** | 宽袖长袍的**体积**。给光身人偶，AI 要从零编整件袍子的形与动，那是它最不稳的地方 |

**脸不难，布料难**——有多角度参考图时人脸一致性已经很好，帧间闪烁主要发生在垂坠与飘动上。

#### J. Blender 还是 UE5

判据是**这一层的产物是「语义地图」还是「美术」**。
若 3D 渲染的任务是告诉模型「哪个是人物1、哪个是建筑1」，则需要身份可分辨、
位置准确、遮挡正确、运动连贯，**不需要照片级**——而照片级恰恰是 UE5 相对 Blender 的核心优势。

**更要紧：照片级渲染在这里有害。** 给模型一段「看起来挺真但不完全对」的渲染，
它会去抄这段渲染的长相（含通用资产的破绽）；给它干净的灰模，它没得抄，
只能从参考图取长相。这与 rule 12.16「previz 不得携带美术」同源。

**默认 Blender**：整条工具链已在跑（闸门、渲染器、previz 引擎、ID 色机制）、可无头脚本全自动驱动、
灰模正是要的输出。语义地图不需要百万面构件，**减面后 Blender 的场景承载问题即消失**
（Nanite 的意义是让你不用减面，而此用途本就该减面）。

**切 UE5 的触发条件**（满足任一）：① 长镜头运镜需要实时试镜；② 建筑 > 50 栋或实例 > 2000；
③ 人物动画复杂到需要动画蓝图与重定向；④ 决定让 3D 层直接出接近成片的画面（分工改变）。
资产是 glb，两边通吃，先在 Blender 做不会白做。

*(来源：2026-08-30 会话——duikang_shangzeng 白模闸门实测 + 苏州古镇多长镜头用例推演。
Hyper3D/Rodin 的 vendor 决策与许可判定见 §4d §A1b 与 specs/ai_video/duikang_shangzeng/changelog.md follow-up 015。)*
...
> **⛔ 全流程默认关闭（follow-up 2026-06-20「所有 shot prompt 都不要烧字幕，加进流程」）。** 默认**不烧任何字幕**：pipeline **不再生成** per-shot `subtitles.md`，stage-6 不产出字幕时间轴；字幕统一由用户在后期自行添加（与 rule 12.4 「prompt 内不含字幕、字幕后期另加」一致）。下方的 `subtitles.md` 格式 + webapp 烧字幕按钮（📝生成台词 / 💬中文·EN·中英 / 按语言合成整集）**代码保留**，仅作**用户手动 opt-in** 时可用，**不属默认产物、不参与格式契约校验、缺失绝不报错**。新项目默认无 `subtitles.md`；旧项目已生成的可直接删。整集合成默认走「原片」`ep{NN}.mp4`（无字幕版）。

渲染完成后把台词（含内心独白 OS）按时间烧进 shot mp4 的 render-side 后期步骤。**与 prompt 层解耦**：shot 视频 prompt 里的 `台词:` 字段不变，本规则只新增一个「渲染产物 → 带字幕渲染产物」的后处理。**（以下为 opt-in 时的实现细节，默认不触发。）**
...
### 11d. 人物出场字卡（character intro card / nameplate）— 后期叠层 · 重要角色首次登场

重要角色**首次登场**时：在画面**顶角浮现一张完整的人物名牌卡图**（角色名 + 一句身份），淡入淡出。**视频不停顿、照常流畅播放**（无 freeze/定格——卡图只是叠在动态画面上）。**卡图 = 美术成品 PNG（透明底），由用户用 Kling 生成、不由程序绘制**（程序画框/字效果差，2026-06-20 用户反馈后改为图片合成）。这是与 11c 字幕烧录同类的**后期叠层**，**与 prompt 层解耦**——shot 视频 prompt 的 ```text``` 块**绝不写字卡文字**（守 rule 12.4「prompt 不含画面文字」/ K4 / K19 契约）。
...
- **出现时机 + 可读时长（重要）**：字卡**在该角色一出场/入焦的那一刻浮现**（新人物一露面就出卡）；**显示时长 ≥ ~3.5s 才看得清**。故**该角色首登镜的首句台词 / 焦点在场窗口必须够长（≥ 字卡显示时长）**——若太短（观众还没看清卡就没了），**适当加长其首句台词 + 焦点 beat**（连带 shot 时长，3-way 同步 dialogue/script），不要让重要人物的出场一闪而过。
- **卡图存角色文件夹内（不另开 folder）**：卡图就放在该角色的 `2_世界观人设/characters/cN_{中文名}/` 里——`intro_card.md`（出图 prompt，开头带「{角色名} · 出场名牌卡」做导入路由 token）+ `intro_card.png`（用户出的整张卡图·透明底）。同角色 reference 资产聚在一处，不再单开 `intro_cards/` 库。
- **道具库（玉佩等 prop reference）**：`2_世界观人设/props/{道具名}/`（与 characters/scenes 同层），每道具一对 `{道具名}.md` + `{道具名}.png`。与人物卡同走「Kling 出图 + 导入」流程。
- **每集一个 `episodes/epNN/intro_cards.md`**（与 `shotlist.md` 同级）。行式、可 grep，每个重要角色一行：`角色 | 首登 shot | 出现点(秒) | 卡图 | 位置(右上/左上) | 显示时长(秒) | 宽度占比`。`卡图`=**角色名**（writer 在本剧找 `characters/*{角色}*/intro_card.png`）或路径；`宽度占比`=卡图宽÷视频宽（默认 0.28）。无重要角色首登的集可省此文件。
- **shot 侧（首登镜引入人物卡 + 构图留白）**：角色首登镜在 `## Shot context` 加一行 `首登字卡: 角色X → 卡图 characters/cN_X/intro_card.png（顶角）；最终 shotNN.mp4 = render + 该卡图合成`，并确保该角色在出现点附近**入画清晰**。
...
*(Originated from follow-up "重要人物首次登场：定格 + 美工字卡介绍（裴知秋·镇北王府…）；加进流程、全剧默认、只对重要角色、龙套不发" — 2026-06-20；产物落点 per follow-up "renders 下是原片不可覆盖；生成物放外面、直接叫 shot{NN}.mp4、二次烧录可覆盖" — 2026-06-20；**改为图片合成 per follow-up "程序画的效果不好，改成 Kling 做模板卡图、上传 _intro_cards 库、烧录时直接叠进去" — 2026-06-20**（弃程序化 ASS 画框/字，改为整张卡 PNG overlay）。)*
...
>   - **`参考:` 行配套「声音参考」token（2026-06-28 follow-up 078）**：`## 视频 prompt` 的 `参考:` 行里，**每个在本镜有台词的角色，在其视觉 ref token 之后紧跟一个 `{角色}声音=>` token**（视觉 ref 与声音 ref 配对出现），并在 `## Shot context` 的 `Reference uploads` 同步列一条「{角色}声音（voice_id {VID}·声样源）」。例：`参考: \`萧若云=>, 萧若云声音=>, 围观武者乙=>, 围观武者乙声音=>, 镇演武场_bg1_场口_入场=>\``。**声音 ref ＝该角色锁定 voice_id 的声样源**（turntable 0–2s 统一声样 trim / casting.md voice_id），与本镜 `## 台词配音 prompt` 的 `音色(锁定 voice_id)` 一一对应——把"用哪条音色"在视频 prompt 层也显式上成参考（视觉 ref 锁脸/造型、声音 ref 锁音色）。规则：**声音 ref 的触发＝该角色在本镜有「台词」或「内心独白 OS」**——二者皆无＝本镜不发声→**不配 `声音`**（哪怕入画也只挂视觉 ref，如公主只仪仗驾临不开口的镜、台下只反应不说话的配角）；**纯 OS/画外内心独白（发声但不入画）只挂 `{角色}声音=>`**（无视觉 ref）。**按镜判定、不跨镜继承**（同角色 A 镜说话→配声音、B 镜只露面→不配）。声音 ref 同走 inline `=>@N` 槽位号（如 `{角色}声音=>@k`·@k 为其音色源槽，编号按 `参考:` 出现顺序连号）。机检归 `ai_videos__格式契约`（声音 token 齐全＝K29、inline @N＝K27）。
...
>   - **内心独白(OS) = 发声镜、非默剧（2026-06-22 follow-up「内心独白不要视频静音、视频要有声音」）**：OS / 画外心声**必带 `## 台词配音 prompt`（锁定 voice_id，如主角 `PR-hero-01`）**；成片**必须用 `tools/mux_av.py` 把 OS 台词 MP3 mux 进该 shot 视频**——mux 视频流 copy + 写入新音轨，**自动覆盖** Seedance 自带的任何 baked 音，故得到**锁定音色**的 OS 旁白、且必有声。**OS 交付件绝不静音、绝不当默剧/静默镜处理**（省略配音块的豁免只对真正无台词镜）。OS 句嘴不动的口型契约不变（视觉上不张嘴，声音是画外旁白）；无需为锁音色而在 prompt 里改 `台词:` 或加人声抑制负向（mux 覆盖音轨即可，避免 K22b 那类"啰嗦反把声压没"）。**⚠ 坑（2026-06-27 ep3 shot05 实测变哑）：单声轨 OS 镜的 `台词:` 字段表头禁写「视频静音」**——rule 704「Seedance 从 `台词:` 文本生成台词配音」对单声轨镜照样适用于 OS，`台词:` 表头写「视频静音」会让生成器**不 bake OS 人声 → 成片对该镜变哑**。单声轨 OS 镜 `台词:` 表头一律用 canonical `（画面不显示文字、仅供口型与配音参考；逐条↓）`，**不写「视频静音/不生成人声」**。「视频静音 + 全 mux」只属**多声轨/系统音色镜**（rule 762/K22b），不可漏到单声轨 OS 镜。
>
...
> **⚠ 2026-06-27 amendment — 角色建立模式统一：每个人物（主角+配角+龙套）一律先生成 7s 建立视频（turntable），不分出场方式.** Per follow-up「把所有人物的建立模式都变得 consistent，都需要先生成一个 7 秒视频，不管主角还是配角」。**取消**旧「出场角色 checklist 派生规则」表里 turntable 的**按出场方式条件化**（正脸✅必需 / 全景✅推荐 / 背影·远景·剪影❌无须）——改为：
> 1. **每个命名人形角色都必须有一个 7s 建立视频块（rule 12.5 turntable，5-phase locked-framing single-take）**，无条件、不分主配、不分出场方式（正脸 / 全景 / 背影 / 远景 / 剪影 / 群像里的具名个体都要）。它是该角色后续所有 shot 的视觉 reference 来源；先有 7s 建立视频、再进 shot。
> 2. **输入分两种**：① 有 `_cast.md` 选角脸的角色（主角 / 核心角色）→ turntable **吃选角脸**（line 418）；② 仅有自身 ref 立绘 png 的轻量配角 / 龙套（无选角脸）→ turntable **以该角色自己的 `cN_名字.png` 立绘作参考图**（`参考: 上传该角色 ref 立绘图作参考脸`）。两种都念**统一声样台词**「你好，今天天气还不错，外面很安静。」(0–3s)、走同一 5-phase 结构、≤2000 字、9:16 / 7s、生成块内不含真人演员名。
> 3. **唯一例外＝非人形实体**（「系统」/ UI 等）：无 turntable，沿用 UI 浮现块；纯道具 / 法宝（无人形）亦无 turntable。
> 4. **下游不变**：turntable 是否必需 ≠ 是否列入 shot `参考:`（line 708 仍成立）；纯 OS / 画外（完全不入画）仍不列 `参考`。但凡入画的命名人形角色，turntable 一律已备。
> 5. **范围＝流程级 + 现在回填**：所有 ai_video 项目；wushen_juexing 现有 8 个缺块配角（c6/c7/c8/c9/c10/c11/c12/c15）本次补齐建立视频块。`ai_videos__格式契约` K-turntable 同步：校验**每个命名人形角色卡含建立视频块**（不再按出场方式豁免）。
> 6. **角色卡原子性（1 step·no-中间态·2026-06-28·follow-up 080）**：建一张命名人形角色卡是**原子操作**——要么不建此卡，要么**立绘 ref prompt + 建立视频块（turntable）一次齐全**。**禁止**把 turntable 留成 `待出 ref 后生成` / `待生成` / TBD 之类的延后占位行（「只有立绘、turntable 待出」的半成品＝违规中间态）。轻量配角无选角脸也照样在建卡时**直接以自身 `cN_名字.png` 立绘**写全建立视频块、不延后。机检：`ai_videos__格式契约` K16b 把任何 turntable 延后占位行视为**缺块 blocker**。（本条因 wushen_juexing c20/c21/c22 三个新配角只出了立绘、turntable 被延后而立——已回填补全。）

> **⚠ 2026-06-27 amendment — turntable 建立视频压缩 7s → 4s（在 4s 内完成 正面→侧面→背面）.** Per follow-up「把所有人物的视频 prompt 都改为 4 秒，要在 4 秒内完成原来的动作，正面到侧面还有背面；提取 3 视图的功能也相应改（时长变了）」。**turntable 时长由 7s 改为 4s**，保留同一 front→side→back 三段静定弧，只是压进 4 秒。**取代**前述 amendment 里所有「7s / 7 秒」turntable 时长说法（其余内容不变）：
> 1. **新 4s 5-phase locked-framing single-take（3 静定落点 + 2 短转桥）**：`0-1s 正面静定`（念统一声样台词「你好，今天天气还不错，外面很安静。」0–2s 念完、0–1s 正面口型清晰）→ `1-1.5s 转左 0°→90°` → `1.5-2.5s 左侧身静定` → `2.5-3s 转背 90°→180°` → `3-4s 背面静定（锁机位仅呼吸）`。机位全程不动、不 dolly/zoom，仅主体自转。
> 2. **抽 3 视图时间戳同步改**（落各静定段中点）：front `t=0.5s` / side `t=2.0s` / back `t=3.5s`（旧 1.0/3.5/6.0 作废）；webapp `character_video__valueobject.py` 的 `CANONICAL_VIEWS` 已同步。trim（前 2s 声样，前述 trim=2s 不变）即 0–2s，覆盖正面念词段。
> 3. **角色卡字段同步**：建立视频块标题 `# turntable 角色 reference prompt — 4s 建立视频`、`时长: 4s`、`音频:（0–2s）`、`负向: …不要超过 4s`、`统一声样台词`表时间窗 `0–2s`、`turntable 说明`的画幅行与抽帧点（front 0.5s/side 2.0s/back 3.5s）全部按 4s。**动作内容不变**（同样的正侧背转身），只压时长 + 改时间窗。
> 4. **范围＝流程级 + 现在回填**：所有 ai_video 项目；wushen_juexing 现有全部 17 个命名人形角色卡（c1/c3/c4/c5/c6/c7/c8/c9/c10/c11/c12/c13/c14/c15/c16/c17/c18）本次回填到 4s。`格式契约` K16/K16b 的时长/时间窗/标题匹配按 4s（标题匹配做成时长无关）。
...
> 2b. **游戏 IP 名 + 引擎商标禁入（2026-06-21 wushen_juexing 实测扩充）**：风格串 / 场景 seed prompt **不得点名商业游戏 IP 或引擎商标**——`黑神话悟空 / 原神 / 虚幻引擎 / Unreal / Genshin` 等触发 IP 审核拒（字节已因 Disney/奥特曼侵权函收紧 IP 拦截）。要 AAA 游戏 CG 质感改**描述性等价词**：`黑神话悟空/原神既视感` → `3A级国风游戏CG大片既视感`；`虚幻引擎次世代实时渲染` → `次世代游戏引擎级实时渲染画质`（"3A/AAA"「游戏级」「开放世界」属通用词、不点名即可）。
> 2c. **暴力叙述词替换（2026-06-21 实测扩充）**：社区+实测确认 `杀 / 刺 / 砍 / 斩 / 死伤 / 无数 / 血战 / 踏破 / 厮杀` 触发内容审核（视频尤严）。说书/旁白/情节叙述战事时换中性词：`踏破` → `连破`；`死伤无数 / 血战死伤无数` → `节节败退`；`血战` → `苦战`；`杀妖` → `伐妖 / 镇妖`；`厮杀` → `激烈交锋 / 快速碰撞`。良性复合词也建议软化避误伤：`死寂` → `沉寂`；`血色正常` → `气色正常`；`暴闪` → `过曝乱闪`；走位指令 `写死` → `固定 / 锁定`。**注**：仅改进生成块的字段（情节 / 小说原文 / 台词 / 场景 / 走位 / 光线 等会送 Seedance 的文本）；纯 planning 注记（铺垫锚 / 钩尾说明里的「杀妖」剧情概念词）不送模型、可保留。
> 2d. **负向词也被审 + 校园暴力/未成年标记敏感（2026-06-30 rexue_gaoxiao 实测）**：即梦实测对 `c3_廖彦廷` 人物 turntable 生成块「多次显示内容不符合平台规则」。两条新教训：①**负向/反向词同样过审**——生成块 `负向:` 里写 `不要会打架的狠相 / 不要血腥` 等，违禁词 `打架 / 血腥` 即便处于否定语境也命中关键词审核被拒。规则：违禁暴力词从生成块**整体移除**（含负向），反向意图（如「别显得能打架」）改成**正向中性描述**写进 `神情/气质`（「气质温和书卷清秀平实」），不靠含违禁词的负向句。②**校园暴力题材的未成年标记敏感**——`17 岁 / 少年 + 学生服 + 暴力剧` 组合易触审，生成块去掉显式年龄数字与「少年」，改 `青年学生 / 学生`（剧情设定的真实年龄保留在描述符表 metadata、不进 ```text``` 块；锁定 byte-identical 描述符本就不含年龄/暴力词、shot 不受影响）。范围＝流程级，校园/暴力/涉未成年题材尤须自查。③补充软触发词：`日系学生服 / JK 校服`（擦边+国别双敏感）改 `深色立领校服 / 现代校服`；显式 `高中生 / 高中` 词同未成年标记，改 `学生 / 校园`。④**name vs content vs 参考图三分诊断**：报「内容不符合平台规范」先分清是谁触发——(a) 角色中文名几乎从不触发（除非含敏感词/谐音），可排除；(b) 文字内容（违禁词/未成年/日系/引号待渲染文字）；(c) **上传的真人脸参考图**——即梦对真人脸 reference 常直接拒（人脸属敏感个人信息，见 §593/nvdi020④），这是「内容不符合平台规范」高频元凶且与文字无关。**隔离法**：先 text-only（不传脸）试跑——能过＝触发在参考脸图（改走纯文字描述/自有非真人参考），仍挂＝触发在文字（继续清词）。
...
**⚠ 2026-06-21 amendment — 真人古装仙侠剧出片铁律（编剧/情节/分镜大师 + 生成 默认遵守·follow-up 034 用户实测密集反馈固化）.** 多镜实测暴露一批「不像真人古装剧」的系统问题，定为流程级铁律（生成时默认落、`ai_videos__剧情连贯`/`运镜`/`时长节奏`/`动作表演`/`格式契约` 审查时默认查）：

> 1. **真人实拍·零致假词（分镜+生成）**：风格＝真人演员实景拍摄的影视剧（photorealism·真实皮肤毛孔·胶片颗粒·自然光·35mm）。**渲染串与各字段禁出现致假词**：`游戏 / 3A游戏 / 游戏引擎 / 全局光照 / 体积光 / 次世代 / CG / 工笔 / 唯美渲染 / 史诗级(堆砌)`——这些即便配 photorealism 也把输出拽向游戏CG/插画/磨皮假脸。细节靠「实景置景考究丰富」、美感靠「电影级自然光影」。负向每镜挂 `不要 CG渲染感/游戏感/游戏引擎画面感/3D建模感/磨皮/假脸/失真塑料感`。
> 2. **中式古风·非西方（情节+分镜）**：古风场景禁西式既视感——演武场=青石法坛+飞檐宗门阁楼牌坊+云雾仙峰（非斗兽场/角斗场/环形看台/罗马柱）；负向挂 `不要 西式斗兽场/角斗场/环形看台/罗马柱式/西方游戏感/西方审美面孔`。
> 3. **群像古装发型 + 面孔多样（生成+格式契约）**：所有群像/路人/配角必写 `古装束发长发(束髻或裹头巾)·非现代短发` + `面孔各异老中青胖瘦不一·非千人一面同脸`（薄描述会让模型默认现代短发+复制脸）；负向挂 `不要 短发/寸头/现代发型/千人一面/群众同脸/复制粘贴脸`。
> 4. **相邻镜过渡铺垫·禁静态硬切（运镜·分镜大师）**：一镜不要静态收尾（站定+OS）直接硬切下一镜的动态起手——**镜末留运动或视线接点**（哪怕无言走两步、或闻声转头望向下一镜方向），在 `镜头:`/`动作:`/`情节:` 写明接点。`ai_videos__运镜` 跨镜审查加「镜末运动/视线接点·避免静态→动态硬切」。
> 5. **台词↔画面具体物件一致（剧情连贯·情节大师）**：台词点名的具体物件/店铺/人（如「酒楼、药铺、铁匠铺」），**画面 `场景:` 必须真有这些元素、`走位:`/`动作:` 主角视线依次落到对应物件**。`ai_videos__剧情连贯` 加「台词点名实体 ↔ 画面可见 + 视线对应」检查。
> 6. **功能角色要有反应·别走过场（剧情连贯+动作表演）**：每个上场的功能角色（受测者/被处置者/旁观者）对眼前事件要有可信反应与交涉，别「一句话带过直接切下一个」——如测资：下品被招狂喜、废品不甘求再试被赶、中品「谁要→宗门抢→道谢」，让机制有血有肉。台词归属写死（谁说哪句、谁对口型），杜绝把 A 的台词安到 B 头上。
> 7. **正常语速·超 15s 拆镜（时长节奏·编剧大师）**：台词按**正常语速**给足时长——**每镜「字数÷时长 ≤ 4」**（统一标准·follow-up 2026-06-23：旧 ≤5 太赶念不清、≤3.5 偏慢拖；**≤4 ≈ 中文正常语速 4 字/秒**为合理区间，全项目统一用 ≤4）；念不完就加时长；**正常语速 > 15s 就拆成两镜**（不怕超时·每集总时长可放宽）。`ai_videos__时长节奏` 按 ≤4 裁决加时长/拆镜。**注**：多说话人交锋镜按各人台词**顺序总字数**算（同镜串讲）。
> 7b. **每集总时长＝软边界（follow-up 2026-06-23）**：分集/项目里写的单集时长区间（如 wushen「90–120s」、或 180–195s）是**软目标、非硬约束**——**超出可接受**（剧情需要就让它长），**或把溢出的剧情/镜头移到下一集**（哪种好用哪种）。**绝不为凑进区间而硬塞注水或硬删该有的戏**；格式契约只机械卡**单镜** 3–15s（K11），不卡每集总时长。剧本/大纲里的「（落 X–Ys 区间）」一律按软目标读。
> 8. **生成/改 prompt 后默认自检·不待用户提醒（2026-06-21 用户实测：worker 重铺后超 2000 字 / 文言「许」/ 站位朝向「右后」反向 等连环漏，皆因生成后只跑了格式契约、没跑审美审查）**：**任何时候新建或修改生成 prompt（shot / 场景 / 角色 ref / prop / 字卡），完成即默认自跑两层、过了再 declare done**：① **机械层 `ai_videos__格式契约`**——尤其 K10「```text 块 ≤2000 字硬顶」（shot 不计 `情节:`）+ K15/K22/K25 等；② **审美层 `ai_videos__审查总编排`**（串起 台词大师/白话大师/站位朝向/运镜/动作表演/时长节奏/剧情连贯/全剧序列）——**fan-out worker 的产出尤其必跑这一层**（worker 只保证生成、不保证台词白话/朝向/节奏；用户反复抓到的"skill 没 capture"其实是 skill 没被运行）。**至少跑：白话大师(扫文言/书面词如 许/怕是/方才/岂)、站位朝向(对谁说=视线落点·朝向写死·跑动方向不反·配角别放"右后"藏身后)、时长节奏(字数÷时长≤3.5)。** 不依赖用户逐次提醒。
>
...
**⚠ 2026-06-21 amendment — 跨镜首帧承接（continuity-frame handoff）：强关联相邻镜，下一镜首帧＝上一镜成片末帧.** Per follow-up「两个 shot 之间若需强关联，要加收尾帧做指导——其实第二个 shot 的首帧就够；但不是所有 shot 都要（本就切镜头的不需要），缺了衔接会很不自然」。**本契约面向"prompt 怎么写"的展开版 + 正反例 → `agent_refs/project/ai_video_shouweizhen.md`（首尾帧 reference）。** 机制：

> **(A) 何为承接 vs 硬切（默认硬切，只在视觉连续时承接）。** 相邻两镜 A→B 满足**全部**条件才做承接：① 同场景 / 同 bg plate；② 机位连续或渐变（同轴微推 / 拉 / 移、match cut），非越轴 / 正反打 / 景别跳切；③ 动作或情绪不间断（B 从 A 末帧那一刻"长"出来）。任一不满足＝硬切。**硬切（默认）** 不挂承接帧——换场 / 时间跳 / 回忆进出 / 越轴 / 正反打 / 景别跳切 / 虚化转场一律硬切，强行链首帧会与"切镜头"打架。

> **(B) 帧源＝截取上一镜实际成片末帧，禁另生成静帧。** 承接帧一律由用户截取 A 成片的**最后一帧**、作 B 的**首帧**上传到生成模型的「首帧」槽位（image-to-video 首帧图）。**不另画一张"收尾 / 衔接静帧"**——另生成的静帧与 A 实际渲出帧必然漂移、接缝照跳。这与上文「动作最后一拍 frozen 状态＝该 shot lastframe」一脉相承：A 的 `动作:` 末拍写清定格姿态＝交接帧的画面，B 的 `动作:` 首拍从同一姿态续起。

> **(C) shot 文件落点（承接镜才加；硬切镜不写承接字段）。**
> 1. `## Shot context` 增 `衔接:` 行：值＝`承接 shot{NN} 末帧（首帧＝上一镜末帧）` 或 `硬切（独立首帧）`。**每镜必写其一**（每集首镜＝`硬切（独立首帧）`，无上镜可承）。
> 2. 承接镜 `## 视频 prompt` 的 `参考:` 行**最前**加 `本镜首帧=>` handle（缩短形，标明它进模型「首帧」槽、非普通参考图槽；并在 `参考:` 下方加结构化首末帧指令——见 (H)）。
> 3. 承接镜 `Reference uploads` 加一项：`上一镜末帧（从 shot{NN} 成片截取最后一帧、作本镜首帧上传）`。
> 4. 承接镜 `动作:` 首拍 + `走位:` 起始姿态 / 朝向**必须与上一镜末拍 byte-coherent**（位置 / 朝向 / 表情续接、不跳变）——这是承接帧能"长"出来的前提，由相邻镜连贯 check 兜底。
> 5. **承接镜禁"重演已完成的过渡动作"（首拍动作 + 台词皆然·follow-up 2026-06-23）。** 承接镜首帧＝上一镜末帧、那个过渡动作（闭眼 / 睁眼 / 坐下 / 起身 / 转身 / 抬头 / 抽剑 / 推门…）在上一镜**已经做完并定格在交接帧上**。故本镜：① `动作:` 首拍只能写**从该已到达状态续起**的下一拍（如首帧已阖目 → 写"阖目续运功"，**禁再写"闭上眼"**；首帧已坐定 → 写"坐姿运转"，禁再写"坐下"），并在首拍括注「首帧已是X态·不重演X动作」给模型守卫；② `台词:`（尤其 OS / 旁白）**不得复述刚做完的过渡动作**——典型反例 EP3 shot3 首帧已阖目、台词却以"闭上眼…"开头，首尾帧一接就像"闭眼再蹦出来闭一次"。台词从交接帧的状态往下说（"把心神沉进丹田…"而非"闭上眼，把心神…"）。判定归口 `ai_videos__运镜` M8（连同首拍 byte-coherence 一起查）、台词层由 `ai_videos__台词大师` 协同；属审美/语义层，`格式契约` 不强校。

> **(D) 判定归口 `ai_videos__运镜`（跨镜层），机械校验归 `ai_videos__格式契约`。** 运镜大师按 (A) 逐对判承接 / 硬切、写死 `衔接:` 字段（M8）；格式契约校验 `衔接:` 字段在场 + 值=承接时三处落点齐全（参考 handle + Reference uploads 行）+ 值=硬切时不得挂承接帧 handle（K26）。`ai_videos__全流程编排` 阶段 5（运镜判定）与阶段 6（落 prompt 三处）默认执行本机制。

> **(E) 范围＝流程级 + 现在回填。** 所有 ai_video 项目默认走本机制；wushen_juexing 现有 ep01–ep04 全部相邻镜本次按 (A) 判定、回填 `衔接:` 字段 + 承接镜补三处落点。新项目自阶段 5 起默认逐对判承接。

> **(G) 尾帧锁定（handoff-source 侧 · 避免重生连环）。** 若某镜 A 的末帧被下一镜 B（`衔接: 承接 shotA`）用作首帧，则 A 是**交接源**。A 落两处：① `## Shot context` 加 `尾帧锁定:` 行；② **`## 视频 prompt` 的 `参考:` 行末尾加 `本镜末帧=>` handle**（缩短形、与承接镜首端 `本镜首帧=>` 对称、进模型「尾帧/end-frame」槽；首次生成无图可省、handle 在场作提示；并在 `参考:` 下方加结构化首末帧/末帧指令——见 (H)）。语义：**重生成 A 时**须把 A 已保存的末帧图 `shotAA_lastframe.png`（⏮ 生成末帧产出）作**尾帧**上传锁定 A 末帧不变（**首次生成无需**、自由出）。这样重生 A 不改其末帧 → B 的首帧参考仍有效 → **不必连环重生 B、C…**（解决「改一镜要整条链重生」痛点）。A 若本身也是承接镜（首帧承上），重生时**首帧 + 尾帧双锁**＝两端都不动。模型须支持尾帧（Kling 首尾帧；Seedance 视版本——支持则锁、不支持则保留 A 原 take 不重生）。交接源判定 = 其后继是承接镜，随 (A)/(E) 回填；机械校验 K26-④、判定归口 `运镜` M8。

> **(G2) ⏮ 生成末帧一键即承接（webapp，自动 · follow-up 2026-06-21）。** webapp 的「⏮ 生成末帧」（`POST /api/extract-last-frame`）除把本镜末帧写 `shot{NN}/shot{NN}_lastframe.png` 外，**同时把同一张图复制进下一镜文件夹、命名 `shot{NN+1}_firstframe.png`**——承接交接一键完成、无需手动拷。下一镜按数字相邻匹配（shot09→shot10、忽略零填充差异）；无下一镜（本集末镜）则只写 lastframe、不复制。承接镜出片时直接拿 `shot{NN+1}_firstframe.png` 传「首帧」槽即可。`FrameExtractor.extract_last_frame` 返回体增 `first_frame` 字段（无下一镜为 null）。

> **(F) 出片端接缝抹平（webapp，自动）。** 承接帧虽像素承接，生成器仍常把首帧静止保持一拍 → 整集拼接时承接处出现 ~0.2s 重复定格帧卡顿。`ai_video_management` webapp 的整集 concat（`POST /api/concat-episode`，原片/中文/EN/中英）**默认按各 shot `衔接:` 字段自动抹平**：承接镜用 freezedetect 检测并裁掉 incoming 镜头头部的静止重复帧（裁到 motion 恢复点、capped 1s、fail-open），硬切镜不动。用户无需手动处理；裁帧使承接镜略短（音轨用户后期自配）。详见该项目 README「合成本集视频 · 承接接缝自动抹平」+ follow-up 133。

> **(F2) 承接接缝顿挫的根因＝首尾帧缓动，抹平要超出"裁重复帧"（follow-up 2026-06-22「首尾帧生成的两段 concat 后仍有明显切换感」）。** 接缝"切换感"常被误判为重复帧；真正根因是图生视频**向两个关键帧缓动收敛**——A 结尾**减速、贴尾帧近乎定格**，B 又从同一首帧**自静止重新加速**，于是每个接缝＝`减速→定格→加速` 的顿挫/慢镜回弹（叠加 A 末帧＝B 首帧的一帧重复，进一步放大）。(F) 的 freezedetect 只裁了 incoming 镜头**头部**的静止重复帧——治标不治本。完整抹平工具箱（按性价比）：① **两端都裁死运动**——除 (F) 裁 B 头部外，亦裁掉 A 尾部那几帧减速到定格的"死运动"（~2–3 帧 @30fps），再拼；② **跨接缝补帧**（RIFE / FlowFrames）——在 A 末段 + B 头段之间重插光流运动，把"定格→重启"补成连续，**唯一真正修复运动断裂**的手段；③ **色彩/曝光对齐**——两次生成常有色温/亮度漂移、亦被读成"切换"，拼前对 B 做向 A 的 shot match；④ **短交叠转场**（xfade 0.15–0.25s）遮掩速度突变（治标）。`ai_video_management` webapp concat 默认已做 (F)＋本条①；②③需用户在剪辑端补。**设计端根治见 M8 承接 seam 落点原则**：把承接点设计在一个本就该停顿的瞬间（动作收势 / 亮相定格 / 反应 beat），缓动定格就成了"故意顿点"＝节奏而非 bug。**注意边界**：越轴 / 正反打 / 景别跳切 / 换场本就该**硬切**，硬切观众不觉突兀，别为消顿挫强行做无缝。判定/设计归口 `ai_videos__运镜` M8，出片端抹平归 webapp concat。

> **(F3) 设计端两个 prompt 级接缝卡顿根因（即便 seam 落在停顿瞬间也要消·follow-up 2026-06-23「ep1 shot11/12 首尾帧链接明显卡顿、非后期能修」）。** 当 A 末帧与 B 首帧已 byte 同帧（lastframe＝firstframe），接缝仍卡，根因常是这两条 prompt 写法（与画面无关、纯运动 meta）：① **景别/机位标签在接缝处突变**——B `镜头:` 起幅景别标签（如写「中景起」）与 A 实际末帧景别（如「中近景」）不一致，模型遵标签在**第 1 帧重新缩放/重新定位**＝瞬间 pop。修：B 起幅景别/机位标签**严格写成与 A 末帧/B 首帧同景别同机位**、明禁「起始重新缩放/重新定位」。② **双重停顿/重起势 + "冻成截图"两头坑**——A 末段已减速定格，B `动作:` 首拍又写「稍顿/再一顿」叠上去＝`定格→再顿→重新加速` 顿挫；但**反过来把 A 末2秒写成"完全静止 dead-still、零残留微动"会让画面像卡住/截图**（follow-up 2026-06-24「ep1 shot11 末2秒别完全不动、要人动或镜头动一下」）。要归零的是**方向性运动的突停**——行走/推拉/摇移/转身这类有速度矢量的动作须在 A 末段前收止、B 首拍不重新一顿不重新起势——**而非冻掉一切**。正解：A 末段**锁住站位/朝向/姿态 + 机位/景别构图不变**（无方向性位移、不变焦），同时**保留环境级微动**——门外风掀发梢/衣摆下缘、肩背随极缓呼吸轻起伏——让末帧"活着但没移动"；环境微动无方向性速度、不构成"运动突停"、不引入卡顿，且末帧构图仍＝B 首帧（站位/朝向/景别一致、可 byte 承接）。B 首拍仍自该构图 ease-in 顺滑承接。即原「A 末帧 dead-still ↔ B 首帧静止顺起」修正为「**A 末帧锁构图+留环境微动 ↔ B 首帧同构图 ease-in**」。归口 `ai_videos__运镜` M8（承接对默认按本条写 A `动作:`末拍+`镜头:`末段锁死 与 B `动作:`首拍+`镜头:`起幅同景别 ease-in），与 (F2) 出片端抹平互补、本条为设计端根治。

> **(H) 结构化首末帧指令字段 + prompt body 禁写跨镜/工作流 meta（follow-up 2026-06-21）。** `参考:` 行的 `本镜首帧=>` / `本镜末帧=>` 只是**上传 handle 的缩短形**——`(上一镜末帧)` / `(重生时上传)` 这类括注是给人看的、对生成模型无用且占字数，一律不写。在 `参考:` 行**下方另起一行**加**结构化首末帧指令**告诉模型画面起止点：① 首+末双帧镜 → `首末帧: 画面以首帧为起始、以末帧为结束、自首帧顺接渐变至末帧`；② 仅本镜首帧 → `首帧: 画面以首帧为起始、自首帧自然顺接展开`；③ 仅尾帧锁定 → `末帧: 画面以末帧为结束、收束至末帧`。硬切镜（无承接/锁定）不加此字段。
> **铁律——shot 视频 prompt body 内禁写任何跨镜/工作流 meta**：`承 S\d` / `接 S\d` / `接 shotN` / `顺承接 shotN` / `同场景跨镜一致` / `同场景接 shotN 光线一致` / `供下镜承接` / `承 SN 同一人` / `diegetic…` / `(重生时上传)` / `【写死·防对空气/背对/重复】` / `【全程绝对无字幕…】` / 日期·follow-up 编号 等一律删。理由：模型只生成**单镜**、读不懂 "shot5/S8" 的跨镜引用，这些纯是给人看的编辑/审计注释、占字数；正向提「字幕」类提醒还会**反噬**（增加烧字幕概率，无字幕只靠负向 + 渲染样式末「画面不烧字幕」即可）。承接连贯由 (C)④ 的 byte-coherent 落点 + 相邻镜 check 兜底、不靠 prompt 文字注释。落 prompt 归 `运镜` M8 / 阶段 6；机械校验归 K26。

> **(H2) 承接镜=尾锁源时，**首次生成走「首帧单图」、双帧指令仅留给重生（follow-up 2026-06-27「ep1 shot10末帧作 shot11 首帧、生成总差 50+ 帧」）。** 一个镜既是**承接镜**（首帧＝上一镜末帧）又是**尾锁源**（末帧供下一镜承接）时，它在**首次生成**那一刻**本镜末帧根本还不存在**。此时若按 (H)① 写**首+末双帧**指令（`首末帧: …自首帧顺接渐变至末帧` + 参考分配 `@图N为尾帧`），等于命令模型"渐变到一张不存在的末帧"→ 模型不再把首帧当硬性第 0 帧、转而自由收敛 → **生成起幅相对所给首帧漂移数十帧**（实测 50+）。正解：**首次生成只走 (H)② 首帧单图**——结构化指令写 `首帧: 画面以首帧为起始、自首帧那一刻续起自然顺接展开、起始不重新缩放/不重新定位/不重摆姿`；`参考分配` 明写"首次只上传 @图1首帧+@图角色+@图场景、**不上传尾帧**，@图N尾帧仅重生本镜锁末帧时上传"；`参考:` 行末 `本镜末帧=>` handle 仍可在场（(G)「首次无图可省、handle 在场作提示」）。**首+尾双帧指令（H①）只在重生本镜**（此时 `shot{NN}_lastframe.png` 已存在、作尾帧上传锁定末帧）时才启用。② **配套两条铁律**：(a) 首帧务必传进生成器的**专用「首帧」槽**（image-to-video 首帧图），**不是**全能参考/普通参考图槽——进全能参考槽＝模型当"引导图"而非第 0 帧、照样漂；(b) 承接 seam 落点**禁落"走路/转身/出招中途"**（mid-stride / mid-turn）——i2v 接续半步/半转姿态会**重置步态/动作相位**致跳帧，应把交接帧设计在**收势/立定/亮相定格/反应 beat**（承 (F2)：本就该停顿的瞬间）；故**交接源镜的末拍应收到一个静定 beat**（如"转身完成、面南立定收势一拍"），而非停在仍在位移的中途帧。归口 `ai_videos__运镜` M8（承接对默认按本条写"首次首帧单图 + seam 落静定 beat"），机械校验由 `格式契约` K27 兜「参考分配与 参考 handle 一致」。
...
> 把这个人当一个**活生生的人**来写透，不是平铺叙事的功能位。每行一句，写不出的标 `待深化`、随集补，**严禁留空当成不存在**。这一段是后续所有剧情 / 台词 / 创新的创作前提（rule 12.11 创作前必读）。
- **年龄 / 当下处境**：实岁 + 此刻身处的局面。
- **前史（之前在干什么）**：登场前的人生轨迹 / 身份 / 营生。
- **为什么在做现在做的事**：行动的根因（不是表面理由，是底层驱动）。
- **目标（想要什么）**：短期目标 + 长期目标 / 终极追求。
- **需求（needs）**：他真正缺、真正需要的（常与目标错位）。
- **内心欲望（desire）**：藏在心底、未必说出口的渴望。
- **核心矛盾（contradiction）**：身上拧着的两股劲（想要 X 又怕 X / 嘴上 A 心里 B）。
- **恐惧（fear）**：最怕失去 / 最怕面对的东西。
- **最大创伤（wound）+ 致伤事件**：人生最深的伤口，由哪一件具体的事造成。
- **此刻怎么 make decision**：在当前这段情节里，凭什么逻辑做选择、怎么行为处事（随情节推进逐集更新）。
- **性格特点**：3–5 个关键词 + 一句行为体现。
- **成长 / 变化轨迹**：他正在怎样变（这是 ongoing 过程，每集回看是否还成立、有没有该推进的转变）。
...
**⚠ 多音字读音锁定（默认字段·防 TTS 念错·per follow-up wushen_juexing/ep5 shot10 实测 — 2026-06-28）**：实测 TTS 对台词里的**多音字 / 专名生僻字会随机念错某一两个字**（如 天行宗的「行」念成 háng、得心应手的「应/得」、一脉的「脉」念 mò、宿主的「宿」念 xiǔ）。**铁律**：凡 `台词:` / `## 台词配音 prompt` 含多音字、易错专名、生僻字的发声单元，配音块在 `类型:` 后**加一行 `读音锁定(多音字·防念错): {字}={拼音}(语境)、…`**，逐个把易错字钉死正确读音（含「叮/嗯」等拟声的咬字提示）。系统/合成锁定音色镜同样标。判定归口 `ai_videos__台词大师`（审台词时顺手标出易错字读音）+ `ai_videos__格式契约`（机械层扫含多音字的配音块是否带 `读音锁定:` 行）。
...
> 7. `---` → `# 步骤二 · 场景 walk-through video prompt — {场景名}（14s 环视，逐秒方位·运镜连贯平稳·大小场景一律一致）` (逐秒方位时间轴 `>` note 5段 + `参考:` + `画幅:` + `## Prompt` + ```text``` block, 时长 14s·见 rule 12.10 2026-06-27 amendment)
...
>
> **⚠ 2026-06-21 amendment — folder-per-plate「当场建·禁待建」(反复复发 bug 根治).** 实测教训（wushen_juexing EP4 `镇主街`/`镇演武场`：只写父卡 + index 表、per-plate folder 全标「待建（stage5/6 渲染前）」没建，shot 却已 `参考: 镇主街_bg1_城门=>` 引用不存在的 plate；此「scene 缺 bg 子文件夹」已非首次）。根因：① 建卡时把 item 9 的 folder-per-plate 写成「待建/以后再说」→ 永远没人补；② stage5/6 worker 交付清单只含 shotNN.md、不含 plate；③ 无 QC 扫 plate 存在性。**铁律**：(a) **建场景卡的同一步就必须创建全部 `{plate}/{plate}.md`**（image→image 图生图 prompt：首行=plate 路由 handle + `参考:` 全局底图 + 主体/视角/光线/风格/负向/比例、纯背景无人物、复用场景锁定描述符 + 全剧渲染串）；**禁用「待建（stage5/6 渲染前）」字样推迟 plate**（PNG 渲染可推迟、prompt .md 不可）。(b) 任一 shot 引用 `{场景}_{plate}=>` handle 前，对应 plate folder/.md 必须已存在。(c) **QC：`ai_videos__格式契约` K23** 校验「index 行 ↔ `{plate}/{plate}.md` ↔ shot `参考:` handle」三方一致，缺即 blocker、当场补建。 (d) **plate 命名 = `bg{N}_{方位}_{描述}` 三段式、方位段取短且与全 drama 现有方位 token 互斥**（DownloadsImporter 用方位 token 子串匹配下载名路由；方位是长 blob 或与别场景方位互为子串→多命中→导入失败/误投。实测：`bg3_街角茶棚` 的方位"街角茶棚"⊃ EP2"街角"→撞车 not_matched）。建卡前先列全 drama 已有方位 token 避让；QC 由 **K23b** 校验命名三段式 + 方位跨 drama 唯一。 (e) **改名兼容已渲图**：若 plate 已据旧名渲过图（用户 Downloads 里已有按旧 handle 命名的 PNG），改名时新方位 token 应**仍是旧名的子串**（如 `bg2_全景旗台`→`bg2_旗台_全景`、方位"旗台"⊂旧名，已下载文件仍路由命中）；若新方位不在旧名内（如"人群"→"人潮"）则已下载文件 not_matched、须重渲或把方位改回旧名子串。实测 wushen EP4：8 plate 改名后 7 个新方位⊂旧名仍可导入、唯"人潮"破例失配、改回"人群"后 5/5 通过。

> **⚠ 2026-07-03 amendment — 场景立绘/背景板 = 固定空间结构，且要合现实（rexue 学校泳池 bg1 用户实测）.** 步骤一底图与各 plate 是**纯空间背景板**，只画该地点**长期固定的空间结构与建筑元素**；两条铁律：(a) **后期剧情小道具不入立绘/背景板**——只在某镜出现、随剧情带上的道具（如泳池的饮料瓶/信物吊牌）归 **shot 层**（shot `参考:`/`情节:`/`动作:`），**禁烘焙进场景锁定描述符 field 4 或步骤一/plate 主体**（否则背景板张张带着该道具、跨镜穿帮）；场景卡只锁真正固定的装置（出发台/救生圈/记录牌/护栏/机床等）。(b) **场景内物件几何摆放须合现实**——按真实世界该物件的正确位置写死，别让模型乱摆：泳池**出发台对准每条泳道中心线（居相邻两条分隔线正中、不压在分隔线上）**、非贴墙非骑线；类推：门在墙上开口处、楼梯接楼层、灯具在合理位置。**(b-补) 定位反复失败先查「参考照片里有没有会被模型误读的相似元素」，逐一点名区分（rexue bg1 用户实测）**——bg1 出发台屡被放到红色浮漂上，根因＝真实泳池参考照片里**池底还有黑色泳道中心导航线**、经水折射恰落在泳道中心，即梦把**池底黑线**误当分隔线、于是把出发台"居中"放到了红浮漂上。修法三件套：① 抽象几何+空间否定句（"对准中心线/不压在分隔线上"）对图像模型不可靠，改**正向具体物理描述**；② **给模型一个它必画且能对齐的锚点**——用"出发台正对**池底黑色中心导航线**的正上方"顺着模型注意力锚定、而非跟它的错误感知硬顶；③ **把易混元素逐一点名**（"池底黑线＝中心导航线·非分隔线；水面彩色浮漂绳＝真正分隔线；台立岸上·绳漂水面"）。凡"prompt 改多次仍复现同一定位错误"，先怀疑参考图有干扰元素，别只反复改措辞。**(b-补·升级) 消歧仍不灵 → 干脆删掉所有会混淆的竞争参照物、只留那个唯一锚点。** rexue bg1 即使逐一点名"黑线≠分隔线、浮漂＝分隔线"，Seedance 仍把跳台放到红浮漂上；最终奏效的是**位置说明里彻底不提浮漂/分割线，只说"出发台骑在池底黑色导航线正上方"**——模型对多种相似线一律犯迷糊，与其教它区分，不如把定位收敛到它一定画对的单一物上，其余同类元素作纯背景、不参与定位。铁律：**几何定位锚点越少越好、能一物一锚就别提第二种同类物。** **(b-补·再升级：先建"这是什么+它和周围表面的关系"，再谈位置。)** rexue bg1 再次失败——模型把本该在**水下池底**的黑线画到了**岸上瓷砖地面**、跳台仍对着红浮漂；根因＝模型**不认识"出发台/starting block"这个物件、不懂它的功能与它同"水/岸"的关系**，于是锚点线落到了错误的表面上。**修法：定位前先给物件建立 identity + function + 它贴着哪种表面**——"出发台＝齐腰高金属底座+带防滑纹斜踏板、选手站上去向前跳入水中的矮台、**紧贴泳池水边、正前方就是池水**"，再把锚点线**明确绑到正确表面**（"黑线画在**水面之下的池底**、自台底正前方伸向远端；**只在水中池底、绝不在岸上瓷砖地面**"）。教训：**当模型把定位锚点放到错误的表面/位置时，别再调锚点措辞——它其实不认识那个主体物件；先用形态+功能+贴附表面把物件讲成模型有正确视觉先验的东西（必要时给英文通名如 starting block），锚点自然归位。** **(b-补·终局：物件先验太弱就别硬生成、改用干净参考照。)** rexue bg1 连"identity+function+贴附表面"都试过仍不行，最终结论＝该模型对"泳池出发台"的视觉先验就是不够，**再怎么改文字都白费**。收尾策略：**换一张结构清晰的真实参考照作 image→image 底，结构/构图交给参考照带，prompt 退回只描述场景氛围 + 定情绪光（"合理的出发台+合理打光"）、不再堆几何对抗话术。** 通用判据：**同一定位问题改 prompt ≥3 次仍复现 → 停手，判定为"模型缺该物件先验"，转用参考照兜底；prompt 只留 mood/光/色/风格，几何让参考图说话。** 别把工时耗在跟模型的错误先验硬顶上。(c) **打光要 motivated（合场景现实光源）**——主光锁到场景里真实存在的光源并写清方向，禁"无对应灯具的硬顶聚光/舞台光打在人身上"（夜间泳池＝水下灯自下往上为主光＋水波焦散，非顶光；详见 `ai_videos__光线色调` L6）。同场景所有 plate 共用同一空间结构 + 主光源逻辑。

> **⚠ 2026-06-21 amendment — 角色「≥2 镜复用就立档」(对称场景立档门槛·follow-up 034 用户实测).** 场景有「≥2 shots 复用 → 立档 + ref」门槛，**角色同理却被漏掉**：实测 EP4 `测资执事`（6 镜 S8–S14·全段焦点说话角色）、`菜鸡少年`（2 镜）、`围观武者甲/乙`（2 镜）被默认成「群像内联·不锁定」，**无角色卡、无 ref 图 → 跨镜面孔必漂**（每镜生成不同脸）。根因：建卡门槛只对"主角/重要角色"用，episode-local 多镜复用的功能配角被一律内联。**铁律**：(a) **任一角色（含 episode-local 功能配角/群像）出现 ≥ 2 个 shot → 必须建角色卡**（`characters/cN_{名}/cN_{名}.md`·参 c6/c7/c9–c12 轻量档：8 字段锁定描述符 + voice_id + Seedream ref 立绘 prompt），**ref 图锁面孔防漂**；出现它的 shot `参考:` 行带 `cN_{名}=>` + re-paste 识别标签（byte-identical）。(b) **只出现 1 镜**的纯一次性龙套（说书先生/路人/单次执事等）可内联不锁定。(c) **角色卡 ≠ 出场字卡**：角色卡（锁定+ref·跨镜一致）对 ≥2 镜配角必建；出场字卡（顶角名牌）仍只重要角色发（style_guide §7）。(d) **QC：`ai_videos__格式契约` K25** 扫"某角色名在 ≥2 shot 的台词配音/角色行出现，却无 `characters/cN_{名}/` 卡或 shot `参考:` 漏 `cN=>`"= blocker、当场补建+接线。
...
**出场角色 checklist 派生规则**（per follow-up 006；**turntable 条件化已废止·2026-06-27 amendment**）：

| 角色出场方式 | turntable（7s 建立视频）是否必需 | 备注示例 |
|---|---|---|
| **任何命名人形角色**（正脸 / 全景 / 背影 / 远景 / 剪影 / 群像里的具名个体——不分主配、不分出场方式） | ✅ **必需**（统一建立模式·2026-06-27）：每个命名人形角色先出 7s 建立视频作 reference | 「主体角色正脸」「配角全景」「背影 / 远景出场」一律必需 |
| 非人形实体（系统 / UI 等） | ❌ 无须（用 UI 浮现块） | 「系统」例外 |
| 物件 / 法宝（角色专属道具·无人形） | ❌ 无须 | 「仅道具出现，无人形」 |
...
- **⚠ 2026-06-22 amendment — Seedance 2.0 全能参考（Omni Reference）@标签 + 首尾帧角色分配契约（research-backed·指导 shot prompt 写法）.** 来源：用户 research（Seedance 2.0「全能参考 + 首尾帧」机制，2026-06-22；sources: PiAPI seedance-2-0 / WaveSpeed complete-guide / seedance-2ai.org first-last-frame / 即梦全能参考完全指南CSDN / GitHub Emily2040 seedance-2.0 pipeline）。**核心认知：Seedance 2.0 的「首尾帧」不是与全能参考并列的独立模式，而是「全能参考」里的两个角色——把首帧图 / 尾帧图当作 @ 参考素材上传，再在 prompt 正文用 @标签给每个素材派角色（首帧 / 尾帧 / 角色锁 / 场景锁 / 运镜参考 / 背景音）。** 故凡有 `本镜首帧=>` / `本镜末帧=>` 的镜，写 prompt 时须把它们落成全能参考的 @ 首尾帧分配句，而非只留占位。落地规则：
  ...
  2. **`参考:` 行每项 inline `=>@N` 槽位号（2026-06-28 用户改·取代旧独立 `参考分配:` 行）**：`参考:` 行每个上传项在其 `=>` 之后紧跟全能参考槽位号 `@N`（按出现顺序 **1-based**、byte-identical），例：`参考: \`c19_萧若云=>@1, c12_围观武者乙=>@2, c12_围观武者乙声音=>@3, 镇演武场_bg1_场口_入场=>@4\``。**槽位语义由「项名自身」承载**——`本镜首帧=>@k`＝@k 是首帧、`本镜末帧=>@k`＝@k 是尾帧、`{角色}=>@k`＝@k 锁该角色、`{bg}=>@k`＝@k 场景、`{角色}声音=>@k`＝@k 该角色音色源（视频→图槽、声音→对应音频槽，编号仍按 `参考:` 出现顺序连号）。**故旧 `参考分配:` 独立行已废除、不再写**（@图N→角色/首尾帧/场景 的映射已 inline 进 `参考:`，不必再复述一行）。`@N` 须 byte-identical（不写"第一张图"）、与槽位上限（≤9 图 / ≤3 视频 / ≤3 音频 / ≤12 文件）一致。
  3. **prompt 只描述「首→尾之间的运动 / 过渡」、不重新设计主体**：点名一个贯穿两帧的「不变锚点（carrier）」（同一角色身份 / 服装 / 场景 / 光线），让模型只在两帧间生成运动；禁在过渡描述里改造主体外观（否则违藏锋 / 锁定描述符）。
  4. **首尾帧须有显著差异（＝分镜设计铁律）**：首帧 ↔ 尾帧要在**位置 / 姿态 / 朝向 / 景别 / 表情 / 物体状态**上有明显变化，Seedance 才 capture 得到运动；场景 / 光线保持一致；大变化铺在镜时长里自然、挤进过短时长会赶。**整镜静态背影凝立 = Seedance 出不来运动**（EP1 shot11 实测教训：背影立门前凝立立誓→首尾帧几乎一致、capture 不到运动，遂重设计为「厅中背身→门前驻足」的大位移）。设计每个 shot 时即按此把关。
  5. **全能参考槽位上限**：≤ 9 图 + ≤ 3 视频（总时长 ≤ 15s）+ ≤ 3 音频（总 ≤ 15s）、混合**总计 ≤ 12 个文件**。超限须拆镜或减参考。
  6. **首帧单图 vs 首+尾双帧**：只需一张起始图的镜走「首帧」单图（`@图1为首帧`，比例跟随该图）；有承接帧 + 末帧锁定的镜走「首 + 尾」双帧（`@图1为首帧、@图N为尾帧`）。
  机械校验归 `ai_videos__格式契约` K27：`参考:` 每项须带 inline `=>@N`（按出现顺序 1-based、byte-identical、不超槽位上限）；**不再写独立 `参考分配:` 行**（2026-06-28 用户废·@N 已 inline 进 `参考:`，承接镜的首/尾帧由项名 `本镜首帧=>@N`/`本镜末帧=>@N` 自明）。
...
- **⚠ 凡发声角色必先建卡（2026-06-28 用户·all-speakers-carded gate）**：**任何在 shot 里有「台词」或「内心独白 OS」的角色——含背景 / 一次性的招募执事、报场路人、群像里开口的某人——都必须先在 `2_世界观人设/characters/cN_{中文名}/cN_{中文名}.md` 建卡**（至少：锁定描述符 #识别标签 + voice_id + ref 立绘 prompt），并在 `casting.md` 登记 voice_id，**才能在 shot 的 `角色:` / `参考:` / `## 台词配音` 里引用**。**禁止 inline「背景一次性·无卡」说话人**（旧做法作废）。纯无台词群像（围观群众 / 甲士 / 让道百姓等不开口者）仍可不建卡、只在 `场景:` / `动作:` inline 群像描述。**落地 gate（stage5/6 出 shot 前）**：先扫本集所有 `台词:` / OS 发声单元 → 每个发声角色确保有卡（缺则**先建卡 + casting 登记**再出分镜）→ 再生成 shotNN.md。机检归 `ai_videos__格式契约` K30。
...
- **「系统流」金手指 = 在画对话框（per follow-up wushen_juexing/025 — 2026-06-14）**：系统流 / 金手指类剧的「系统」一切交互（绑定 / 提示 / 选择 / 发奖）应表现为**直接出现在画面中的悬浮 UI 对话框 / 面板**（网文系统流游戏既视感）——半透明描边圆角框、框内字（系统流多用鎏金描边）、选项分行可带【】按钮高亮，主角与观众都看得见框内字；系统文字是**在画对话框内嵌字 + 提示音**（非纯画外音 / 后期软字幕）。**发奖要「送大礼」仪式感**：弹「恭喜宿主获得：X」贺词框 + 礼花光效 + 光门 / 礼匣自框炸开把奖励加身——而非平淡兑现。`情节:`/`走位:`/`台词:`/`光线:` 各字段都要点明对话框的位置与框内字，让视频模型把它当画面元素渲染。
  - **⚠ 框内字默认符号化 / 省略号、不渲染大段可读汉字（per follow-up wushen_juexing/ep5 shot10 实测 — 2026-06-28）**：实测视频模型对「框内大段可读汉字」**有一定概率渲成乱码 / 错字**（系统提示越长越爱乱）。**铁律**：系统 UI 框内**只保留极短符记**（≤2-3 字的栏目/宗名标签，或干脆全符号），长描述句一律抽象为**鎏金等线短划 + 省略号「……」状光点序列**，`动作:`/`情节:` 写「框内不渲染可读长句、以极简鎏金符号化呈现（…），两路/选项具体含义由系统配音念出」，`负面词:` 加「系统框内大段可读汉字或乱码错字」。框内**语义信息靠 `## 台词配音 prompt` 系统句承载、不靠画面字**——观众听系统音即懂，框只作氛围符号层。判定归口 `ai_videos__特效设计`（系统 UI 视觉层）+ `ai_videos__格式契约`（机械层扫框内是否仍写大段可读句）。
...
- **`台词:` 字段只放台词正文；非台词文本一律移出码块（2026-06-21 教训：EP3 shot9 把"（本镜无台词·默剧静默拍；环境音…）"写进 `台词:` 字段 → 被生成器当字幕烧进画面）**：凡坐在 `## 视频 prompt` 码块 `台词:` 槽里的文字都可能被模型渲染成屏上字。故 `台词:` 只允许「`· {说话人}〔类型·口型注〕：{台词正文}`」逐条，或**静默/无台词镜直接写 `台词: 无`**。**舞台提示 / 环境音说明 / 旁注**（"默剧静默拍""环境音：夜雨渐歇"等）**禁写进 `台词:` 或码块内任何位置**——移到码块外的 `## 台词配音 prompt` 注释或 `## Shot context`。静默镜同时省略 `## 台词配音 prompt` 的生成块、仅留码块外环境音注释。机械校验归 `ai_videos__格式契约` K21。
- **静默镜（`台词: 无`）还须加"无人声"负面词，`台词: 无` 单独挡不住（2026-06-21 追加教训：EP3 shot9 已写 `台词: 无`，成片仍念出"裴知秋"——生成器从 `角色:`/`情节:` 抓角色名 TTS 成画外音）**：静默镜 `负面词:` 必须含一组人声抑制词 `人声 / 配音 / 旁白 / 念白 / 念出角色名或台词文字 / 任何生成人声音轨 / no voiceover / no speech / no narration`。⚠ **人声抑制只写进 `负面词:`，禁写进 `台词:` 槽**——`台词:` 仍只写 `无`（写进去会违反 rule 705/K21、被当字幕烧进画面）。⚠ **只对静默镜加**——**有台词镜禁加人声负面词**：rule 704 已确认 Seedance 正是从 `台词:` 文本生成台词配音，给有台词镜加人声负面词会把台词音一起压哑。机械校验归 `ai_videos__格式契约` K22。
- **多声轨 / 系统音色镜 → 视频静音 + 全 mux（rule 704 的边界·2026-06-21 教训：EP3 shot7 同镜有 系统×2(SYS-gold-01) + 主角 OS(PR-hero-01)，靠 Seedance 单声轨 TTS → "语言彻底混乱"、且用不上上传的系统 MP4 音色）**：rule 704「Seedance 从 `台词:` 文本生成台词配音」**只适用单一主角声轨的镜**。当一镜满足以下任一——① 含 ≥2 个**不同 voice_id** 的发声单元（如 系统 + 主角、甲 + 乙 对话）；② 含**系统/合成锁定音色**（系统 voice 来自用户上传的系统 ref MP4 / 锁定 voice_id，Seedance 生不出）——则该镜**视频须静音**：`台词: 无` + 负面词加人声抑制组（同上），所有台词正文移到 `## 台词配音 prompt` 各自 voice_id 块，后期用 `tools/mux_av.py` 分轨 mux（系统句用系统 MP4 音色、主角句用其 voice_id）。⚠ 注意与「在画对白 lip-sync 镜」区分：若镜里有**在画口型对话**且只一个声轨，仍走 rule 704（保留 `台词:` 正文喂 Seedance）；多声轨即便全程不对口型也走本条 mux。机械校验归 `ai_videos__格式契约` K22b。
...
- 字数计 fenced ```text 内文（不含 ``` 标记 + 不含 Shot context / Reference placeholders 段 + 不含 Seam-frame still prompts 段）。**`情节:` 块计入** —— 它和其余字段一起被原样粘贴进模型、一起承受 Seedance/即梦 的 silent-truncation，所以全局 2000 硬顶按**整段 prompt body（含 `情节:`）**统计。
- **`情节:` 豁免已废止（2026-06-22，follow-up「所有 shot 的 prompt 字数 < 2000」）**：旧「`情节:` 字段不计入字数上限」一条作废（它绑定的是已废止的 soft 2000 / hard 2500 旧体系）。现在 `情节:` 与所有字段一并计入 ≤ 2000 hard，无任何豁免；情节过长时用经济语言压缩、删与其余字段重复的叙事，但不改情节走向。
...
**镜头取词契约**：景别 / 运动必须出自 `style_guide.md § 镜头语言关键词字典`（每个项目须维护至少景别 + 运动两张表）；`镜头:` 行格式 = `{景别} + {运动}（一句运动节奏描述）`。静帧 seam 行格式 = `{景别}` 单独。景别词须落在**五级阶梯**（远景/全景/中景/近景/特写）内，定义、戏剧功能与**切换铁律（绝不同机位同景别硬切）**见 `agent_refs/project/ai_video_jingbie.md`；项目 style_guide 景别表须与该五级骨架对齐。
...
> **编排入口（2026-06-17）**：本节的 D/S 准则已按单一职责拆进各专项审查 skill，**全 9 项均已建成**：台词→`ai_videos__台词大师`(D1–D6)；站位朝向→`ai_videos__站位朝向`(C1–C5)；运镜→`ai_videos__运镜`(M1–M7)；动作表演→`ai_videos__动作表演`(A1–A6, 含 P7/S1)；时长节奏合理性→`ai_videos__时长节奏`(PA1–PA6, 台词念得完/动作演得开/不赶不拖)；光线色调→`ai_videos__光线色调`(L1–L5)；整集剧情连贯→`ai_videos__剧情连贯`(P1–P6+N1–N4, 含 S2/S3/S5)；全剧序列→`ai_videos__全剧序列`(R1–R8, R8＝跨集连续事件状态/进度连续·后集禁把前集已开幕/已进行的大会·测资·宴·战当"正式开始"重启·须带「承接状态」头注·2026-06-28)；机械契约→`ai_videos__格式契约`(K1–K14)。由 **`ai_videos__审查总编排`** 按「单镜→整集→全剧」顺序统一调用。**每个 ep/shot 的任何改动 + 出片前，默认跑该 suite**（见下方 2026-06-17 amendment）。本节下表保留为各 skill 准则的总索引；权威定义在各 skill 的 SKILL.md。
...
#### 12.10 场景 reference 视频 prompt（14s walk-through 建模样片，per follow-up 017 + 2026-06-27 改 15s→14s）
...
- 1.0-3.5s 缓慢 dolly back + 平滑 yaw 约 180° {机位轨迹 ~30 字} 至反向位（匀速缓动·无骤停骤起）
- 3.5-4.5s 悬停 #2 反向广角（28mm）: {Reverse 详描 ~100-130 字 — 覆盖正面建场拍不到的反向 180° 空间 + 反侧主要元素}。t=4.0s
- 4.5-6.8s 缓慢 {上升至高位 OR 下沉至低位} 机位，俯仰角 {渐变描述}（匀速缓动）
- 6.8-7.8s 悬停 #3 {高位俯瞰 OR 低位仰望}（28mm）: {Vert 详描 ~100-130 字 — 垂直方向最强视觉信号}。t=7.3s
- 7.8-10.0s 缓慢 dolly 回中景轨道位，横移扫过 {本场景主要元素序列 from #3 + 关键区位 from #2 ~80-120 字}（匀速缓动）
- 10.0-11.0s 悬停 #4 中景定格（35mm）: {Mid 详描 ~100-130 字 — key zone 锁定 + 与周边元素同框关系}。t=10.5s
- 11.0-13.0s 缓慢 zoom 推进至 85mm 长焦位置，焦点对准 {detail 目标}（匀速缓动）
- 13.0-14.0s 悬停 #5 长焦特写（85mm）: {Detail 详描 ~100-130 字 — 标志道具 from #4 + 材质纹理 + 高光细节}。t=13.5s

光线/色调: {详描 ~200-300 字 — 主光 / 辅光 / 反光 + 各表面材质对光的反应 + 各自然色名在不同表面上的表现 + 时辰特有大气感}。配色主/辅/点缀自然色名严格锁定，14s 全程不漂。

节奏: 匀速平稳，5 个 canonical dwell 1.0s 给锐利静帧 + 4 段 transition monotonic 缓入缓出过渡、无顿挫；信息密度优先于速度。

负向: {场景专属负向 from scenes/s{N}/锁定描述符 #负向，4-8 项} /
不要 抖动 / 剪辑 / 跳切 / hard cut / 淡入淡出 / 加速或瞬反 / 顿挫骤停 / 镜头朝向跳跃 /
不要 时辰漂 / 配色偏 / 角色入画 / 起手非正面 / {场景专属构图禁忌 1-3 项} /
不要 任何音频 / 超过或短于 14s。
```
...
短剧好不好看的根，是**每个人物都是活生生的人**，不是推动情节的功能位、不是平铺叙事的旁白对象。本规则把「研究透每个人 + 理清人物间的关系网 + 创作时全程吃透这些」立为 ai_video 全流程的硬纪律。三件事：

**A. 人物灵魂（每个具名角色卡的 `## 人物灵魂` 段，rule 12.1 模板已含）。** 把人写透，至少覆盖 12 维：年龄 / 当下处境、前史（之前在干什么）、为什么在做现在做的事（根因）、目标、需求(needs)、内心欲望(desire)、核心矛盾、恐惧、最大创伤 + 致伤事件、此刻怎么 make decision、性格特点、成长/变化轨迹。**这是 ongoing 过程**——人是会成长变化的，每推进一集、每次回看都要补深、校正（哪一维"待深化"就标 `待深化` 随集补，**严禁留空当不存在**）。落点＝该角色卡 `## 人物灵魂`。

**B. 人物网（`2_世界观人设/relationships.md`，全剧一份）。** 当所有人物的灵魂都立起来后，建一张关系网：① **每对相关角色之间是什么关系**——社会关系（血缘 / 上下级 / 门派 / 阵营）、精神关系（师承 / 信念同盟或对立 / 镜像）、情感关系（爱 / 恨 / 愧 / 依赖 / 嫉妒）；② **每个人物与本剧当前主题的关系**——他在主题命题上站哪一边、印证还是反诘主题；③ 关系**随剧推进会变**，是 ongoing 的（标变化态，如「EP1 父子冷遇 → 边疆同台」）。relationships.md 与各角色卡 `## 人物灵魂` 双向一致。模板见 stage2 playbook §4。

**C. 创作前必读（cross-cutting 纪律·默认强制）。** 凡做**剧情脑暴 / 写或改大纲 / 写或改剧本台词 / 任何创新与改动**，都必须在**已读透相关角色 `## 人物灵魂` + `relationships.md`** 的前提下创作——据此判断「此情此景这个人该做什么选择、说什么话、事情怎么发展才顺」，而不是为了情节硬凑。创作后顺手回看：这次写的选择/台词是否还对得上人物灵魂与关系网？对不上就**要么改情节、要么补深人物灵魂**（人物在成长就更新轨迹），不留矛盾。这条纪律叠加在既有「叙事改动连贯性 check」「审查总编排」之上，是它们的**人物动机层**。

> 落地归口：人物灵魂 / 人物网的**生成与逐集深化**由 `ai_videos__stage2_世界观人设` + 大纲/剧本各 stage playbook 执行；机械层（段落是否存在、relationships.md 是否在、是否留空）由 `ai_videos__格式契约` 扫；动机可信与「选择对不对得上灵魂」由 `ai_videos__剧情连贯` / `ai_videos__台词大师` 审。
...
**13b episode 级 BGM 编排 + 烧录（per follow-up bgm_library/004 — 2026-06-20）。**
- **落点是文件夹**：novel 每集 `episodes/epNN/bgm/bgm.md`（独立 `bgm/` 文件夹 + md，非 episode 根 `bgm.md`；反查扫描两者皆兼容）。short 仍项目根 `bgm.md`。
- **稀疏铁律**：cue 时间线**不是每秒都有 BGM**，只在**强烈激情 / 武打 / 悲伤 / 系统转机 / 打脸爽点**等特定剧情段写一行；市井日常 / 纯过渡段**留白只走环境声**。每集典型 6–8 条 cue。
- **cue 由 pipeline 人工读剧本产出（不在 webapp 自动生成）**：时间窗按 shot 累计时长换算成**整集时间轴秒数**。行式扩展（slot 占位 + 期望情绪）：
  `起-止(秒)  bgm_NNNN｜-  | cat=情绪 | vol=0-1 | duck=on/off | fade=in/out｜in｜out｜none  # 剧情注释`
  slot=`-` 为待分配占位；`cat=` 是期望情绪（BGM 库 12 枚举之一），分配后仍保留以便 UI 过滤与解绑。
- **assign 像 casting**：webapp 的 episode BGM 面板按 `cat` 过滤库内 `bgm_NNNN` 供选，写回该行 slot；不手编。
- **烧录目标 = 带字幕整集视频**：源 `ep{NN}_zh.mp4`（保留不动）→ 输出 **`ep{NN}_zh_bgm.mp4`**，**重烧覆盖**；`renders/` 原始视频与 `shot{NN}_zh.mp4` 母版绝不覆盖。已分配 cue 按窗 `aloop→atrim→volume→afade→adelay` 铺位，`duck=on` 用整集自身音轨 `sidechaincompress` 让路，`amix=normalize=0`，`-c:v copy`；未分配 cue 跳过并在结果里报。

*(Originated from run bgm_library — 2026-06-16. 后端从 MusicGen（CC-BY-NC 非商用）换成 Stable Audio 因商用安全要求。13b episode 烧录 per follow-up 004 — 2026-06-20。)*
