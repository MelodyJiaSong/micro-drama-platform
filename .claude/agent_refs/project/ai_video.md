...
### 2b. Series nesting — `ai_videos/{series}/{episode}/`（per follow-up hy2 — 2026-09-09）

一部剧不再一定是 `ai_videos/` 下的一层目录。**同系列的多部短片挂在一个系列目录下**：

```
ai_videos/huangye_shenghuo/          # 系列根
├── series.json                       # ← 「这是系列」的唯一判据
├── _series/series_bible.md           # 跨片不变量的唯一出处
├── hy1/                              # 荒野生活 · 雨林第一夜
└── hy2/                              # 荒野生活 · 红杉第一夜
```
...
- **系列判据是显式标记文件，不是命名约定、也不是清单。** `ai_videos/{series}/series.json`
  存在即为系列；**成员剧仍从文件系统派生**，不在 `series.json` 里维护集数清单
  （对齐 CLAUDE.md § State surfaces 的「状态从文件系统派生」）。
- **`_` 前缀的目录永远不是成员剧**，所以系列可以安心把跨片共用材料放在 `{series}/_series/`。
- **片目录用 `{prefix}{N}`（`hy1` / `hy2`），不带题材后缀**——题材会变，编号不会；
  中文片名住在各片 `README.md`。specs 落 `specs/ai_video/{episode}/`；
  生成器 `tools/gen_shots_{episode}.py` / `tools/gen_scene_prompts_{episode}.py`。
- **跨片不变量只写一次**，落 `_series/series_bible.md`：那个人（锁定描述符 byte-identical、
  服装不换季）、声音结构、签名收尾、制作规格、实拍纪律、长线伏笔的进度表。
  各片 `concept.md` **只写这一部特有的东西**，不复述（rule 4i ①）。
- **系列最大的风险是第二部看起来像第一部。** 开新一部前先填这张四格表并写进 concept：
  ① 他做的是什么事（动词必须换）② 对抗者是谁 ③ 黑科技是什么（观众看懂的那个原理）
  ④ 奇观是什么。**天气预兆/活物功能也不许重复**——hy1 用了「鸟先飞走」，hy2 就得换成「雾反转」。
- **webapp 侧：剧根是 2 段或 3 段，判定集中在一处。**
  `projects/ai_video_management/libs/common/drama_ref.py` 的 `drama_depth()` 是唯一答案；
  树节点用 `is_drama` 标记剧根、`type: "series"` 标记系列，
  **任何消费方都不得从路径深度反推剧根**。系统库（`_actors` / `_bgm` / `_performances`）不是剧，
  但仍是导入目标，两类判定要分开写。

*(Originated from follow-up hy2 — 2026-09-09：用户「我要做很多集，所以要有更好的 structure，
比如一个 top level folder 叫荒野生活，然后下面第一个是 hy1」，并在被告知重构代价后仍选择真嵌套。)*
...
#### 4b-A 人物卡 / 道具卡里的出图 prompt，首行必须是路由键（per follow-up hy2 — 2026-09-12）

场景主体一直守着「**prompt 首行 ＝ 落盘文件名 stem ＝ `{key}-{N}_{视图名}`**」
（见本文件「参考图资产的命名与路由」），但 `characters/` 与 `props/` 的卡片是手写的，
很容易漏——**漏了下载导入按钮就匹配不到，图落不进目录**。三条：

- **凡是「拿去出图」的代码块，首行必须是 `{key}-{N}_{视图名}`**
  （`p1-1_砍刀锚点` / `c2-1_香蕉蛞蝓锚点` / `p3-1_树皮门板锚点`）。
  键纯 ASCII+数字、在首位、极短——出图工具按 prompt 前几个字命名下载并截断，
  键在首位才保证截断后仍带着它。
- **锁定描述符与负向清单不进 `text` 代码块。** 它们不是 prompt。
  锁定描述符用**裸围栏**，负向写进 prompt 自己的 `负面词:` 行。
  混进去的后果不是报错，是**长得像一个可以直接复制去出图的东西**——
  用户复制了、出了图、导入按钮匹配不到，然后来问为什么。
- **跨片复用的资产不写 prompt 块**（它不重出图），改挂快捷链接，见 rule 4b-B。

机检：`props/` 与 `characters/` 下每个 `text` 代码块的首行，要么以本目录的 `{key}-` 开头，
要么整卡一个 `text` 块都没有（＝复用卡）。两者之外都是缺陷。

#### 4b-B 跨片复用的资产走 `*.link.json` 快捷链接，不复制、不软链（per follow-up hy2 — 2026-09-12）

一个系列里，后一部常常沿用前一部的人物 entity、道具 ref 图。三条路只有一条对：

| 做法 | 为什么不行 / 行 |
|---|---|
| **复制字节** | 违反 rule 4i ①「一份东西只有一个出处，副本必漂」。改了一边另一边不知道。 |
| **OS 软链 / junction** | webapp 的每个 reader 都**主动跳过 symlink**（遍历加固），所以它在 UI 里根本不存在。 |
| **`*.link.json` 清单** ✅ | 一个 tracked 的小 JSON 落在「资产本该在的位置」，字节仍然只有一份。 |

清单长这样（放在 `hy2/2_世界观人设/props/p1_砍刀/p1_砍刀.png.link.json`）：
`{"target": "ai_videos/{series}/hy1/2_世界观人设/props/p1_砍刀/p1_砍刀.png", "note": "跨片复用 · 同一把刀，不重出图"}`

- **命名**：`{目标文件名}.link.json`，放在**资产本该在的那个目录**里。
- **渲染**：tree reader 把它渲成一个坐在本片目录下的叶子，但**节点的 `path` 是目标的路径**——
  于是 `/api/media` 直接就能服务它、预览无需任何媒体层改动；节点另带 `is_link` 供 UI 标记。
- **安全**：target 必须落在 `ai_videos/` 沙箱内、必须是真实文件、不能是 symlink；
  解析失败就**不渲染成链接**，退回显示那个裸 json，让用户看得见、改得了。
- **卡片写法**：复用卡**不写出图 prompt**，只写锁定描述符 + 本片的用法差异 +
  一句「本目录下有快捷链接」。
- 参考实现：`projects/ai_video_management/libs/common/asset_link.py`。

*(Originated from follow-up hy2 — 2026-09-12：用户「props 下面 prompt 也要 start with key
like p1、p2，这样下载的时候导入按钮才 work」＋「如果 character 或 prop 延续使用上一部的，
请建立快捷链接，UI 里能看得到链接以及 preview」。)*
...
**这条禁的是「低保真的自制 3D 渲图」，不是「参考图」本身（2026-09-18 澄清，shikong_lvxing/sk3）。**
判据是**参考源的信息密度**：blockout / 白模 / 灰模是**我们自己刚造出来的方盒子**，
信息密度比生成模型的先验还低，喂进去只会把上限拉到方盒子水平——所以禁。
而**史料驱动项目的公版历史原图**（版画 / 油画 / 老照片 / 地图）方向相反：
它带着模型先验里没有的**具体的那一座城、那一座桥、那一年的天际线**，
信息密度高于自由生成，image-to-image「保持原图完整性」出来的东西
**比纯文字自由生成更准也更好看**。这类项目**允许且应当**把史料原图当 image-to-image 输入。
分工不变（rule 4g ③）：**史图给长相，blend 给几何与运动**；
仍然禁止的是拿 blend 渲出来的图回头当出图参考。
外部佐证：Chloe VS History 创作者一手访谈——「信任来自史料，不是来自你让 AI 凭文字想象然后信它吐出来的东西」；
该做法要到 2025 年底模型能「保持原图完整性」才成立，在那之前上传参考图只被当风格参考。
拆解见 `ai_videos/shikong_lvxing/_series/format_teardown_chloe.md` §5，
登记见 `specs/ai_video/sk3/divergence.md` D3。
...
#### 2026-09-22 amendment — scenes 可按「世界自己的层级」嵌套；锚点级档；bg 登记簿（shengji_zhilu follow-up 006）

一部要把**整个世界**铺进 scene 的剧（艾泽拉斯两片大陆 49 区、600+ 主体），平铺 `scenes/bg{N}_*/` 是读不了的。
三条：

1. **`scenes/` 下可以嵌世界层级**：`scenes/{大陆}/{区}/bg{N}_{主体}/`（与游戏内地图「大陆 → 区 → 子区」一致）。
   **主体目录的判据不变**（`bg{N}_` 开头 + 同名 md），**层级目录只是索引**：每层放一份同名 md（H1 中文，左树按它显示）
   + `ref/`（原版区图的 `.link.json`，图只进人眼）。**所有消费者按判据递归找主体，不按深度**
   （webapp `series_shared.scene_subject_dirs` / `check_stage2` / `planschema.audit` 都已递归）。
   `blocks.toml` 的 `bg =` 键仍只许指**同级**目录——一个区里的主体互相引用，跨区不引用。
2. **锚点级档**（抬头一行 `档级：锚点级`）：只有锚点 prompt，没有 walk-through 与五张 plate
   （rule 4f「必做只有锚点」的落盘形态）。`check_stage2` 对它跳过 K23 五 plate 与「步骤二」；
   其余契约（零 hex / 一句话锁定 ≤30 / 首行路由键 / ≥1500 字）照查。要拍到它时再升成 v3 五 plate 档。
3. **bg 编号只有一个出处**：`scenes/registry.toml`（node id ↔ `bg{N}` ↔ 目录 ↔ 区）。生成器按登记簿追加、永不改号；
   区级主体（`bg{N}_{区名}全境`）拥有区级 floor plan，块 ＝ 世界树里有坐标的 bg（`src = 坐标表`），
   地形（山脊 / 水系 / 道路）由作者写在同目录 `terrain.toml`、生成器合并进 `blocks.toml`——**生成的与手写的不混在一个文件里**。
   生成器：`tools/gen_world_scenes_szzl.py`；闸门：`tools/check_world_scenes_szzl.py`（W1–W8）。
...
- **例外是「高度档」：航拍镜必须挂一张全城 / 全域俯瞰锚点**（2026-09-19 沉淀 sk1 shot01/shot02）。
  斜瞰属于派生视图、默认不做，但**平视锚点与高空俯瞰之间没有共同画面**——25 m 以上看到的是
  屋海、院落天井、街网与树冠，平视图里一格都没有。航拍镜只挂地面锚点 ＝ 长相无源，
  模型逐镜自编屋顶颜色与密度，同一条长镜拆成的两镜屋海对不上。
  所以每个 `World × State` 给一张俯瞰锚点，**全部航拍镜共用**（一张，不是每镜一张：
  方向仍由 previz 给）。`参考用法:` 必须写死分工——**白模动画给运动与几何、俯瞰锚点只给长相
  （屋瓦颜色质感、墙土色、水色、树色、空气通透度、「人只是黑点」的尺度感）、光线以本镜 `光线:` 为准**。
  ⚠ **这张俯瞰锚点同样是零参考自由生成，绝不能拿 blend / previz 的渲图去喂图模型**——
  城一旦建出来，手上就有一张看着很像「完美参考图」的俯瞰渲染，它是想要的结果的**下游**不是上游，
  喂回去就是 rule 4d ① 那个「拿白模渲图当参考、把质量上限锁死在方盒子水平」的坑第三次。
  方向永远是**图 → blend**；反向那条路是对账（`tools/plate_match.py` 同机位并排比色带与天际线），不是出图。

实测：entropy_city 5 个地点 + 3 个复原态 ＝ **8 张必做**，
而非「每地点固定 5 视图」的 26 张。**先出 8 张跑通全流程，再按证据补。**

**锚点 prompt 的篇幅窗口：1500–1600 字**（下限 1500，上限 **1600**）。锚点是全片视觉源头、且是唯一
零参考的自由生成——它的信息量直接决定全片的细节天花板，写短了后面所有图一起降级。

> **上限 1600 是实测值，不是估计（2026-09-19 xianjian_yi/bg1_大堂）。** 旧写法「硬顶仍是 2000」是错的：
> 即梦 text2image 服务端对 **image prompt** 的硬限是 **1600 字符**，1627 字直接被拒，报
> `ret=1046 InvalidNode ... prompt len is larger than limited len, prompt len: 1627, limited len: 1600`。
> **计法是原始字符数，含换行与空格**——同一段 prompt「去空白」数 1569、原始数 1627，服务端按后者判。
> 分层写的锚点空行多，两种数法能差 60 字，按去空白的数法自检一定会踩线。
> **被拒的提交不扣积分**（实测前后 210 → 210），所以踩线的代价只是一轮往返，但会拖慢批量出图。
> 注意与 **shot 视频 prompt 的 5000 字全局硬顶**（§12.4 / K10）区分：那是 Seedance 视频侧的限额，
> 两者不是一回事，别互相套用。1829 条旧注记的「即梦 实测 ~1500 字符更低」由本条给出确切数字。
...
> **2026-09-21 amendment —— 本节被 rule 4h-K 取代，只对没有 `planning/blocks.toml` 的旧场景有效。**
> 新场景一律走 **`planning/blocks.toml` + 通用引擎 `tools/build_scene.py`**，
> **不写 `blender_build.md`**。理由是实测出来的：建模指令是散文，散文既不可机检、也会与 blend 漂；
> 换成结构化 toml 之后，闸门能在**生成时**就 raise（未登记 kind / 块扎进山脊 / 块压块 /
> 块坐在河道上 / 机位埋在体块内部），`shengji_zhilu` 九个场景那一轮实测抓出 **21 条布局错
> 与 4 个埋在体块里的机位**——全都是人眼看渲图看不出来、或要看很久才看得出来的。
> 本节以下内容保留，供旧场景与「该建什么、精度分几档」的判据参考。
...
#### J. 场景 `.blend` 不出 video，只有 shot previz 出 video（2026-09-01）

**两层 blend，两种产物，不要混：**

| 层 | 文件 | 产物 | 干什么用 |
|---|---|---|---|
| **场景**（World 一份） | `scenes/{name}/_blender/{name}.blend` | **只有 `.blend` + 校验用 PNG**（平面图 `plan.png`、各锚点机位的灰模透视 `check_*.png`） | 几何源 + 人看布局。**不进任何 prompt 的参考位** |
| **shot**（每镜一份） | `shots/shot{NN}/shot{NN}_previz.blend` | `.blend` + **`shot{NN}_previz.mp4`** | 那段 mp4 进本镜的**视频参考位**，锁机位/走位/时刻 |

**场景层不需要 video reference，所以不要给它渲 mp4。** 场景 blend 里没有机位动画、
没有主体走位、没有时间轴——渲出来的视频不回答任何问题，纯烧时间（一份场景 blend 渲一遍
是分钟级）。场景层要回答的是「几何对不对」，那是**平面图 + 若干张定机位灰模静帧**的活儿：
平面图查走廊通不通、肌理有没有侵占走廊；灰模静帧查体量比例与锚点图对不对得上。

**唯一出 video 的是 shot previz**（`tools/previz/build_previz.py`），因为 previz 的全部价值
在于「第几秒到哪」——这是静帧表达不了的。

*(来源：duikang_shangzeng follow-up 032。)*
...
### 4h. 3D 层的工程契约：previz · builder · 生成式 3D（2026-09-05 沉淀 duikang_shangzeng 001/031/032/033 + xianjian_yi_mv previz 实践）

rule 4d 管 object 怎么建、4e 管场景主体、4f 管层级、4g 管环境几何。
**本条管「3D 这一层作为一套工程该怎么组织」**——谁是唯一真相、谁生成谁、并发怎么跑、
生成式 3D 什么时候上。全仓库所有剧共用。

#### A. 每镜必配 previz（覆盖率 100%），两层 blend 不许混

**previz 不是「复杂镜才做」的选做项，是每一镜的标配。** 早期「只给运镜复杂 / hero 的
18 镜做 previz，其余靠 prompt + 图像参考位」的分级取舍**已作废**（duikang_shangzeng
follow-up 001，61/61 全做）。理由：previz 锁的不只是机位几何，还有**动作细节的时刻表**——
而「第几秒做到哪一拍」是每一镜都有的东西，不是 hero 镜的专利。

两层 blend 的分工见 rule 4g §J（场景层只出 `.blend` + 校验 PNG，**不渲 mp4**；
只有 shot previz 出 mp4）。**它们的文件也必须分开住**：

```
2_世界观人设/scenes/{world}/_blender/{world}.blend      ← 几何源，唯一一份，全片共用
5_6_分镜与prompt/shots/shot{NN}/previz/
        previz_config.toml          ← 本镜 3D 层的唯一真相
        shot{NN}_previz.blend       ← 从场景源【拷贝】出来的副本，脚本只动副本
        shot{NN}_previz.mp4         ← 进本镜 prompt 的视频参考位
```

⚠ **previz 脚本绝不写回 `2_世界观人设/scenes/` 下的场景源文件。** 先 copy 再改。

**内容分档**（决定 previz 里摆多少东西，不决定做不做）：

| 档 | 内容 | 典型 |
|---|---|---|
| **A** | 完整建场：环境体块 + 全部主体 proxy + 机位动画 + 关键帧走位 | 多主体互动 / 并驾 / 对打 / 交接物件 |
| **B** | 单主体 + 机位动画 | 独角戏、空镜运镜 |
| **S** | A 档 + 逐拍手写编排（见 §C 第 2 条） | hero 长镜、成套武打/法术编排 |

#### B. `previz_config.toml` 是 3D 层的唯一真相

**shot md 的 prompt 不再复述 previz 管的东西**——画面坐标、相对大小、机位参数、
走位数值一律只写在 TOML 里；prompt 只写模型必须从文字知道的（表演、情绪、材质、光色）。
previz README 只记「为什么这么摆」，不记数值。

声明式 schema（`tools/previz/build_previz.py` 的 `SCHEMA` 白名单，**键名拼错直接报错、
绝不静默忽略**）：

| 段 | 键 |
|---|---|
| `全局` | `shot` `项目` `fps` `total_sec` `场景` `分辨率` `地面` `原点偏移` `贴地` `素模场景` |
| `机位` | `焦距` `俯角` `方位角` `基准主体` `占画高` `横向偏移` `距离倍数` `位置` `切墙` `起始俯仰偏移` `锁定主体` |
| `运镜` | `类型` `起` `止` `量` |
| `角色` | `名` `色` `身高` + `关键帧`(`t` `位置` `朝向` `姿态`) |
| `道具` | `名` `形` `尺寸` `位置` `色` `朝向` `档` + `关键帧`(`t` `位置` `朝向` `尺寸`) |

坐标约定（写在每份 TOML 抬头，跨剧一致）：
`+X 画右 / −X 画左 / +Y 画深处 / Z 高度`；朝向 `0 面朝镜头 / 90 面朝画右 / −90 面朝画左`。

**两条硬约束：**

1. **关键帧的 `t` 必须与 shot md `动作:` 字段的时间轴逐拍对齐**（duikang follow-up 001）。
   两处不一致 ＝ previz 视频和文字在对模型说两套话，模型只会选一套，通常选错那套。
   改任一处必须同步另一处。
2. **跟随镜里，被跟随的主体在 previz 中保持原点不动。** 机位与它同速平移时它在画面里
   本就不动；真正要 previz 定的是「**其它主体相对它**怎么动」。若给马也写 40m 的绝对
   位移，车不动而马狂奔，几帧内冲出画面。向前的绝对位移交给出片模型（路面流动、环境掠过）。

3. **航拍航线走解析式定义，闸门从「已打好关键帧的相机」回读**（2026-09-19 沉淀 sk1 shot01，
   用户反馈「中间突然加速」「把折返全去掉」「不要总是 180 度转镜头」「飞进城门直接撞墙」）。
   四条判据做成产物级闸门（`tools/build_bianjing.py verify_camera`）：净空 ≥ 2.5 m、
   侧向加速度 ≤ 1.2 g、视线偏航向 ≤ 100°、折返 ≤ 3 m。四条经验：
   - **闸门要测产物，不要测中间变量。** 净空修正一度写在打关键帧之后，于是闸门量的是修正后的
     数组、blend 里留的是修正前的路径——闸门报「净空 4.0 m」而成片直接撞墙。任何改路径的
     步骤都必须排在产物生成之前，闸门则从相机回读。
   - **路径要解析定义，不要事后修。** 折线采样 + 低通 + 净空拉回 + 权重重配，四步互相打架，
     产出过瞬移（263 m/s）、尖点（164 m/s²）与 36°/5 m 抖动。
     「直线 → 圆弧 → 直线」由切点几何解出，天生 C1；逐帧发关键帧后插值无事可做。
   - **闸门算三维，路径每一维都得 C1。** xy 干净不等于过关：高度分段若用 `x**0.9`
     （指数 <1 在 0 处导数无穷大）就是一个尖点，相机回读读到 81.7 m/s²。
     分段高度一律用 smoothstep（`u²(3−2u)`，两端导数为 0），任意两段自动 C1。
     同理，看点偏移与焦距变化也要余弦渐入渐出——硬开关会让云台在段落两端各弹一下。
   - **写进 TOML 的小数位也是精度。** 逐帧二阶差分把量化误差放大 fps² 倍：位置只留 2 位
     小数（1 cm）时凭空多出 ±12 m/s² 的假加速度，留 4 位后噪声 < 0.13 m/s²。
   附带一条通用教训：**键名只能有一处定义**——schema 写 `平滑`、reader 读 `平顺`，
   于是 TOML 里调的阈值一律不生效、默认值悄悄接管，排查花掉整整一轮。

4. **look 层两条容易反向的物理**（2026-09-19 沉淀 sk1 shot01 用户反馈「太像劣质 CG」「城门是关着的」）：
   - **水的菲涅尔方向**：垂直俯视＝看进浑水里最暗，掠射＝整片映天最亮。接反（`mix(Facing, deep, bright)`
     这类写法里 Facing 在俯视时≈1）在地面镜上看不出来，一到**航拍镜就整条河渲成死黑板**——
     因为航拍几乎全程掠射。它还会顺带吃掉**透过门洞/桥洞看见的光**（洞里看到的正是那段水），
     于是开着的水门在成片里一眼像关着的。改法：掠射 → 去饱和的亮灰天光（不是艳蓝），俯视 → 浑水本色。
   - **「看得出是开着的」靠透光，不靠把门做对**。门洞几何再正确，只要洞另一端没有更亮的东西，
     渲出来就是一块黑板。判据是**对着锚点图问「透过洞口能看见什么」**——bg2-1 里看得见对岸水面、
     远处的树与天光，我们的洞里什么都没有。同理适用于桥洞、窗、门廊。

5. **两条 clip 拼成一镜到底：接缝怎么设计**（2026-09-19 沉淀 sk1 shot01↔shot02，
   用户「要链接成一镜到底，画面、声音如何衔接得好，以这个作为 priority 来设计飞行路线」）。
   顺序很重要——**先定接缝状态，再回头排两边的航线**，反过来做一定对不上。
   - **接缝位置由两镜总里程的中点定，不是「上一镜飞到哪算哪」。** 两镜时长相同而里程不同，
     速度就必然不同；实测 sk1 原本是 57 vs 33–41 m/s，**接缝两侧近一倍速差**——
     这是拼接最扎眼的破绽，比构图错位、比色差都明显。先量两镜合计里程 ÷ 总秒数得巡航速度，
     中点即接缝；两边各自按这个速度排。量**产物**（从相机回读逐帧位移），别量关键帧折线长：
     插值后的实际路径比折线长，按折线估会偏（实测偏 57 m）。
   - **接缝要逐通道连续，不只是位置相接**：位置（`承接` 契约保证）、速度、航向、
     **垂直速率、转率、焦距**。缝后 ≥1.5 s 不许有任何机动——用一个显式关键帧把头几十米
     钉成直线平飞，否则观众看到的不是「继续飞」而是「换了一镜」。
   - **不要把缝藏在遮罩后面**（桥洞、拂过的枝叶、扫过的帆）。生成式管线的真正风险不是首帧
     （`承接` 保证首帧就是上一镜末帧，像素级相同），而是第二条 clip 在**随后几秒整体漂移**
     （水色/屋瓦/墙色）；遮罩遮得住一瞬，遮不住漂移。**应当反过来：把缝放在画面最简单的地方**
     （开阔水面 + 天，没有近景建筑），内容越简单两条 clip 越难跑偏。
   - **声音是接缝的主承力件，而且不能靠生成。** 模型给每条 clip 各生成一床互不相干的环境/音乐，
     两条拼起来一定听得出。做法：① 出片后一律 `tools/mux_av.py` **覆盖音轨**（rule 12.4-H）；
     ② 铺**一条跨两镜连续的 BGM + 环境床**，缝上不做淡入淡出、不换段；
     ③ **让一句台词跨过接缝**（上一镜末尾起头、下一镜开头落尾）——人耳对语流断裂的敏感度
     远高于对画面切口，**一句话没断，观众就判定没剪**。
   - **一帧的位移不是断点**：`承接` 解到上一镜的最后一个关键帧（t = total），而渲染止于
     t = total − 1/fps，两者相差正好一帧的位移（实测 1.79 m @ 44.8 m/s）。拼起来正是连续的，
     不要去「修」它。

5. **几何生成器必须知道相机飞在哪里；Place 足迹内的房子只能由 Place 自己建**
   （2026-09-19 沉淀 sk1 shot01，用户反馈「城门外的建筑规划像劣质动画」）。两条：
   - **「镜头先于几何」要机器化，不能靠人记。** 把城外关厢从「散盒子」改成「共墙连排街面」后，
     房子从 484 栋涨到 9396 栋，**其中一栋正好长在航线上**（闸门报净空 0.1 m）。
     原因是城层此前只认「走廊」（河/街折线两侧的半径带 ＝ *建到多远* 的范围问题），
     从不认**相机实际飞的那条线**（＝ *哪里不许有东西* 的净空问题）——相机沿门洞轴飞，
     走廊却是沿河量的，两者根本不重合。做法：builder 直接读各镜 `previz_config.toml` 的
     相机关键帧，在**低空段**（爬到屋脊高度以上就不必挖）沿航线挖一条 keep-out 管
     （`camera_keepouts()`）。高空段不挖，否则把城掏空。
   - **城层不得伸进 Place 足迹**（Place 的地面是开了洞的，城层房子摆进去就浮在洞上）。
     所以「城门外最热闹的那一段」这种**恰好落在 Place 足迹内**的密集肌理，
     必须作为该 Place 自己的表行 + 生成器出现（sk1 加了 Place B 方块 29「关厢街面」），
     不能把城层的禁区放开了事——放开当场就被 QC 抓住。
     判据：**看得见的空地如果在 Place 足迹内，是 Place 缺表行，不是城层缺覆盖。**

6. **previz 机位的三条实测契约**（2026-09-20 沉淀 sk1 shot35 全 36 镜巡检）：
   - **「悬停」要用速度曲线表达，不能用「两个重合的关键帧」表达。** 单调 Hermite 的文档说
     「相邻两帧同值＝速度归零」，那是**时间域**的说法；一旦开了 `匀速`（按弧长重新配时间），
     零长度段分不到时间，重合点就退化成**全速撞上一个急停**——闸门读到 1054 m/s²。
     正确写法：逐帧发关键帧 + `匀速 = false`，弧长按 smoothstep 走完，末速自然为 0。
   - **倒飞镜要显式声明。** 「边后退边拔高、镜头回看」是合法机位，但视线与速度本就相反，
     前飞口径的「视线偏航向 ≤100°」会把每一个合法退镜判成「边往前飞边回头看」。
     `[全局] 倒飞 = true` 让闸门改按**反向速度**量。**闸门误伤要给显式豁免，不要改路径去迁就闸门。**
   - **折线航点必须磨圆再逐帧化。** 四个航点直连是 C0 的，拐点处逐帧二阶差分必炸
     （实测 114 m/s²）。周围无几何时直接在弧长空间 0.5 m 重采样 + 两遍 ±20 m 低通
     （偏离几米，换处处 C1）；门洞这类窄处不能平滑，改走 rule 4h §B 3 的解析式。

#### C. 一个引擎，两种表达；**per-shot 脚本绝不与引擎重名**

1. **A/B 档一律走声明式 TOML + 通用引擎**（`tools/previz/build_previz.py`）。
   **禁止把引擎拷进 shot 目录改**——那是 rule 4e「一个目录 ＝ 一个主体」和
   §4i「一份东西只有一个出处」的同一条理由。引擎缺功能就改引擎，让全仓受益。
2. **S 档（hero 长镜的逐拍编排）允许写 per-shot Python**——22 拍的剑术编排、
   IK 解算的手臂与握持、逐把飞剑的轨迹，声明式表达不了，硬塞进 TOML 只会把 TOML
   变成一门难用的语言。但它**必须 import 共享库**（`tools/previz_rig.py` /
   `tools/previz_human.py` / `tools/previz_config.py`），只写本镜独有的编排。
3. ⚠ **per-shot 脚本必须叫 `shot{NN}_previz.py`，绝不能叫 `build_previz.py`。**
   改名要连工具链一起改：webapp 的 `previz__writer.py` 按文件名找脚本、并从中解析
   `SCENE_MASTER` 来重建 blend，**只认旧名就会在改名当天悄悄不再重建**。
   现行解析顺序 ＝ `shot*_previz.py` 优先，`build_previz.py` 作旧名兜底。
   （踩过：`shots/shot12/previz/build_previz.py` 与通用引擎同名，在盘上看起来就是
   「引擎被 fork 了一份 1372 行的私货」，实际是一份合法的 S 档编排脚本。
   **同名让一份合法产物看起来像违规，也让真正的 fork 无法被一眼认出**——命名是审计手段。）

#### D. 场景 blend 由 builder 脚本确定性生成，**不手改**

```
tools/build_{world}.py   ──►   scenes/{world}/_blender/{world}.blend
```

`.blend` 走 R2、被 gitignore（rule §媒体）。**所以手改过的 blend 没有可审阅的来源**：
没人能 review 它、没人能 diff 它、下一个人不知道那堵墙为什么在那儿。
改几何 ＝ **改脚本再重跑**，删掉 blend 重跑得到同一座城（确定性种子）。

推论：**一次性补丁脚本必须随 builder 落地而删除**。它们打在旧 blend 的物件名上，
对新 blend 重跑只会打坏。（duikang 的 `add_city_fabric` / `add_tunnel` /
`fix_plaza_opening` 三个补丁在 `build_entropy_city.py` 落地时一并删除。）

#### E. 几何覆盖范围按「镜头要拍到什么」定，不按「到原点多远」定

rule 4g §F 说「镜头先于几何」，这是它在**覆盖范围**上的具体形态。

踩过的坑：城市肌理用「到原点距离 > 320m 不生成」的**圆形裁剪**，而桥在 y≈260、
全长 600m，桥的两头早就出了那个圆——桥两侧 80m 内每 50m 只有 0–4 栋楼，北侧十二段
里九段是零。原点是广场，桥不在原点。改成按镜头覆盖画的**矩形范围**后即修复。

同处的第二个坑：为省物件数把远处街区从「2×2 个体量」合成「一个 52m 大体量」，
结果**更糟**——走廊 keep-out 是整块剔除的，大体量一旦压到走廊就被整块剔掉，
在桥边留出大洞。**小体量塞得进走廊之间，大体量只会被整块剔掉。**

还有一类只有渲图才看得见的缺陷，文档永远看不出来：洞口骑在广场地面上、
机位站到场地外面被门洞框住、桥上机位只比护栏顶高 0.1m 把护栏压成一条贴地细线
（看着像"没建"）。**平面图与灰模透视各抓各的**：平面图抓「走廊通不通、肌理有没有
侵占走廊」，灰模透视抓「洞口在不在该在的地方、街廊有没有中途豁然开阔」。

#### F. previz 批量任务不许重叠跑

**`TaskStop` 杀的是 shell，已经 spawn 出去的 `blender.exe` 还在继续写文件。**
停批之后必须确认 blender 进程真的退了再起下一批，否则后一批撞上被占用的文件、
报出**假失败**——而失败的恰好是停批当时正在处理的那几个镜，会把人引去查几何，
而几何是对的。

**更省事的做法：blend 定稿之后再跑 previz，别边改边跑。**

#### G. Hyper3D / Rodin：接入与使用契约（2026-09-05）

**接入**（blender-mcp addon，per-scene 开关 + 全局 key）：

| 项 | 值 / 位置 |
|---|---|
| 开关 | `scene.blendermcp_use_hyper3d`（**per-scene**，随 .blend 存；换文件要重开） |
| 模式 | `scene.blendermcp_hyper3d_mode` ＝ `MAIN_SITE`（官方站）/ `FAL_AI` |
| key | addon preferences `hyper3d_api_key`（持久）；scene 上同名属性会覆盖它 |
| 免费额度 | 「Set Free Trial API Key」＝ 共享 key `vibecoding`，**额度有限、供试手，不供批产** |
| 自检 | `mcp__blender__get_hyper3d_status` → 必须回 `enabled and ready`，并报 key type |

⚠ **开关是 per-scene 的，这是最容易误判「hyper3d 没装好」的地方**：preferences 里
key 明明设了，打开另一份 .blend 却报 disabled——因为那份文件保存时开关是关的。
`execute_blender_code` 里 `scene.blendermcp_use_hyper3d = True` 即可，不必重装。

**调用链**：`generate_hyper3d_model_via_text`（英文描述 + 可选 `bbox_condition` 长宽高比）
或 `generate_hyper3d_model_via_images`（有参考图时优先）→ `poll_rodin_job_status`
（MAIN_SITE 传 `subscription_key`，等全部 `Done`）→ `import_generated_asset`
（MAIN_SITE 传 `task_uuid`）。生成结果**尺寸是归一化的，导入后必须重新定尺**。

**什么时候用它，什么时候不用**——判据仍是 rule 4g §B 的四问，Hyper3D 不改变判据，
它只是「第 3 问＝是」时的一个 vendor：

- ✅ 有机曲面的、**有界的**小件：器物、构件（门楼/花窗/斗拱/瓦当/桥栏/石狮）、法宝、道具。
- ✅ 人物 proxy 的**服装剪影**（rule 4g §I：3D 层要给对的是宽袖长袍的**体积**，
  不是脸）。比色块人形强，比 MetaHuman 省。
- ❌ **环境**（城/镇/村/街区）——永远不用，四个失败模式见 4g §B。
- ❌ 平直重复精确的构件（墙/屋顶/街面/桥拱）——脚本更快更准。

**无论来自哪个 vendor，白模一律过同一道闸门**（rule 4d §A1b，
`tools/whitemodel_normalize.py` + `object.toml` 探针）。**genericity 来自闸门，
不来自来源**——Rodin / Hunyuan3D 本地 / 素材市场买的 / 脚本 blockout，
验收标准完全一致。free trial 的额度不该花在「反正能重生成」的试错上，
先把闸门和 `object.toml` 写好再生成。

##### G1. 实测失败：整身着装人物走 image-to-3D ＝ 一团光滑坨（2026-09-05）

拿一个角色的**立绘正面 + 两张视图页**喂 `generate_hyper3d_model_via_images`
（free trial / MAIN_SITE，bbox 5:3:17），六个 job 全 `Done`，导入、归一化到 1.8m 之后
渲出来是**一个没有四肢、没有头、表面光滑的花瓶状体**——不是人。

这与 §4g §B 的判据是一致的，不是意外：**「有界的有机小件」是甜区，整身着装人物不是**。
人物有细长突出物（四肢）、有需要被理解为分离结构的手与头、有大面积自遮挡的衣褶；
生成式把它塌成一个凸包。**判据里那句「有界」指的是「一圈转台就穷尽它」，
不是「有边界」**——人四肢一动，剪影就变，转台穷尽不了。

**代价小但要主动止损**：出了模型必须**先渲一眼再往下做**（一张正视 + 一张侧视足矣，
`tools/render_scene_view.py` 十秒的事）。别拿一个没看过的网格去做绑定、去做 previz——
下游全部白做。**免费额度不该花在「反正能重生成」的试错上**，这条已写在 §G。

**人物 proxy 的正解仍是参数化人体 + 现成衣物网格**：MPFB（或同类）出带骨架的人体，
衣物取**已有的布料模拟成品**（本仓 `characters/c8_酒剑仙/model/c8_酒剑仙.blend` 的
`JJX_Robe` 是 12309 顶点的交领长袍布料解算结果），蒙到骨架上。
它比生成式给的剪影准，也是唯一摆得出逐拍姿势的做法——**生成式给的是静态网格，
摆不了 22 拍**。Hyper3D 在人物这条线上的正当位置是**随身小件**（葫芦、器物、佩饰），
不是人本身。

##### D1. 换成纯几何 blend 时，要问「原来是谁在提供光」（2026-09-05）

场景 blend 按 §D 是**纯几何、零材质、零灯光**。而 previz 自己的 proxy 与特效通常全是
**自发光**材质——**不吃灯**。两者一合，场里没有任何光源照得到灰模，
庙与地面整片渲成纯黑，previz 就没能把「哪儿有遮挡、构图边框在哪」传下去。
（踩过：把手改的、自带材质的旧 blend 换成 builder 生成的纯几何 blend，第一版 previz 全黑。）

**修在 previz 侧，不修在 builder 侧**——builder 一旦开始塞材质，长相就往 blend 里渗了。
previz 补的是**型面可读的最低限度**：一盏弱定向光 + 极暗环境光（暗部不死黑），
不调色、不做阴影特效、不碰自发光道具。

> **更一般的一条：数值自查与看图各抓各的，两个都要做。**
> 那一版的画框自查报告 UV 全对（庙 u=0.183 ✓、人头 ✓、高悬剑 ✓），画面却是黑的。
> 自查能证明「东西在该在的位置」，证明不了「东西看得见」。

#### H. config 派生的事实用 `{{占位符}}` 写进 prompt，别手抄一遍（2026-09-05）

previz config 里已经有的数字——剑数、圈数、跺几下、翻几个跟斗、俯角、人占画高、总时长——
~~**不要在 shot md 的 prompt 里再写一遍死数字**。写成 `{{n_swords:zh}}` / `{{tilt_deg:zh}}` /
`{{total_sec:round}}` 这样的占位符，由 webapp 在**展示与复制时**从 config 实时填充；
盘上的 md 原文保留占位符，复制到出图平台的是填充后的文本。~~
**2026-09-06 改：盘上的 prompt 必须是可直接复制给出片模型的文本，不许留 `{{占位符}}`**（用户从文件直接复制，占位符会原样粘进 Seedance）。config 仍是 3D 层唯一真相；prompt 里出现的 config 派生数字（剑数 / 圈数 / 次数 / 俯角 / 总时长）写实际值，并在 shot md 顶部的说明里列出哪几个数字来自 config，改 config 时一并同步。

这是 §4i §A「一份东西只有一个出处」在 shot 层的落法：改参数只改 config，
Blender 预演与 prompt 文字同步更新，**不存在「改了 config 忘了改 prompt」这个状态**。
（来源：xianjian_yi_mv follow-up 051/053，shot12 实跑至今。）

配套的 config 写法值得照抄：**只暴露每个动作的 `duration`**，动作顺序固定、逐段首尾相接，
...
#### J. 白模的取景必须和 shot md 的 `景别档` 对账（2026-09-15 sk1 实测）

**previz 是给生成模型的构图参考，框错了等于没给参考。** sk1 的 29 个 S 档镜逐镜量测（运行库
把每个机位关键帧的「人占画高」打进 `shotNN_previz_report.txt`）后发现：**只有 3 个镜两端都落在
`景别档:` 的 ±40% 内**，最离谱的差 18 倍——人几乎贴在镜头上，或者整个人被裁到画外。原因是
`景别档:` 由排镜阶段定（它同时是切口判定与 K31 的输入），而 S 档机位是逐镜手写坐标，两边从来没有
对过账。三条：

① **每次渲完就落一份量测报告**（机位关键帧 → 人占画高 / 在不在画内），报告跟产物同寿，别只留在
   会被清掉的构建日志里；
② **反解而不是手调**：`人占画高 ∝ 焦距 / 距离`，所以偏差可以一次算准——先沿视线拉远 / 推近
   （室内 ≤1.8 倍，退多了穿墙；室外 ≤2.5 倍），差额交给焦距、焦距夹在 20–120 mm。
   已实现：`tools/previz_frame_fix.py`（默认只修「人被裁出画」的关键帧）；
③ **只修真正失效的，别全量套用**：偏紧偏松但人还在画里，往往是 spec 与机位设计的**判断题**
   （「50 mm 近景」和四米的房间本身就矛盾，只能改分镜或改景别），机器一律只报不改；
   spec 人占画高 > 1.2 的关键帧是物件特写档（茶盏微距、包子特写），人不是主体，也不许自动修。

④ **室内不许靠「往后退」找景别**：退 1.8 倍就可能退到墙外，渲出来后半段整屏灰（sk1 shot31 实测）。
   室内只改焦距；退距留给室外。
⑤ **人离「看向点」很远的镜，机位沿视线缩放会失控**：反解假设人就在看向点上，不成立时 frac 会在
   2.93 → 0.01 → 9.98 之间来回震荡（sk1 shot18 实测）。对策是阻尼（一次只走 70%）＋ **回滚到作者原值、
   标「人工定框」**——机器改不动的镜，留原样让人看图定，比留一个乱数机位强。

#### K. 批渲进行中不许改运行库（2026-09-15 sk1 实测）

渲染队列跑到一半时给 `tools/previz_sk1.py` 加了一行落盘报告，字符串里的换行没转义，**接下来的
17 镜全部 3 秒内 `SyntaxError` 失败**——队列不会停，它会一路把剩下的镜刷成 FAIL。两条：
① 运行库的任何改动都先 `python -m py_compile`（或跑一镜）再放行；
② **要改就等队列跑完再改**；已经在跑的批次里改运行库，等于把后面每一镜都赌上。

#### L. 缩放世界时只缩坐标，不缩尺寸（2026-09-19 sk1「精致缩小版的城市」）

「把城缩小到够拍就行」不是把整个世界乘一个系数——那样街会变窄、房会变矮、
墙会变薄，人站进去就不对了，拍出来是**模型沙盘**而不是**缩小的城**。
正确的切法是**只缩坐标、不缩尺寸**：地块锚点 / 路径航点 / 河流折线的 xy 乘 `CITY_SCALE`，
而街宽、开间、层高、墙厚、树高一律保持 1:1。效果是「同样真实尺度的东西，彼此挨得更近」，
镜头飞过时每秒看到的内容更密、空跑的路段消失，而每一样东西的绝对大小仍然对得上人。

配套的两件事：① 压缩后原本不相交的足迹会开始互相穿插，**必须加矩形相交 QC**
（landmark 与地块、设施与街道）；② 压缩改变的是「一镜能扫过多少内容」，
所以航线时长与 `停留` 权重要跟着重配，不能沿用旧值。

#### M. 诊断几何问题要「定点拉近」，且诊断相机必须是干净的（2026-09-19 sk1 实测）

**低分辨率的全景帧看不出几何错误。** 用户报「城门是关着的」，在 960×540 的成片帧里
洞口只有十几像素、就是一团黑；换成从门外 110 m、40 mm 的一张定点特写，
「拐子城翼墙贴着河道、洞内没有透光、船正好剪影在洞口」三件事同时暴露。
**判几何、判遮挡、判「这东西看起来对不对」，一律先渲一张定点特写再下结论。**

⚠ **诊断时必须新建一个相机并 `scene.camera = ` 指向它。** previz 的相机带着关键帧，
手写 `cam.location = ...` 会在渲染那一刻被动画求值覆盖回去，
于是**渲出来的根本不是你摆的机位**，而你还会拿它当证据——踩过一次，浪费一轮。

#### N. 「像是美术问题」的现象，先查有没有搞反的物理量（2026-09-19 sk1 实测）

整条河渲成一块死黑板，看上去是「材质调得难看」，实际是**菲涅尔接反了**：
着色器写成「正对着看＝亮、掠射＝暗」，而真实的水正相反（垂直看进浑水里最暗、
掠射角上整片映着天空最亮）。航拍镜几乎全程掠射，于是整条河全黑，
**连带把水门洞口也堵成黑的**（透过洞口看到的正是洞内那段水）——
一个物理量接反，表现成了两个看起来毫不相关的「美术缺陷」。

所以：**凡是「随角度 / 随距离 / 随高度变化」的着色参数，验收时都要同时看两个极端**
（正视 + 掠射、近 + 远、俯 + 仰），并对照一句物理判据。只在正视图里调材质，
必然在航拍镜里翻车。同类可疑量：菲涅尔、次表面、雾的密度随距离、
法线贴图的 Y 轴方向、粗糙度与各向异性。

### 4h-K. 每个场景主体都必须有一份 3D 产物（2026-09-21 用户定调）

**规则**：`scenes/` 下的**每一个 `bg{N}_{主体名}/`**，都要有它自己的 3D 产物，二选一——

| 主体形态 | 产物 | 放哪 |
|---|---|---|
| **单个物体**（一块碑 / 一扇门 / 一口井，一个网格就说完了） | **`.glb`** | `scenes/bg{N}_{名}/{bg}.glb` |
| **多个物体或有建筑**（一条街 / 一座院 / 一片山谷 / 一个洞窟） | **`.blend`** | `scenes/bg{N}_{名}/_blender/{bg}.blend` |

判据就一句：**「这个主体是不是一个网格能说完」**——不是，就走 `.blend`。
实践中绝大多数 `bg` 都含围合、地面、道具与复数建筑，所以**默认是 `.blend`**；
`.glb` 是那些其实该归到 `props/` 但被建成了场景主体的例外。

**为什么**：主体卡（`bg{N}_{名}.md`）+ 锚点图只锁**长相**，锁不住**几何**——
同一个地点在不同镜里会长出不同的开间数、不同的门位、不同的地平面。
rule 4g ③ 已经写过「一致性来自只有一份几何，不来自模型的记忆」；
本条把它从「一个世界一份 blend」收紧到 **「一个场景主体一份 3D 产物」**，
让每个 bg 都有可测量的几何底座，previz 机位、`景别档:` 的 `subj_frac`、
以及跨镜站位朝向才有共同参照。

**怎么建**（与既有规则完全对齐，不新增流程）：
1. `planning/blocks.toml` 仍是**几何唯一出处**（rule 4i ①）——山脊 / 水系 / 道路 /
   每个建筑组的 `xy · size · h_m · kind`，以及校验机位 `[[anchor]]`。
2. `.blend` / `.glb` 由**通用引擎** `tools/build_scene.py <scene 目录>` 确定性生成，
   **不手改**（rule 4h ④）。一个 scene 一份 toml，**不拷脚本**（rule 4h ③）。
3. `kind` 未登记的块**直接 raise**，blend 根本生成不出来；
   布局闸门（扎进山脊 / 块压块 / 坐在河道上）同样在生成时 raise——
   **左移进生成器，不留给出片前的 reviewer**。
4. 产物只有 `.blend` / `.glb` + 校验 PNG（俯视正射 + 每个 plate 锚点一张灰模透视）。
   **场景层不渲 mp4**（rule 4h §J），mp4 是 shot previz 的事。
5. **与 rule 4j 的「`floor plan` 一词退役」不冲突（2026-09-22，rule 4i ② 判为伪冲突）。**
   4j 退役的是**拿「floor plan」称呼一条空中航线**那个用法——它在影视工业里指室内 / 场地平面图，
   用来叫航线是错的，所以那边改叫**航线图 flight plan** / **走位平面图 ground plan**。
   而本条这份 `planning/{bg}_floorplan.png` **画的正是一块场地的俯视布局**（地面轮廓 + 建筑组方块 +
   道路水系 + 比例尺），**就是 4j 自己给出的那个正确词义**，因此保留原名。
   三者的分工：**场地平面图**＝这个地方有什么、各在哪、多大（场景层，本条）；
   **走位平面图**＝地面镜的机位与人物怎么走（shot 层）；**航线图**＝机位在世界里怎么飞（shot 层）。
6. `.blend` / `.glb` 属媒体字节，走 R2 + `assets.json`，被 gitignore；
   **`blocks.toml` 与引擎进 git**——所以几何永远可从源头重建。

**顺序不变**（rule 4e ①）：**图先行、3D 在后**。
锚点图纯文字自由生成、零参考图；**绝不拿这些白模渲图去当出图参考**。
白模是给 previz 与站位对账用的，不是给图生成模型看的。

> **2026-09-24 amendment —— 平面图编号块可以引用「区级资产库」（shengji_zhilu follow-up 007）。**
> 平面图上每个编号块都要能生成：`kind = "asset"` + `asset = "{key}"` 引用
> `scenes/{大陆}/{区}/_assets/{key}_{名}/`（资产卡三视图 prompt + `asset.toml` + 真实照片参考 + Rodin `mesh/{key}.glb`，
> `tools/gen_bg_assets.py` 出）；**同一区的同类物件只立一件**（rule 4i ①），成片块写单体 + `count`。
> 规划与落地：区级 `_assets/plan.toml`（agent 写）→ `tools/apply_asset_plan.py`（确定性写进 blocks.toml、补校验机位；
> `--build` 以 `build_scene.py` 的布局闸门为唯一判据，自动声明逐条记进 `apply_report.md`）。
> 四条随之定下的口径：
> ① **资产三视图的参考图只用真实世界、许可干净的照片**（Wikimedia Commons，逐张记作者与许可）——
>   这不违反上面的「零参考图」：那条管的是场景锚点图；权利人官方图与游戏截图不进任何模型（divergence #8 / #108）照旧。
> ② 线性构件（城墙 / 木栅 / 寨墙 / 围栏 / 道路 / 地形）仍走脚本 kind，**不做 image-to-3D**（rule 4g）。
> ③ 单件资产**逐轴撑满块的占地与 h_m**（占地来自坐标表，是权威；实测 Rodin 把 41×17 m 的旅店出成 1:2.5 的瘦高楼）；
>   没出模的资产以方盒占位（`…·待出模`），blend 照样建、机位照样能对。
> ④ 线性 kind（palisade / low_wall / tunnel / arch_bridge）**按占地的长轴定走向**：南北向的墙写成 `size = [3, 40]`、不写 `rot` 就对。
> 额度现实（2026-09-24 实测）：即梦三视图 9 积分 / 件；Rodin Sketch 0.5 credit / 件——Rodin 是瓶颈，先给第一季路线的区出模。

> **2026-09-25 amendment —— GLB 只装单个物体；场景 blend ＝ 场景图 + prompt + 单物体 GLB（用户定调，shengji_zhilu follow-up 011）。**
> 上表「单个物体的主体出 `.glb`」一行作废，**场景主体一律只出 `.blend`**；`build_scene.py` 不再导出 `{bg}.glb`（旧产物构建时顺手删）。
> ① **一件资产 ＝ 一个 GLB ＝ 一个物体**（一栋建筑 / 一张桌子 / 一只木箱）。块里有几样东西就写
>   `parts = [{asset, at = [x, y], yaw}, …]`（块自身坐标，正面朝南时 x 东 y 北；每件按自己的 `size_m` 逐轴缩放），
>   由引擎在 Blender 里摆——「长桌 + 两条长凳」是三件资产加一行 `parts`，不是一件「长木桌与长凳」。
>   `gen_bg_assets.py check` 拦下名字读起来是几样东西的资产（并列连词 / 「…群」/ 杂物 / 木箱木桶），
>   确是一件的（树屋连着它的环干平台）写 `single = "理由"`。
> ② **每个场景都要出图**：bg 锚点图 `{目录}/{目录}.png` 与每个方位 plate 图，`tools/gen_bg_images.py`
>   按依赖分层出——世界锚点零参考 → bg 锚点挂世界锚点 → plate 挂所属 bg 锚点；画幅跟成片（`style_guide` 画幅条）走，
>   卡里写的旧画幅要先改构图再出；即梦优先，送审稿（去掉 `参考:` / 画幅行）超 1600 字或即梦失败才退 ElevenLabs。
> ③ **没有场景图 `build_scene` 直接 raise**（图先行、3D 在后，本条上文与 rule 4e ①）。blend 里：[[anchor]] 的 key ＝ plate 目录名
>   （或写 `image = "{文件名}"`）时该图挂成这台相机的背景图（半透明，对着几何对账）；场景卡 / plate 卡 / 用到的资产卡的
>   prompt 进文本块；资产三视图与场景图立成图片 empty（只在视口可见，不进渲染）；机位相机随 blend 保存。
> 机检：`check_world_scenes_szzl.py` W11（bg 目录树里有 `.glb`，或资产库 GLB 不在 `{key}_*/mesh/{key}.glb`）blocker、
> W12（缺场景图）warning。实测额度：即梦场景图 3 积分 / 张（2560×1440）。

> **2026-09-25 amendment (b) —— 区级资产库并进本剧 `props/`；场景引用 prop 出图与建 blend（用户定调，shengji_zhilu follow-up 013）。**
> 上面 2026-09-24 那条「区级资产库 `scenes/{大陆}/{区}/_assets/`」作废：物件一律住 **`{剧}/2_世界观人设/props/p{N}_{名}/`**
> （`p{N}_{名}.md` 卡——三视图块首行 `p{N}-1_正面` / `-2_侧面` / `-3_背面` · `asset.toml`（`from` 记旧键）· `ref/` · 三视图 · `mesh/p{N}.glb`）。
> ① **一套编号**：剧情物件（p1–p14，单张锚点卡、无 asset.toml）与场景物件共用 `p{N}`，唯一出处 `props/registry.toml`
>   （`kind = story | scene_object`）；新物件 `python tools/props_lib.py new <剧> <名>` 领号，`gen_props_szzl.py` 的键也必须登记为 story。
>   目录定位只在 `tools/props_lib.py` 一处，build_scene / gen_bg_assets / apply_asset_plan / link_bg_assets / gen_bg_images 全引它。
> ② **场景只引用**：blocks.toml 写 `asset = "p{N}"` / `parts`；区里只留 `_plan/{plan.toml, apply_report.md}`（plan 的 `[[asset]]` 只列新物件）；
>   每个 bg 的 `assets/` 仍是指向 props 文件的 `.link.json`。Z4：区里还有 `_assets/` 即 blocker。
> ③ **合并判据**：同一件东西（形制、材质与状态都相同）全剧只留一件（保留已有网格的那件）；区域风格或状态不同的各留一件——
>   首次迁移 462 → 449（两名独立核验员都同意才合并，13 件）。剧情物件与场景物件的重叠（p7 修道院兵器架 ≈ 木制兵器架）暂不合并。
> ④ **场景图引用物件**：`gen_bg_images.py` 在世界基调 / 所属 bg 锚点图之外，挂该 bg 占地最大的前 3 件 prop 正面图，送审稿末加一行
>   「物件参考」（只借长相、不借构图机位光线）；即梦字数容不下就逐件减，卡太长就修卡。世界锚点也挂——4e ② 的「零参考」
>   防的是白模与别处场景图把基调锁死，物件产品照只锁单件物件的长相，不在它的禁区里 *(judgment call — 2026-09-25)*。

---

### 4k. 场地平面图（floor plan）：每个 bg 必备，且它是 previz 与出片的上游（2026-09-22 用户定调）

**链条**（每一环只有一个上游，不许跳级）：

```
floor plan（场地平面图 · 本条）
   → flight plan / 走位平面图（shot 层 · rule 4j）
      → Blender + previz MP4（rule 4g / 4h）
         → Seedance 视频
```

用户原话：「每一个 scene 里的 bg 都应该有 floor plan，图上标记着这个 scene 或地图里的
所有建筑物，label 好 1、2、3、4」。本条把它落成可执行的标准。
可执行版：**`tools/previz/planschema.py`**（不 import bpy，平面图生成器与场景引擎共用同一份判据）。

> **与 rule 4j 的命名不冲突**（rule 4i ② 判为伪冲突）。4j 退役的是**拿 floor plan 称呼一条空中航线**
> 那个用法；本条说的正是 4j 给出的那个正确词义。三者分工：
> **场地平面图**＝这地方有什么（场景层）· **走位平面图**＝地面镜怎么走（shot 层）· **航线图**＝机位怎么飞（shot 层）。

#### A. 一张图挂在哪一级：挂在**一个坐标系**上，不是挂在一个目录上

一份 floor plan ＝ **一个原点 + 一套米制 + 一组 footprint**。
**每个 `bg{N}_*` 目录都必须能解析到恰好一份图，但不必自己拥有一份。**
四问，按顺序问，**只看目录名就能答**：

| 问 | 答「是」则 |
|---|---|
| 1. 动了 A 里的一堵墙，B 里的同一堵墙要不要跟着动？ | **同一份图** |
| 2. A 与 B 只差机位 / 镜头 / 光 / 天气 / 时辰？ | **同一份图**，B 什么都不拥有 |
| 3. A 与 B 差在「哪些结构存在」？ | **同一份图** + `[meta] states` + 每块 `state_in` |
| 4. A 与 B 是同一批 footprint 的不同**分辨率**（山谷 260×300 vs 修道院 90×90）？ | **两份图**，子图声明 `[meta] parent` |

**拥有的判据是文件系统级的**：目录下有 `planning/blocks.toml` ＝ 拥有；
否则必须有一份三行的 `planning/plan.toml`——
`plan = "../bg1_xxx"` ＋ `state = "骨架"` 或 `anchor = "bg1-2_xxx"`，**解析不到就报错**。
这样「每个 bg 都有 floor plan」在文件系统上**字面成立**，而**几何只有一份**。

**反面写死，免得日后重新争论**：给 `hy1` 的「台地原始 / 台地骨架 / 台地覆顶中」开三份 toml 是**禁止**的。
台地边缘、林线、溪、坑位会被录三遍；谁挪了一根柱子，三张图开始各说各话**而且三张看上去都对**；
到 shot 层的症状就是那条**自己把自己填回去的沟**（rule 16 ③ / 16.10，两条都是本仓库真实事故）。
顺带一条常被搞错的：**天气与时辰从不构成 state，只有「结构是否存在」才构成**——
`bg4_棚屋建成日 / bg5_棚屋雨前 / bg6_棚屋暴雨夜 / bg7_棚屋雨歇夜` 是**一个 state 的四种天气**，四份 `plan.toml` 全指同一个 `建成日`。

#### B. 必填刻意压到最少——**没人写得起的标准比没有更糟**

`[meta]`：`bg` · `name_zh` · `size_m` · `scale_note` · **`scale_src`**（新）
`[[block]]`：**`id`**（新） · `name` · `xy` · `size` · **`h_m`**（新，升为必填） · `src`

三个新字段各自解决一个具体事故：

- **`id`（`^b\d{2,3}$`，分配一次、永不复用）** —— 圆牌号是**按面积降序生成**的，
  **把一块加宽 2 m 就会让一半的号重排**，于是所有引用它的航线 / 闸门 / 批注静默指到别的建筑上、不报错。
  所以：**圆牌 1、2、3、4 只用来看图（用户要的那个 label），`id` 才是可以被引用的键。**
- **`h_m`** —— 航线图**唯一算不出来**的那个数。今天它靠开一次 Blender 射线扫描 90 秒、
  再手抄成 `alt_floor=((340.0, 75.0, 9.5),)`，屋顶一改就过期**且没人知道**。
  `0` 是合法且有意义的值（草坪 / 石板地 / 空场）。图上**填充深浅 ＝ 高度，斜线 ＝ 贴地**。
- **`scale_src`** ∈ `实测 / 原典 / 坐标表 / 地图量取 / 推定` —— `推定` 的在图上挂一枚 ⚠，
  **不设抑制开关**。`bg1` 的场地 260×300 m 自己都写着「这个换算没有一手出处」，而图以前照样把它印成事实。

**条件必填**（下游要用才声明，不要一上来全填）：`kind` / `face`（本地要建 `.blend` 时）·
`[[anchor]]`（本目录有 plate 图时，一张 plate 一个）· `[meta] frame/origin_world/coord_scale/scale_source`
（有镜头在这地方走动时）· `state_in`（有 `states` 且该块并非每个 state 都在时）。

**可选但强烈建议**：`[meta] absent` —— 这里**没有**什么。
原来这些 ❌ 躺在 toml 注释里（`bg2` 的「无柱廊回廊 · 无钟楼 · 无训练假人」），
**任何读者、审阅者、负面词生成器都够不着**；它是本图唯一直接喂到 Seedance 负面词的内容。

#### C. **零个 `[[block]]` 是合法的**

全仓一大半 bg 是没有建筑的自然场景（林中列柱 / 空心红杉 / 冰湖 / 溪谷 / 巷道 / 水下）。
硬要它们「标出所有建筑物」，作者只会**编出建筑**或者**干脆不画**，标准就在最常见的那一档上静默失效。
这类图的内容是 `[[ridge]]` / `[[water]]` / `[[road]]` ＋ 自然类 block（`tree_cluster` / `boulders` /
`rock_shelf` / `pool`，全都已在 `build_scene.KINDS` 里）。

#### D. 「必备」靠**消费者跑不起来**来咬人，不靠全仓盘点

`build_scene.py` / previz / `shot_plan.py` **遇到解析不到图的地方就拒绝运行**。
于是**新活被挡住、没动过的旧剧一个都不扫**（CLAUDE.md 2026-09-06「不回溯旧剧」）。
`python tools/build_floorplan.py <根目录> --audit` 只报告覆盖率、**退出 0**；`--strict` 才判错。

#### E. 收据与反向禁令

- PNG 页脚与 `{bg}_blocks.md` 抬头各印一份 **`blocks.toml@<sha8>`**——
  **看图的人才知道自己批的是哪一版**；下游的 blend / previz / 成片各自记下自己是照哪一版建的。
- **`blocks.toml` 必须进 git。** 2026-09-22 实测：全仓 11 份**一份都没提交过**，
  而**派生出来的 PNG 反倒被 gitignore + 同步到 R2**——派生物有备份，唯一真相在工作区里一个 `rm` 就没了。
  这直接把 rule 4h-K ⑥ 反了过来。已全部 `git add`。
- **平面图 / 航线图 / 走位平面图 / 校验渲图，一张都不许出现在任何 `参考:` 行上。**
  图只进人眼与 previz，**不进生成模型**（rule 4e ① / divergence「图不进模型」）。
  它抵达 Seedance 的路径是 **previz MP4 ＋ prose 的 `场景:` / `走位:` 字段**，不是被当成参考图上传。

#### F. 明确**不要**做的（评审判定为过度，做了会被关掉）

- **可见性矩阵当硬闸门**（anchor × block 射线，判「直视 / 被遮挡」）。意图对，但图自己的页脚就写着
  「本图为示意，真几何在 .blend」，而 `bg1` 有三分之一的块是 `src = 推定`。
  **拿近似盒子在推定几何上跑硬闸门，必出假 blocker，而出假 blocker 的闸门会被关掉**——
  本仓库已记录过这个模式。降级为 md 里的距离 + 方位 + 遮挡物**参考表**；
  只保留那半个确定性的（机位眼位是否落在实体块内、视线是否离开场地）当真闸门。
- **任何需要起一个 Blender 才能跑的检查**（渲两遍比对 glb、plan↔blend 包围盒互校）。
  一次 Blender 约 90 秒、`build_scene` import bpy 所以外面调不动、而且仓库明令 previz 批量不许重叠跑。
  它们只会被跳过，而**被跳过的检查比没有更糟，因为它会被当成跑过了**。
- **per-block LOD / `nearest_m` 自动扫 shot 航线**。循环依赖：rule 4j 的顺序是
  图 → 用户点头 → 才渲 previz，rule 4g ④ 是镜头先于几何；要它就得先有还不存在的 shot，
  而且航线一改图上的数就churn，**毁掉它作为审批件的全部价值**。
- **`[[sun]]` 按时辰存方位角**。太阳位置是纬度 / 日期 / 时刻的函数，不是这块地的属性；
  把照明排期混进几何源，等于在更上游重开一次 16.9 那个事故（共用串断言了不是每镜都成立的光源）。
- **手抄一份「应有结构数」再与实际相等** —— 那是同一个数在两个文件里的第二份副本（rule 4i ①），
  作者合并两块或拆开一块时必然误报。

#### G. 已登记、未执行（需要用户或另一 session 拍板）

1. **shot 层的 `planning/blocks.toml` 是几何副本。** `tools/shot_plan.py` 从 **shot 目录**读它，
   而 `sk1/shot01` 那份的位置是把 `build_bianjing.DERIVED_ANCHORS` 的推导**又实现了一遍**、尺寸是手给的。
   **方向应当是：shot 层不拥有几何，只写「看哪一份场地平面图 + 哪几段时间」**，
   即 `previz_config.toml` 的 `["全局"]` 增一个有序的 `场景 = [{dir, t0, t1}, …]`（一条 1.2 km 的航线会跨好几个 bg）。
   今天全仓只有 1 份这种副本，**趁早改方向还来得及**；但 shot01 正在另一个 session 手上，不动它。
2. **一份图格式，不许有两类。** builder 拥有的世界（`bianjing`）应当由 builder **生成** `blocks.toml`
   落进 scene 目录并标 `[meta] generated_by`（`sk2` 的 `plan_stormwind.py` 已经是这么做的），
   手改禁止；shot 层那份示意 toml 随之删除而不是长期共存。两类格式 ＝ 每个下游永远多一个分支。
3. **系列共用的地点**，图跟着资产走进 `_series/`（CLAUDE.md 2026-09-15），
   每集的布局差异是那一份图上的 `states`，**绝不是一份分集子图**。

---

### 4i. 产物一致性纪律：单一出处 · 冲突判定 · 开工对账（2026-09-05 沉淀 duikang_shangzeng 031/032/033）

三条互相独立的纪律，都是在「产物已经很多、彼此引用」的阶段才会咬人的。
**它们适用于本 ref 的全部产物类型**（prompt、场景档、blend、大纲、shot md），不限 3D。

#### A. 一份东西只有一个出处；副本必漂

维护过一份「视图 prompt 摘要表」，与主体 md 是同一份 prompt 的两个副本。
结果必然发生：摘要表停在「参考 ＝ 平面图」的旧路线上，主体 md 早已改成
「参考 ＝ 世界锚点图」，而**漂的那一份还会被下一个人当成真的读**。

- **多份同构 prompt 走生成器**：`tools/gen_scene_prompts.py` 从单一模板生成 26 份主体 md，
  骨架跨 26 份 byte-identical，改一处全体同步。**改 prompt 的正确姿势是改生成器再重跑**；
  手改产物会在下次重跑时被覆盖，所以产物顶部要写明它是生成的。
- **索引/流程文件只写指针，不抄内容**：`scene_build_flow.md` 指向各主体 md，不复制 prompt。
- 同一条规则也解释了 §4h §D（blend 的出处是 builder 脚本）与 rule 4e（一个目录 ＝ 一个主体）。

#### B. 冲突判定三问，顺序不能颠倒

发现"证据打架"时按顺序问：

1. **这两个东西是不是同一个东西？** ——最容易漏、代价最大的一问。
2. 若确是同一个：**谁错？** 判据是**剧情需求 + 其它产物的交叉印证**，不是「谁更新」，
   也不是「默认听图」。
3. 错的那一方改，并把「为什么它是错的」写进产物（挂证据）。

**踩过的坑（伪冲突）**：两张参考图画的是「弯着跨水、两岸是城」的桥，大纲一句写
「下方的**城市**仍在褪色」，另两张图里长街上方有一道横跨结构。判成「二选一」，
选了跨城，于是**删掉了图里真实存在的水，又把三张好图标成待重出**。
真相是长街上方那道是**窄天桥**、图里那座是**城北跨水弯桥**——两座桥、两个地方。
分开之后根本没有冲突。

> **在伪冲突上做取舍，取出来的结果两边都不对。**

**镜像的坑（真冲突里默认听图）**：广场/主街/隧道三处的尺寸文字与图打架，
是**字错图对**；高架一处是**图对文字错**。判据是剧情与交叉印证，逐条挂证据，
不是统一"以图为准"或"以文为准"。

**推论：文字尺寸如果是「先建模时代」凭空定的，它不是证据。** 校正后每条空间事实
都应挂一列「证据」（哪张图 / 哪句剧情），没有证据的数字要标出来。

#### C. 「文档记载的现状」不是现状——开工第一件事是自查对账

按文档记载动手前，先量一遍盘上的真实状态。踩过的：建模指令文档抄的是 changelog 里的
记载（5 塔 / 4 墩 / 有 collection），实际是 6 塔 / 5 墩 / **零 collection**——
文档还把「collection 名不许改」列为不可动项，**它假设了一个并不存在的东西**。

所以每份「照此开工」的指令文档（如 `_blender/blender_build.md`）都必须**以一节自查对账
开场**，把「按文档应该是什么」与「实际量到什么」列在一起，对不上先改文档再动手。

*(来源：duikang_shangzeng follow-up 031/032/033。)*

#### D. 管线是单向的：下游产物永远不许回喂上游生成器（2026-09-19 抽象自 sk1）

rule 4d ① 的「绝不拿 blockout / 白模渲图当出图参考」不是关于白模的，它是**方向问题**的一个实例。
一般形式：**A 生成 B，就永远不许把 B（或 B 的渲染、截图、导出）当作 A 的输入。**

已经踩过或差点踩的三处，形状完全一样：
- 白模是图的下游 → 拿白模渲图去喂图模型，出图质量被锁死在方盒子水平。
- blend 是图的下游 → 城建出来之后，手上有一张看着很像「完美参考图」的俯瞰渲染，
  拿它去生成全景锚点，等于让城自己给自己当范本，误差从此只会自我放大。
- previz 是分镜的下游 → 拿 previz 的构图去反推 `镜头:` 字段，而不是反过来。

**为什么必然出事**：下游产物携带的是**当前实现的全部缺陷**（方盒子、缺材质、错误比例）。
把它喂回上游，模型学到的是缺陷而不是意图，下一轮产物继承这些缺陷、再喂回去——
这是一个正反馈环，每一轮都更像它自己、更不像目标。

**反向那条路只有一个合法用途：对账。** 同机位渲一张、和锚点图并排、报色带亮度 / 饱和度 /
天际线偏差（`tools/plate_match.py`），用来判断 blend 该往哪改。
**对账是读，回喂是写**——读没问题，写才是坑。

#### E. 先测下游，再投上游：别在会被覆盖的层上打磨（2026-09-19 抽象自 sk1 shot01）

本管线是分层的：3D 层给几何与运动、参考图给长相、出片模型给最终质感。
**每一层的产物有多少会活到成片，取决于下游模型覆盖它的程度**，而这件事是**可测的、且很便宜**。

所以在对某一层做大投入之前（例：为 previz 补地面纹理、大气、水面波纹，这是好几天的活），
**先用真实出片跑一个最小样本**（一个镜、甚至前几秒），看下游到底继承了什么、覆盖了什么：
- 若出片模型把画面整个重新贴图 → 上游只需**几何与构图正确**，材质打磨是白做的；
- 若出片明显继承了上游的塑料感 → 这笔投入是必须的，而且应该优先做画面占比最大的那几项。

**判据是画面占比，不是修复难度**：航拍镜里地面占 60%、天占 20%，
那么「地面有没有纹理」「有没有大气透视」的优先级就高于任何一栋建筑的细节。

一个配套的分工判据，用来决定某个缺陷该在哪一层修：
**构图 / 几何 / 遮挡 / 谁挡住了谁 → 出片模型会忠实继承，必须在 3D 层修；
材质 / 光色 / 质感 → 大概率被重新生成，先别投。**
（实测：门洞被误读成「关着的」是几何与构图问题，必须修；同一条片子里水面像塑料板，
则应该先测再决定。）

*(来源：sk1 shot01 —— 用户看完彩色 previz 反馈「太像劣质 CG」+「城门是关着的」，
两个反馈落在不同层，处理方式相反。)*

---

### 4j. 镜头平面图：走动镜在建 previz 之前，先出一张图给用户点头（2026-09-20 用户定调）

> **2026-09-25 修订（用户定调，全仓所有剧；来源 shengji_zhilu ep01：shot 的 `参考:` 挂着 `shot01_previz.mp4`，
> 却没有任何一步说明它怎么来）——每一镜都要先出镜头平面图，且它是分层出片的第二层：**
>
> | 层 | 产物 | 管什么 | 不管什么 | 改一次 |
> |---|---|---|---|---|
> | ① 场景 | scene blend + 场地平面图 floor plan（rule 4k） | 静态的地方 | 人、镜头 | — |
> | ② 镜头平面图 **overhead** | 每镜 `planning/shotNN_overhead.png` | 机位路线与视角、每个人的走位与时刻、建筑与本镜物件的位置 | 动作细节、形状 | 秒级 |
> | ③ shot blend | `previz/previz_config.toml` → `shotNN_previz.mp4` | 真实动作、走位细节、物件形状、镜头远近变化 | 色彩、表情、特效 | 分钟级 |
> | ④ Seedance | 成片 | 色彩、表情、特效、质感 | — | 最贵 |
>
> 1. **适用：每一镜**（不再限走动镜）。顺序仍是硬的：**overhead → 用户点头 → 才做 shot blend**。
> 2. **位置只写一次**：`shots/shotNN/planning/overhead.toml` 是唯一出处（`[meta] scene` 挂在该场景 floor plan 的坐标系上，
>    `[[camera]]` 路点 t / xy / 离地高 / 瞄准点 / 焦距 / `cut = true`（硬切到这里，图上画虚线「切」，不是运镜走过去），
>    `[[actor]]` 路点 `[t, x, y(, 面朝x, 面朝y)]`（Δt ≤ 0.2s 的换位＝切点的时间压缩，画虚线），`[[object]]` 本镜物件，`[[beat]]` 时间轴）；
>    shot blend 的 `previz_config.toml` **从它读位置、只加动作与形状**，不另填一份坐标（rule 4i ①）。
> 3. **工具**：`python tools/shot_overhead.py --all <ep 目录>`（底图直接复用 `build_floorplan.draw_site`，两张图不各画各的）；
>    构建闸门在画图前拦：键名白名单 · 时刻落在镜长内且不倒退 · **入画人物与 shot md 的 `角色:` 行双向一致** ·
>    机位扎进实心体块 · 起幅 / 落幅按 `景别档` 与焦距反算的机位距离差太多（提示）。实测 ep01 首跑就拦下机位埋在柴垛里、
>    埋在修道院主厅里、埋在木屋里各一次，以及三处走位方向写反——散文 prompt 里一处都看不出来。
> 4. **名字统一叫 overhead（镜头平面图）**：影视工业里画机位、运镜与演员走位的俯视图叫 overhead（overhead diagram /
>    camera blocking diagram）；floor plan 是场地本身（rule 4k），flight plan 是航空用语。下文 2026-09-21 的
>    「航线图 / 走位平面图」两分法由此取代；空中航线的生成器与闸门细则（sk1）仍有效；旧剧已出的图不回溯改名。

*(来源：用户 2026-09-20 —— *「shot1 的镜头还是很不满意，我觉得有几个事情要做，第一个是我们之间的
交流少了……先给我一个 shot1 飞行的 floor plan，让我知道你准备从哪里先飞到哪里，途径什么建筑」*。)*

**适用**：任何机位在世界里走动的镜（航拍 / 穿行 / 长距离移动 / 跨地点）。原地推拉摇不必。

**顺序是硬的：图 → 用户点头 → 才建 previz、才渲。** 反过来做 ＝ 拿渲染时间换沟通。

```
python tools/shot_plan.py <shot 目录> [--waypoints 0,8,15,30]   # 航线图 flight plan
python tools/shot_plan.py <shot 目录> --kind ground             # 地面镜：走位平面图 ground plan
python tools/shot_plan.py <shot 目录> --coverage                # 这一镜到底拍到了哪些主体
python tools/shot_plan.py --city <scenes/{world} 目录>          # 全城 bg 分布图
```

**名字（2026-09-21 用户定调）**：机位在世界里飞的叫**航线图 flight plan**（`shotNN_flightplan.png`），
地面镜的机位与人物走位叫**走位平面图 ground plan**（`shotNN_groundplan.png`），统称「镜头平面图」。
`floor plan` 一词退役——它在影视工业里指室内 / 场地平面图，拿来称呼一条一公里多的空中航线是错的。
旧剧已经出过的 `*_floorplan.png` 不回溯改名（规则变更只对新增与改动的产物生效）。
**注意退役的是「拿它称呼航线」这种用法，不是这个词本身**：场景的场地布局图
（`tools/build_floorplan.py` → `{bg}_floorplan.png`）继续叫 floor plan 是**对的**，
因为那正是这个词在工业里的本义。三者是一族：航线图 / 走位平面图 / 场地平面图。

**每个建筑组都要标 bg 归属键（2026-09-22 用户定调「每个建筑请标记 bg1、bg2 类似的标号」）。**
`blocks.toml` 的每个 `[[block]]` 可写 `bg = "bg2"` —— 指这一块**属于哪个 bg 主体**（多对一：
修道院的主厅 / 前庭 / 两翼都是 `bg2`）。图上青色写键、灰色写「无专属 bg」，**两种都显式写出来**：
留白会让读者分不清「漏标」与「本来就没有卡」。键是**可选**的，但**写了就必须指得着**——
`build_floorplan.py --check` 校验它对应一个真实存在的兄弟目录 `bg{N}_*`，指不着直接报 blocker
（指向不存在的目录比留空更糟：图上看着有卡，实际点不开）。
这条把 `shot_plan.py` 早就在做的事（航线图上标 `bg20 城内河街铺面` / `无专属 bg`）补到场地图。
**用途是盘缺口**：`bg1_北郡山谷` 的 14 块里 9 块其实就是别的 bg 主体（修道院 bg2 占 4 块、
矿洞 bg3、墓地 bg17、葡萄园 bg18、小屋 bg19、隘口 bg20），剩下 5 块没有卡 —— 一眼可见。
**叶子场景全是「无专属 bg」是正常的**，不是漏标：它们的 block 是内部陈设（祭台 / 书架列 / 矮院墙），
本来就不该各自成为一个 bg 主体。
⚠ **`bg{N}` 是全剧唯一的主体路由键，不是每张图的序号。** 图上的圆牌序号（按面积降序）与
bg 键是两套东西，**不可互相替代**——把圆牌标成「bg1 bg2 bg3」会让 `bg2` 在这张图里
意思是「第二大的块」、在别处意思是「北郡修道院」，是货真价实的地雷。

**画法统一走 `tools/previz/planstyle.py`（2026-09-22 用户定调「统一 flight plan 的规范」）。**
配色 / 字体 / 标题块 / 北针 / 比例尺 / 图例 / 光晕文字**只在这一个模块里定义**，
`shot_plan.py` 与 `build_floorplan.py` 都从它导入；改图面规范 ＝ 改这一个文件，两张图一起变。
统一后的约定：**米白纸底**（不用深褐画布，打印与投屏都不糊）· **标题块靠左上**
（不居中——右侧要留给航点侧栏，居中标题会和侧栏打架）· **细箭头 + `N` 指北**
（不用红三角圆章，那是地图装饰、会跟强调色抢注意力）· **分段刻度比例尺**
（不用单段，分段才能不换算就估中间距离）· **底部色块图例**（不塞进副标题，那行读者只扫一次）·
**红色 `ACCENT` 是全族唯一强调色**。
起因：同一族俯视图长期是两套视觉语言，而这些图存在的**全部理由**就是「读图比读散文快两个
数量级」——每换一张就要重建一次视觉索引，那点速度优势就被吃掉了。
反面教训：两个工具各写一份配色与字体字面量，属于 CLAUDE.md「一个名字只有一处定义」
说的静默漂移；`build_floorplan.py` 还把字体硬编成 `C:/Windows/Fonts/simkai.ttf`，
换机器直接抛 `OSError`，而平面图是「随时重跑」的产物。`planstyle.font()` 改为按候选名查找。

**① 单镜图** 落 `shots/shot{NN}/planning/`：`{shot}_flightplan.png`（河 / 城墙 + 建筑组方块 +
红线航线 + 方向箭头 + 航段牌 + 航点 + 高度剖面）、`{shot}_route.md`（同一批数字的表）、
`blocks.toml`（建筑组方块：名字 + 中心 + 尺寸 + 旋转 + **bg 归属键**，随手改随手重跑）。

**图上只留四样**：河与城墙（定位）、灰方块 + 名字（途经的建筑组）、红线 + 箭头（航线与方向）、
航点圆点。网格、视锥、街道线一律不画——看图的人只需要判断「路线对不对」。
灰方块是**示意**、不是几何真相（真几何在 `.blend`），位置由真实坐标推出、尺寸手给，图上写明。

**② 全城图**（`--city`）读 `scenes/{world}/bg_anchors.toml`，出 `{world}_bg_map.png` +
`bg_index.md`，回答「这个世界哪些地方有主体卡、哪些没有」。

**③ 坐标只有一个出处。** 两张图的城市几何都来自世界地理坐标表（sk1 ＝ W11 §2.8），
经 `tools/previz/city_layout.py` 乘同一个 `CITY_SCALE`——**该模块是 `build_bianjing.py` 与
本工具的共用出处**。`bg_anchors.toml` 里一个坐标都不写，只声明挂在坐标表的哪一条
（`gate` / `landmark` / `river_offset`）。理由见 CLAUDE.md §「一个名字只有一处定义」：
尺度抄第二份不会报错，只会让图上的城市与航线错位，而且看上去毫无破绽。

**④ 图出来必对三件事**，都有数可查、不靠目测：

- **航线与它声称跟随的地物**。prompt 写「贴着汴河飞」，就看 `route.md` 的 `离{河}中线` 一列。
  **实测事故（sk1 shot01）**：全程只有穿门洞那一瞬离河 2 m，其余 96 / 150 / 205 / 318 m，
  与河道夹角最大 61°。成因是航线用「直线 → R400 圆弧 → 直线」解析生成以保 C1 平滑，
  而汴河在东水门这段是折的（门外 330° → 门内 297° → 305°），大圆弧接不住折角，几何上必然甩开河。
  **后果不是「不好看」，是模型收到两套指令**——白模 previz 是几何权威（prompt 里明写
  「机位路径以它为准」），prompt 文字却说贴河飞。这是「镜头怎么调都不满意」的一类可定位根因。
- **途经的建筑组有没有主体卡**。标着「无专属 bg」的块，出图只能借 bg0 全城航拍的长相。
  反复入画还没卡的，回阶段 2 补，别在 prompt 里用形容词硬顶。
- **方向**。箭头指航向；一条没有箭头的线，两头都说得通。

**④b 入画覆盖表要机器算，不要目测**（2026-09-21）。`--coverage` 逐帧算相机水平视锥，
列出每个世界坐标条目的**入画秒数**与**最近距离**，并标出哪些反复入画却**没有主体卡**——
`bg_anchors.toml` 的 `[[todo]]` 就是这张表的产物，改一次航线重算一次。
sk1 shot01 实测：繁塔 17.5 s / 510 m、旧宋门 12.4 s / 534 m、汴河南岸角门子 13.6 s / 451 m、
迎祥池 10.9 s / 685 m——四个都够格补卡，而在此之前它们只是图上一行「无专属 bg」。

**④c 航线本身必须有生成器，逐帧关键帧不许手改**（2026-09-21，sk1 = `tools/gen_route_sk1.py`）。
航点表写「沿河第几米、偏哪一岸几米、这一点多快、多高」，逐帧机位由它算：
弦长参数化样条 → 等距低通（窗口自适应放大，**判据是逐帧产物上的侧向加速度**，
不是曲率估算——估算会偏小、循环提前收手，闸门却在下游报 2.9 g）→ 速度剖面积分配时间。
**时刻由速度积分得出、不由作者写死**：按 (时刻, 弧长) 配时间，两个航点差半秒就能挤出
9 m/s 或 100 m/s，而逐帧二阶差分把这种「时间上的抖」读成侧向加速度，
于是闸门拦下一条几何上明明很平缓的航线。闸门不过就不写盘。

**④d 航线要按 blend 里的真几何验，不能只按坐标表**（2026-09-22，代价最大的一条）。
Place 局部几何与全城坐标表并不重合：sk1 东水门外 40 m 是两道拐子城夹出来的 15 m 水巷，
而 **W11 的河中线正好压在南墙里**；门内北岸贴着水边就是沿城客店排。于是「沿着中线飞」
在图上完全合理、在几何里连撞五次（builder 的净空闸门逐次报 0.0 m）。做法三条：
① 用 `blender --python-expr` 对候选航线**逐点射线扫描**，把「哪一段多高、偏哪边是空的」量出来；
② 把结论写回生成器的 `corridor` / `alt_floor` 闸门，两秒就能拦住同一个错误，不必每次开 Blender；
③ 穿门这种十几米宽的窗口，**航线在那一段要写成世界坐标里的直线**——任何弯都会被平滑切掉一截
而蹭到墙，直线没有可切的角。物理上过不去的事也要认：sk1 门区必须从 45 m/s 减到 22 m/s，
因为「40 m 内完成 13→3.6→9 m 的俯冲拉起」在 30 m/s 上是 2 g 起步。

**⑤ 没有点位的主体要写明原因，并印在图上。** 全城俯瞰、内景、卡里没指名地点的，
进 `[[unplaced]]` 并附一句为什么。**把「漏标」与「本来就没有点位」分开**——
否则每看一次图都要再问一遍同一个问题。
...
### 6. Per-shot duration — flexible 3–30 s by plot beat (≤ 30 s ceiling)

> **⚠ 2026-09-07 amendment — 4–6 s 的碎镜优先合并，不优先保留（用户 follow-up hy1/001）。** 时长仍按 beat 给，但**在写 shotlist 时，凡落在 4–6 s 的镜先问一句「它能不能和相邻镜并成一镜」**——理由是**视频生成器是按镜生成的，一条镜的环境音 / BGM 是整段统一的**：一条 12 s 连续镜只有一条音床，拆成两条 6 s 就要在剪辑台上接两段互不相干的声音，剪辑成本凭空多一倍。**合并的正确形态是「一镜到底的连续运镜」**（下摇接特写 / 推进到微距 / 拉开到中近景 / 转手持跟随），不是在一个 prompt 里塞一个内部硬切——后者只会逼模型在一个 take 里演一次转场。**真的并不了才保留**（换了地点、换了建造状态、换了时间），此时给它补一个真实的小 beat 撑到 ≥7 s，而不是留着 4 s。用户可按项目把下限写进生成器（hy1：`MIN_D=7`）。快切仍然合法——**这条针对的是「碎」，不是「快」**：真正需要 3–5 s 的反应镜、跳切、蒙太奇节拍不受影响。
>
> **⚠ 2026-09-22 限定（来源 `shengji_zhilu` E1/E2 · W19 决策 2）——「统一音床」这条理由只在保留生成器自生音的项目成立。**
> 上面把合并的首要理由写成「一条镜一条统一音床，拆开就要在剪辑台上接两段互不相干的声音」。这个理由的前提是**成片用生成器自带的环境音 / BGM**。
> 若项目裁定**去掉全部自生音、四层音轨 100% 自制**（`shengji_zhilu` 就是这么定的），那就不存在「两条不相干的音床」，**这条理由对它不成立**。
> **但合并本身仍然该做**，只是换一组理由：切口（相邻镜要一眼可辨）、渲染与返工成本、叙事密度。
> 写 shotlist 时别再拿音床当论据去说服一个自制音轨的项目——**它会发现这条不对，然后连带怀疑整条规则**。


> **⚠ 2026-09-06 amendment — 单镜上限 30 s、prompt body 硬顶 5000 字（用户：生成器新限制）。** 时长仍按 beat 给：普通对白 / 反应镜 3–8 s 不变；**15–30 s 只给 hero 单镜**（如 xianjian_yi_mv shot12 的 20.5 s 一镜到底剑法），> 30 s 才拆镜。5000 字是硬顶不是目标，12.4-E 密度与去重原则不变；`格式契约` K10 → 5000、K11 → 3–30 s，`时长节奏` PA6 拆镜阈值 → 30 s。**同日用户裁定：做镜阶段没有分集时间轴——每个 shot 的时长可以自由变，`timecode` / `成片用段` 只是记录不是约束，与旧槽位对不上不算问题、不列 pending；等每个 shot 都满意了再考虑怎么剪成成片。**

> **⚠ 2026-09-08 amendment — 镜内允许切镜，合并单位改为「一个地点 ＋ 一个状态 ＝ 一条镜」（用户 follow-up hy1/002；**部分修订上一条 2026-09-07 amendment**）。** 上一条把合并的正确形态限定为「一镜到底的连续运镜，不是内部硬切」——**这条限制取消了**。用户实测裁定：**在一条 20s+ 的生成视频里切换镜头是 OK 的**，模型在一条 clip 内能保持画面与音乐一致。于是合并单位从「能不能连续运镜」改为 **「一个地点 ＋ 一个建造/剧情状态 ＝ 一条镜」**，落在 **20–27s**。两边都要认：**收益**＝一条镜一条统一音床（镜内换段不必接音）+ 出片轮次成倍减少；**代价**＝**一条 27s 的镜里任何一拍崩了，整条都要重跑**，好的几拍一起赔进去。所以 ① **不无限往上并**（上限 30s 是渲染硬顶，实际压在 27s 以内）；② **hero 段（情绪最高点 / 签名定帧 / 封面候选）排在各自镜的片尾**，坏了只重跑那一条、前面的段还能单独取用；③ 换地点或换状态就必须分镜——硬并只会逼模型在一个 take 里演一次转场。落地要求：shot prompt 新增一行 **`分镜:`**，写死每一段的**起止秒 + 段名 + 切进来的方式**（`连续运镜` / `切`），并在 `参考用法:` 声明「标『切』的段之间是干净硬切、不做任何转场特效；**切镜不改变时间线**，全镜是同一个连续时刻，环境音与光的连续性不许断」；多段各自的场景参考图按段顺序挂在 `参考:` 行上。参考实现：`tools/gen_shots_hy1.py` 的 `cuts` / `vs` 字段。**这条不改快切品类的默认**——短剧仍按 rule 6 的 fast-cut 走；它适用于**单条成片、镜数远少于时长预算、且以环境音为主要音轨**的片型。

Every shot's `时长:` is chosen **per-shot** to match what the dramatic beat actually needs, anywhere in **3–30 seconds**. 30 s is the **ceiling, not the target** — padding a 5 s reaction to 15 s of slow camera drift dilutes the beat and tells the model to invent uninstructed filler (where it typically drifts in motion / framing / lighting). Conversely, hook / cliff / monologue / cover-frame shots that genuinely carry information toward 15 s should use it.
...
> **(甲) 排镜阶段的第一约束。** 定完每镜情绪与景别后，**先逐对算 `人占画高` 比值再往下做**——比的是**上镜落幅 ÷ 下镜起幅**。✅ ≥2.0 或 ≤0.5（跳两档，首选）／⚠️ 1.5–2.0 且同时换 plate 或机位方位 ≥45°（下限）／❌ <1.5 且两者皆无。五级阶梯的数值量尺、判据表与正反例见 `agent_refs/project/ai_video_jingbie.md` §1 / §2.0（铁律 ⓪）。
>
> **(乙) 每镜必写 `景别档:` 行**（`## Shot context` 内，格式 `景别档: {起幅景别}{人占画高} → {落幅景别}{人占画高}`，无推拉写单值）。它与 previz 的 `previz_config.toml → ["机位"].subj_frac` 是同一个量，**排镜定的景别与 3D 预演实渲的景别由此对账**。判定归口 `ai_videos__运镜` M3，机检归 `ai_videos__格式契约` K31。
>
> **(丙) 为什么优先级高于 (A)/(E) 的承接机制——它买的是剪辑工时，不是美观。** 切口明显时两条 clip 直接首尾相接即成立，剪辑台零操作；切口含糊时只能靠 (F)/(F2) 那一整套善后（freezedetect 裁死运动帧、RIFE 补帧、色彩对齐、xfade），再叠加 (J) 的接缝两端 0.3s 台词静默守卫。**那套工具箱全是在为一个本该在排镜阶段避免的问题买单。** 故：默认＝硬切＋跳档；**承接首帧只保留给「观众眼里必须是同一个连续动作、中间不能有切口」的对**（例：一个动作被拆成两条 clip 生成），用它即连带认下 (F)/(F2)/(J) 全部工序。
>
> **(丁) 两者互斥，不存在「半接不接」。** 一对镜要么明显切、要么无缝接。**同景别、同机位、同场地、同色调却又不做承接**——既没有切口也没有连续性——正是「衔接不自然」的真面目（现场：`xianjian_yi_mv` shot08 中景弧线环绕 直切 shot09 中景向前推进，比值 1.0，两个都在动，观众分不清是切了镜还是机器抖了一下）。
>
> **(戊) 运动方向差不能代替景别差。** 两镜都是运动镜头、只是方向/轴向不同，读起来更乱而不是更清楚。
>
> **(己) 范围。** 新排的镜与本次真的动到的镜按本条走；**旧剧不做全仓回溯**（CLAUDE.md 2026-09-06 不回溯条），`格式契约` K31 只在被显式指定的范围内报违规。
...
- **⚠ 2026-06-22 amendment — Seedance 2.0 全能参考（Omni Reference）@标签 + 首尾帧角色分配契约（research-backed·指导 shot prompt 写法）.** 来源：用户 research（Seedance 2.0「全能参考 + 首尾帧」机制，2026-06-22；sources: PiAPI seedance-2-0 / WaveSpeed complete-guide / seedance-2ai.org first-last-frame / 即梦全能参考完全指南CSDN / GitHub Emily2040 seedance-2.0 pipeline）。**核心认知：Seedance 2.0 的「首尾帧」不是与全能参考并列的独立模式，而是「全能参考」里的两个角色——把首帧图 / 尾帧图当作 @ 参考素材上传，再在 prompt 正文用 @标签给每个素材派角色（首帧 / 尾帧 / 角色锁 / 场景锁 / 运镜参考 / 背景音）。** 故凡有 `本镜首帧=>` / `本镜末帧=>` 的镜，写 prompt 时须把它们落成全能参考的 @ 首尾帧分配句，而非只留占位。落地规则：
  ...
  2. **`参考:` 行每项以裸 `=>@` 收尾，`@` 后留空——绝不代填槽位号（2026-09-06 用户裁定 · 取代 2026-06-28 的 inline `=>@N`）**：`参考:` 行每个上传项写成 `` `{项名}(类型)=>@` ``，例：`参考: \`c19_萧若云=>@, c12_围观武者乙=>@, c12_围观武者乙声音=>@, 镇演武场_bg1_场口_入场=>@\``。
     ...
     **`@` 是用户上传时手填的占位，Claude 只写项名、不写编号。** `@1` / `@图1` / 「第一张图」全是错的。

     **槽位语义完全由「项名自身」承载**，不需要编号：`本镜首帧(上一镜末帧)=>@` 就是首帧、`本镜末帧=>@` 就是尾帧、`{角色}=>@` 锁该角色、`{bg}=>@` 是场景、`{角色}声音=>@` 是该角色音色源。项名已经说清楚了，再挂一个序号只是**多一处会和实际上传顺序对不上的地方**。

     > **这一条同时消解了仓库内一处长期存在的规则冲突**：rule 4d §B 与 rule 4e 的场景 / 物件主体 prompt 一直用裸 `=>@` 并明写「绝不代填槽位号」，而 2026-06-28 起的 shot prompt 却要求 inline `=>@N`——同一个 `参考:` 行在两条管线里写法相反。现统一为**裸 `=>@`**。

     旧的独立 `参考分配:` 行同样**已废除、不再写**（它的职责就是把 @图N 映射回角色/场景/首尾帧；既然不再有编号，这行也没有存在意义）。槽位上限仍在：≤9 图 / ≤3 视频 / ≤3 音频 / ≤12 文件，超限拆镜或减参考。
  ...
  机械校验归 `ai_videos__格式契约` K27：`参考:` 每项须以**裸 `=>@`** 收尾（`@` 后留空、不带任何编号或文字）；**不再写独立 `参考分配:` 行**；承接镜的首/尾帧由项名 `本镜首帧(上一镜末帧)=>@` / `本镜末帧=>@` 自明。

     > **生效范围（CLAUDE.md 通则）**：本条只约束**新增与改动的 shot**。旧剧里已经出过图的 shot 保持原样，**不做全仓回溯**——2026-09-06 曾按新规则 sweep 过 `wushen_juexing` / `rexue_gaoxiao` / `duikang_shangzeng` 共 175 处，经用户裁定「先不用修复旧的剧，只要保证新加的都按新的流程」后已全部回滚。需要统一旧剧时由用户显式发起。
...
- **Hard limit: 每个生成 prompt body（fenced ```text 内文）≤ 5000 字**（2026-09-06 由 2000 上调）（中文字符 + ASCII 一律按 1 计）。**适用所有 prompt 类型**——shot 视频 prompt / 场景背景图 prompt / 场景 walk-through 视频 prompt / 角色 turntable prompt 一律 ≤ 2000 字，**无 soft/hard 之分、无 cover-frame / 长 shot / 12.4-E 密度例外**（2026-06-18 amendment「全局 2000 字硬顶」收窄，覆盖旧 soft 2000 / hard 2500 + 12.4-E 例外）。超 2000 = blocker。
...
**Length cap: ≤ 5000 characters (2026-09-06, was 2000)** for the entire ```text``` prompt body (measured as raw character count of the code block contents, fence lines excluded). Seedance's prompt window has a soft limit around this size; overflow truncates silently and tail fields (`时长` / `负向`) get dropped first. Authors should trim 动作 / 台词 reaction lines first when over budget — never drop the 参考 line, the 镜头 cut list, the 时长 line, or the 负向 list (Seedance loses character likeness without negatives).
...
**字数上限（2026-06-18 收窄为全局 2000 硬顶；2026-09-06 上调为 5000）：** 所有 shot prompt body ≤ 5000 字（hard、blocker、无 soft/hard 之分）。增厚不是无限制堆字 — 12.4-E 的密度必须用经济语言压进 2000 字：短 shot (3-6s) 600-1200 字，长 / 多角色 shot 也一律 ≤ 2000 字（靠去重、合并同义清单、砍堆砌实现密度，而非堆字）。Padding for padding's sake = 反模式。
...
#### 15 「长相问题」要给图，不要堆形容词（per follow-up xianjian_yi_mv/079 — 2026-09-07）

- **15.1 一个属性反复写进 prompt 却反复不出现，说明它是长相问题，不是形容词问题——补一张参考图。**
  shot12 的「剑气把大地劈开」写了四轮（「深一米」「深两米半」「长十六米」「缝口一掌宽」
  「两侧如刀切」「无堆土」），成片始终只在地上留一点擦痕。**模型没见过「被斩开的地」长什么样，
  再多形容词也只会被归约成一条亮线。** 处置：按 rule 4d「图先行」为该属性单出一张
  **细节参考图**（text→image 自由生成、零参考图、无人无剑无光效，只交代那一样东西的长相与质感），
  落 `shots/shot{NN}/ref/{key}.md` + `{key}.png`，接进 shot 的 `参考:` 行，并在 `参考用法:`
  写清**分工**：参考图给长相与质感，3D 预演给位置、走向与时刻，两者不重叠。
  判据（三问）：① 这条要求描述的是**性状**（土怎么裂、布怎么烂、金属怎么锈）还是**位置/时刻**？
  ② 已经写过两轮以上还不出现？③ 它会在多个镜次复用？三个都是 → 出图。
- **15.2 深度靠「一亮一黑」读出来，不靠尺寸词。** 俯角镜里一道沟是否「深」，观众唯一的
  线索是**受光的那一侧内壁 vs 全黑的另一侧与沟底**的对比。prompt 与参考图都要把这条
  光学线索写死（哪一侧亮、哪一侧黑），只写「深两米半」等于没写。
- **15.3 状态改变要写「此后一直在」。** 一次性事件（劈开地面、砸出坑、烧穿墙）默认会被
  模型演成「亮一下就复原」。凡留下永久痕迹的拍，prompt 必须补一句
  **「此后每一拍都看得见，不淡出、不愈合、不被填平」**，并把反向词写进
  `裂缝淡出消失 / 裂缝愈合 / 地面完好无损`。
- **15.4 previz 灰模里用弦段拼出来的弧形几何，会在成片里被读成「一串独立的坑」。**
  shot12 的月牙沟原是 40 段直盒子拼弧、段间只重叠 6%：相邻两段的侧壁各自是一块平板，
  接缝处露出受光端面，整道沟渲成一排方坑。**弧形沟槽/墙/轨迹一律「段数加密 + 大重叠」
  （本例 110 段 × 1.9 倍重叠、壁厚减半）或改成一次扫掠**——previz 的几何噪声会被
  生成模型当成内容照抄。

*(Originated from follow-up xianjian_yi_mv/079 — 2026-09-07.)*

#### 16 物理与状态连续性契约（per follow-up hy1/001 — 2026-09-07）

生成模型不会自己守物理和连续性——它每一镜都是从零重演一次。以下三类只要不在 prompt 里
写死，就一定会漂。**三条都应该做成生成器字段（或 shot 模板的必填位），不靠作者每镜手写。**

- **16.1 掉落物必须写「真实重力」，否则默认会被演成慢放飘落。** 断枝、整片叶、木片、
  泥块、水珠、碎石——凡脱离支撑的东西，`动作:` 里必须显式写
  **「按真实重力加速下坠、越落越快、落地即停并溅起或弹一下、全程实时速度」**，
  负向块配 `慢动作 / slow motion / 子弹时间 / 落体滞空 / 悬浮下坠 / 羽毛般缓降 /
  低重力 / 掉落物匀速下落`。**只写「落下」＝把速度交给模型，它一定选慢**——慢放看起来
  「更有电影感」，是训练分布里的强先验。同理，**倒下的树／墙／人要写「绕支点转动、
  越倒越快、最后半秒明显加速」**，写「缓缓倒下」会得到零重量的假物理。
- **16.2 随身负重装备只属于「移动镜」，进入作业状态前必须在某一镜里被卸下。**
  背包 / 行囊 / 挎包 / 大件工具袋——**人干活的时候不背着它**（真人不会，画面上也丑）。
  落法是三段：① 赶路镜写「背在身上」；② 选一镜**镜内交代卸下的动作**（这同时是
  「他开始工作了」的免费叙事信号）；③ 此后每一劳作镜的 `走位:` 都带一句
  「不在身上（已卸在 X）」+ 负向 `背在身上 / 干活时背着背包`。**缺了第 ②
  步，观众会觉得包是凭空消失的；缺了第 ③ 步，模型会随机把它穿回去。**
  同一条推广到任何「本应被放下的手持物」（斗笠、外袍、伞、灯笼）。
- **16.3 人物造出来的结构件（绳结 / 捆绑 / 榫卯 / 补丁 / 封口）一旦成型就要锁成
  byte-identical 描述串，跨镜原样复现。** 实测失败模式是**「扎完绳子，绳结在下一镜莫名
  消失」**——模型只记得「一根木头搭在另一根上」，不记得节点。落法与角色锁定描述符同构：
  写一条含**圈数 / 绕向 / 结头位置 / 余头长度 / 在材料上留下的痕迹**的串，
  凡该节点入画的镜，`场景:` 行原样再贴一次；负向配 `绳结消失 / 捆绑处空着 /
  藤条散开 / 结头位置改变 / 捆法前后不一`。**成型之前的镜要反向声明**
  （「此时交点尚未捆绑，看不到任何绳结」），否则模型会提前把结画出来——
  这与 rule 15.3「状态改变要写『此后一直在』」是同一条原理的两个方向：
  **状态是有时间箭头的，前后两侧都要写死。**
- **16.4 重生成 shot 的 md 时，绝不整树 `rm -rf shots/` —— 渲染稿与 md 住在同一个目录。**
  实测损失（2026-09-07，hy1）：合并镜次导致镜号整体重编，为了清掉旧编号的 md
  用了 `rm -rf shots/` 再重建，把 `shots/shot{NN}/renders/` 下 **16 个已渲染的 mp4** 一并删掉；
  那批渲染尚未 `assets_sync push`，git 不跟踪媒体，Git Bash 的 `rm` 不进回收站——**不可恢复**。
  正确做法：**生成器直接覆写 md**（md 是可重生成的派生物），镜数缩减时只清理**多出来且不含
  任何媒体**的空目录，含媒体的目录只报警、留给人处理（`gen_shots_hy1.py`
  的 `prune_stale_shot_dirs()` 是参考实现）。
  **推论：镜号重编号是一次高危操作**——它让「第 N 镜」这个名字指向了别的内容，
  已渲染的素材会突然对不上号。重编号前先 `assets_sync push` 把渲染稿推上 R2，
  再动 md；能不重编号（在末尾追加、或保留空号）就不重编号。


- **16.5 人占画高这把尺子有个洞：两端同机位同取景时，比值会虚高成「合格」。**
  （per follow-up hy2/003 — 2026-09-12）
  铁律⓪的判据是「上镜落幅 ÷ 下镜起幅」的 `人占画高`（jingbie §2.0.2）。但这个量**只描述人**，
  不描述画框。于是出现一类假合格：**两端是同一个机位、同一个取景，只是其中一端人走出了画**——
  比值算出来 3.0、5.0，判 ✅，而观众看到的是**同一个画面在跳**。
  实测两例（hy2 初稿，2026-09-12）：
  * `shot08 落幅 = N1 对比帧 #3`（人 0.25）→ `shot09 起幅 = N1 雾反转`（人不入画）。
    比值 ✅，两端却是**逐帧同框**的对比帧机位，只有光在变。
  * `shot10 落幅 = N1 外景验烟`（人不入画）→ `shot11 起幅 = N1 外景暴雨`。
    同机位、同 plate、同一个夜——全片最糟的一处接缝，而数字判 ✅。
  **补一条硬约束**：`景别档:` 除了两端的人占画高，还要记**两端的机位标签**；
  **上镜落幅的机位标签与下镜起幅的机位标签不许相同**，相同即 ❌，不看比值。
  这条对「三次对比帧共用一个 N1 机位」这类刻意的同机位设计**尤其重要**——
  同机位是那个设计的价值所在，但它必须出现在**镜内**或**隔着别的镜**，
  绝不能出现在**接缝的两端**。
  **实操上的破法永远是同一招**：给后镜换一个**便宜的开场取景**——一片叶子的微距、
  门板上的一道水流、封缝里透出的一线光。五秒，不花额外时长，不需要新的参考图
  （微距骑在已有 plate 上），而且往往比原来的大全景开场更好看。

- **16.6 切口是排镜的第一约束，时长排第二；合并与拆分是达成切口的正当手段。**
  （per follow-up hy2/003 — 2026-09-12，用户原话：*「请确保每个 shot 之间的链接是恰到好处，
  不需要我做特殊处理……这点应该是你设计 shot 的 top priority，时长什么的都好说，
  只要是 4~30 秒之间，当然尽量不要太短」*）
  次序是死的：**先排出一串接缝全部可辨的镜，再看时长落在哪里**——不是先切出「每条 22 秒」
  再去补救接缝。当一对镜的切口做不出来时，允许且应当**把它们并成一条**（镜内切镜，
  一条 clip 一床音乐）或**把一条拆成两条**（拆出一个能当开场的紧取景）。
  时长的地位随之下调：**4–30s 之间都合法**，「尽量不要太短」是偏好而不是闸门——
  一条 5 秒的紧镜若是让前后两个接缝都变强的那块砖，它就该留着。
  这条**改写**了「4–6s 碎片镜要合并」的默认（CLAUDE.md 时长条）：那条针对的是
  *为碎而碎*，不针对*为切口而紧*。

- **16.7 切口审计要做成生成器的构建闸门，不要做成事后审查项。**
  （per follow-up hy2/003 — 2026-09-12）
  `景别档:` 是人写的，写完没人会去逐对做除法；K31 是出片前才跑的。
  正确的位置是**生成器里**：每镜带 `jb=(起幅值, 起幅名, 落幅值, 落幅名)` 与
  `jbcam=(起幅机位, 落幅机位)`，build 时逐对算完，**任一接缝不合格直接 `raise`——
  不合格的分镜根本生成不出来**，并把整张审计表写进 `shotlist.md` 供人一眼扫完。
  参考实现：`tools/gen_shots_hy2.py` 的 `jb_verdict()` / `jb_audit()`。
  一般化：**凡是「可以算的契约」，都该往左移到生成的那一刻**，而不是留给下游 reviewer。

- **16.8 描述「历史痕迹」的词不能用「正在发生」的说法——模型读不出过去时。**
  （per follow-up hy2/004 — 2026-09-12）
  实测：hy2 shot04 是白天挖土的镜，`情节:`/`动作:` 里写了
  「挖出一层旧炭与几块**被烧得发红发脆**的石头」——本意是「很多年前一场火留下的烧痕」，
  出来的画面**坑底在冒火星**，而且他正徒手去拨。
  汉语里「烧红的石头」「烧焦的木头」「燃过的灰」既可以是**痕迹**也可以是**状态**，
  生成模型一律按**状态**画，因为状态才有画面。
  **写法**：凡是描述火/热/湿/新鲜度留下的**痕迹**，必须
  ① 用冷词写材质（「暗红褐色的烧斑与细裂纹」而不是「烧得发红」）；
  ② **正面写死它的当前状态**（「早已冷透多年，不发光、不发烫、不冒烟」）；
  ③ **让人物的动作替你作证**（「他**徒手**把它们拨开」——徒手能碰，就说明是凉的）；
  ④ 负向里挂上该状态的反面词（火星 / 余烬 / 发光的炭 / 红热的石头 / 冒烟）。
  同一条推广到其他「痕迹类」描述：**积水的坑 ≠ 正在下雨**、**烧黑的锅底 ≠ 锅在火上**、
  **踩过的泥 ≠ 有人正在走**。痕迹要写成名词化的材质，不要写成动词。

- **16.9 风格串里凡是描述「某个具体光源」的措辞，必须按镜条件化。**
  （per follow-up hy2/004 — 2026-09-12）
  `渲染样式:` 是所有镜共用的一条串，很容易被写进只属于**部分镜**的光线设定。
  hy1 与 hy2 的风格串都带着「**冷白灰环境与暖橙火光强色温对比**」——
  这是给夜里有火的那几镜定的调，却被复制进了**全部 12 镜**，
  于是白天挖土、林中走路的镜也在跟模型说「这里应该有暖橙火光」。
  hy2 实测受污染 6 镜、hy1 8 镜；shot04 的火星是它与 16.8 共同促成的。
  **写法**：风格串拆成 `STYLE_BASE`（真正全片共用的：影像质感、颗粒、滤镜、字幕禁令）
  ＋ 按条件拼接的光源分句。没有该光源的镜，不但不拼那句，还要**反向声明**
  （「本镜画面里没有任何火、也没有任何暖色光源」）并挂对应负向组。
  **一般化：任何「全片共用串」在写进去之前先问一句「这句话对每一镜都成立吗」**——
  不成立的就不属于共用串。锁定描述符（人物/道具）能共用是因为它们真的全片不变；
  光源、天气、时段、建造状态都不是。


- **16.10 镜内切镜时，被改造物的状态必须逐段写死，且只能前进。**
  （per follow-up hy2/006 — 2026-09-12，用户实测：*「近景的时候通道已经挖好了，
  但是切换远景，还在继续挖通通道」*）
  跨镜有 `door` / `bed` 这类状态字段锁着（rule 16.3），**镜内没有**——
  而一条 26 秒、五段切镜的镜里，被造的东西可能变三四次。
  prompt 写满了「他在做什么」，却没写「此刻它是什么样」；
  视频模型逐时刻渲染，**不写死每一刻的状态，状态就会自己漂**，
  一次镜内切镜就是一次重新构图，模型没有义务记住上一段挖到哪儿了。
  **写法**：`分镜:` 之外再出一行 `镜内状态:`，**段数与 `分镜:` 一一对应**，
  每段写「此刻它是什么样」，并声明**后一段必须包含前一段已完成的全部**。
  负向配 `已完成的部分又变回未完成 / 建造进度倒退 / 成品与半成品交替出现`。
  **副作用比检查本身更值钱**：补账本这个动作会逼作者过一遍时间线。
  hy2 shot05 真正的毛病就是在补账本时暴露的——第三段是「剖视成品」，第四段却还在挖，
  **脚本自己就前后矛盾**，不只是模型漂。修法是把剖视段明确限定为
  「只看已经挖好的那一段，画面尽头沟戛然而止、前方是没动过的原土」。

- **16.11 同一段里既有「肢体探入」又有「身体位移」，必须写死先后次序。**
  （per follow-up hy2/006 — 2026-09-12，用户实测：*「人都从后面往外走了，手还保持从洞里
  伸出来的状态，而且人站的位置显得手太长了」*）
  模型会把同一段里的两个动作**叠在同一时刻**：手还在洞里，身体已经走开 → 橡皮手。
  三条：
  ① **拆成先后**——「**先**把手从洞口收回来，**然后才**撑着膝盖站起身、退开半步」，
     并明写「收手与起身是先后发生的两件事，绝不同时发生」。
  ② **重复动作要写「每次都退出来」**——「每塞完一块，手都完全退出洞口再去取下一块」。
  ③ **探入类动作必须写明身体与开口的几何关系**——「半跪在出烟口正前方、身体与洞口
     相距约一臂」「肩、上臂、前臂到手是一条完整可见的连线」。
     不写距离，模型就按构图需要拉长手臂；写「只看得见他作业的手臂」这种，
     等于邀请模型画一只脱离身体的手。
  负向配 `手臂过长 / 橡皮手 / 脱离身体的手 / 人已经走开但手仍留在洞里 / 四肢比例失真`。

- **16.12 这类「镜内自洽」的契约要做成生成器闸门 + 可执行巡检，不要只写进文档。**
  实现：`tools/shot_logic.py`（L1 建造镜必须有 `镜内状态:` 账本、段数对齐 ＝ blocker；
  L2 同段内探入＋位移而无次序词 ＝ warning + 附原句）。
  `shot_logic.gate(emitted, legacy={...})` 在 build 时拦，`python tools/shot_logic.py [范围]` 巡检。
  **`legacy` 的语义要分清楚**：
  * 对**已出过渲染**的旧镜，它是**技术债豁免**（只减不增，重渲一镜就还一镜的债）；
  * 对**还没出过渲染**的新片，它是 **`LEDGER_TODO` 待办**，不是豁免——
    出片前必须补完、补一镜删一个号。**把两者叫成同一个名字会让待办悄悄变成豁免。**

- **16.13 后面要用的工具，第一次取出之前，每一个入画的携带镜里都要看得见。**
  （per follow-up hy4/002 — 2026-09-13，用户实测：*「背包里看不出有铁锹，之后又有了，感觉很突兀」*）
  只写「背着背包」、到开工那镜才写「从包侧抽出雪铲」，模型就会在前几镜画一个光秃秃的包，开工时工具凭空出现。
  **写法**：把**挂载方式**锁成一条共用串（挂在包的哪一面、哪一截露在外面），所有携带镜原样复制、
  同时上传该工具的锚点图；取出那一镜写「**解下扣着的**」，不写「抽出」。
  **挂在镜头看得见的位置**：人朝镜头走来时，包背面的东西看不见，要让一截（握把、柄头）从肩后露出来。

- **16.14 场景锚点图的尺寸，按「之后要在里面做什么」倒推，并给人体尺度参照。**
  （per follow-up hy4/002 — 2026-09-13，用户实测：*「树坑太小了，根本做不出洞穴来，之后人跳进去后又突然变大了」*）
  「直径约三米、低约一米二」写在纸上合理，模型却画成浅碗；后面的镜要在坑里下挖、横掏出能躺人的洞，
  模型只好当场把坑放大——**同一个地点前后大小对不上**。
  ① **尺寸按最终施工状态倒推**，宁大勿小；② **光写米数模型不认，要拿人和已有的东西当尺子**
  （「比一个成年人还深」「直径是树干的五倍」）；③ **机位要能看出这个尺寸**（深坑别贴地平视，抬到一人高俯拍）；
  ④ 验收清单把尺寸列为第一翻车点，负向挂 `浅浅的小坑 / 碗状浅凹`。
  ⑤ **模型会系统性地往小里画，按真实物理写的尺寸出图常常只剩一半**——锚点要按「一眼看上去夸张」来写
  （hy4/003：bg3 从一米八再加到三米六，用户才认可深度）。
  **宽度同理，而且别用窄比喻**：写「像一口井」模型就画窄坑（hy4/004）——宽度要写成**画面占比**
  （「坑口两侧坑沿出画、画面下半部分几乎全是坑」）＋宽深比，必要时换广角把宽度装进画面。
  hy4 bg3 直径最终从 3m 连加三轮到 32m（005）——**尺寸以用户看图认可为准，不以物理合理为准**，
  物理上说不通时补一句成因（风绕孤树打旋掏雪）让画面自洽即可。
  **米数和倍数都压不住时，改写「它能装下什么」**（hy4/006）：「坑底能盖下一间小屋、屋顶不冒出坑沿、
  人站坑底头顶离坑沿还差两个多人高」——用功能容量给尺度，比数字管用；**参照物本身不入画时必须同时反向声明**
  （「画面里没有任何屋子」+ 负向 `坑里有屋子`），否则模型会把参照物画出来。
  **改了锚点的尺寸或机位，挂它的下游状态图（同机位对比帧、施工中、建成）一律重出**，否则对比帧对不上。

- **16.15 需要「环境有反应」时，让人用手去碰，别让身体去碰。**
  （per follow-up hy4/002 — 2026-09-13，用户实测：*「撞树那一下，动作很脑残……用手拍拍都比身体斜着撞上去好」*）
  「从低枝下侧身穿过、肩膀蹭到枝条」会被演成人斜着身子撞上树。改写成「抬手把枝条拨开、拍掉枝上的雪，
  正着身子走过去，**身体不碰任何树枝和树干**」，负向配 `身体撞树 / 侧身撞上树 / 身体斜着挤过树枝`。
  「擦过」「蹭到」这类暗示接触的词，换成「从帽顶上方一拃处经过、不碰到她」。


*(Originated from follow-up hy1/001 — 2026-09-07：用户三条实拍反馈「人干活的时候不要背着包」「砍下树枝树叶落下的速度不能太慢，不然重力感没了」「扎绳子前后要一致，有的时候扎完绳子绳子结莫名消失了」。)*

#### 16 `publish.md` ＝ 每部片一张「四站发布页」（per follow-up hy1/006 · hy2/001 — 2026-09-12）

用户定调：*「每一部剧我都要有一个单独的 page，放我上传 YouTube／抖音／TikTok／小红书所需要的
title、description、tag、metadata，一个 page 分成每站一个 section，内容生成好我可以直接 copy paste。」*

- **16.1 落点不变、结构固定。** 仍是 `{片}/5_6_分镜与prompt/publish.md`（stage-6 交付件，不另建目录）。
  固定七节，顺序不可换：**① 母版与通用设置 → ② 封面帧 → ③ YouTube → ④ TikTok → ⑤ 抖音 → ⑥ 小红书
  → ⑦ 平台对照表 + 发布前检查清单**。三、四、五、六节各自**自包含**——用户只看一节就能把一个站传完，
  不必跨节拼。
- **16.2 语言按站分，不混。** **YouTube 与 TikTok 一律英文**（标题、描述/caption、标签全英文，
  不留中文括注）；**抖音与小红书一律中文**。同一部片的四份文案是**四次独立创作**，不是互相翻译——
  英文站讲「what he built / what you'll hear」，中文站讲「他抢在暴雨前把屋顶盖好了」。
- **16.3 每一块可复制的内容都进 ```text 围栏**，围栏里**只有要粘贴的正文**，不许夹说明、不许夹
  markdown 强调符。说明写在围栏外。**标题、描述、标签各自一个围栏**，别合成一块——平台的输入框是分开的。
- **16.4 每节必须带该站的字段上限与必填项**（写在围栏外的小表里），并在生成时自查：
  | 站 | 硬限 |
  |---|---|
  | YouTube | 标题 ≤100 字符 ／ 说明 ≤5000 ／ 标签总长 ≤500 字符 ／ 章节 ≥3 条、首条 `00:00`、每条 ≥10s |
  | TikTok | caption ≤2200 字符（含 hashtag）／ 建议 3–5 个 hashtag |
  | 抖音 | 标题建议 ≤55 字 ／ 话题 3–5 个 |
  | 小红书 | 标题 **≤20 字**（超出直接截断）／ 正文 ≤1000 字 ／ 话题 ≤10 个 ／ 封面建议 3:4 竖版 |
- **16.5 非文案的 metadata 也要给全**，每节一张小表：类别 / 语言 / 是否儿童内容 / 播放列表或合集 /
  许可 / 封面用哪一帧 / 画幅与时长 / 定位与话题。**用户不必再去别处查任何一项。**
- **16.6 画幅与竖版的取舍要写明。** 横屏母版投到竖屏为主的站（TikTok/抖音/小红书）时，
  必须在该节写清是「横屏直投」还是「竖裁」，竖裁的要给**裁法**（中心线偏移、哪些元素会被切掉）。
  没出竖版就明说没出，别留空让用户自己猜。
- **16.7 一部片改了任何 shot，`publish.md` 的章节时间轴与精华版剪法即 stale**，与成片同批复核。
...
- **18.1 画类参考图会把画风带进画面，文字否定挡不住。** 喂《清明上河图》原本局部定形制，prompt 里写「绢本的色调、线描笔触和平面透视一律不照搬」——没用，模型对参考图的**风格吸收**远强于对文字否定的服从，出来的就是画的亲戚。**修法**：画类史料降级为**作者对账**（`ref/` 照存、`refs.md` 照索引、形制词对账表照引用），**不进上传窗口**；上传位只给世界锚点、地点锚点与**照片类参考**（现存实物 / 遗构 / 复原实拍）。这一条不削弱史料地位——史料证明形制，由作者读完写成文字，**文字进 prompt，图不进槽位**。
  ...
  **18.1b 适用边界（2026-09-18 澄清，shikong_lvxing/sk3）：18.1 禁的是「拿画当风格参考去生成新构图」，不是「拿画做保留原构图的 img2img」。**
  两者是相反的操作。18.1 的失败模式是**构图由模型新编、风格从画里吸**——于是画风盖过一切，文字否定挡不住。
  而 img2img「保持原图完整性」是**构图由画锁死、材质由模型补**——它要的恰恰是不要模型自由发挥。
  后者要到 2025 年底模型具备「keep the integrity of the image intact」才成立（Chloe VS History 创作者一手访谈），
  在那之前上传参考图只会被当风格参考，所以 18.1 成文时这条路还不存在。
  **分流规则（史料驱动项目照此走）**：
  - **地点 / 建筑 / 街景 / 全景 → 允许画类史料 img2img**，前提是保留原构图、不要新编。
  - **人物、面孔 → 一律不进上传窗口，仍走 18.1 的文字锁定串。** 版画 img2img 会把**铜版线条刻进脸**——
    这正是 18.1 最致命的那个坑，在人物上一步都不能让。
  - **底本分两类、工艺不同**：油画类可一步出照片级；**铜版线条画必须两步走**
    （先低 denoise 压掉线条，再二次提 denoise 补材质），否则第一批图会全是「线描感照片」。
  - **年代状态要先对账**：同一地点的早年画作可能画的是**另一个状态**
    （例：Visscher 1616 / de Jongh 1632 画的伦敦桥北段是满的，而 1666 年那段自 1633 年火后一直空着），
    img2img 会忠实地把错误状态一起保留下来——建卡时写死或切片避开。
...
### 22. 人物卡两条铁律：人种锚 + 4 秒建立视频（2026-09-19，用户指出「本尼迪塔斯为什么生成的是一张亚洲脸」）

#### 22.1 凡具名人物，立绘 prompt 的**开篇第一句**必须写死人种 / 地域

**中文相貌词库自带东亚默认值。** 「脸盘方正」「颧骨平」「眼睛不大」「虹膜浅褐」「暖麦色皮肤」
——每一个单独看都只是在描述一张普通的脸，叠在一起就是一张东亚的脸；
而 **「八字胡」是汉语专有词**，图像模型对它的先验几乎锁死在东亚男性。
写卡的人是在把英文 wiki 的「面相朴实近似农夫」翻成中文，**翻译动作本身就把族裔换掉了**。

实测（sk2 暴风城，16 张卡）：**14 张没有人种锚**，出图后逐张验，**11 张里 3 张塌成东亚脸**。
塌的是 c23（乌黑直发 + 齐额刘海）、c33（没有发色特征的平民女性）、c39（两个孩子）；
没塌的 c24 / c25 / c26 / c34 **全是红发姜发——是发色替锚点干了活，属于侥幸，不是写对了**。

规则：
- **开篇第一句**就写人种（`一名**西欧白人**成年男性…`）。模型对 prompt 开头权重最高，
  这与「侏儒四指」那次的结论是同一条——**与模型强先验相反的设定，只能正向早说，负向堵不住**。
- 面部段落**再写一次骨相**，用可验的解剖特征而不是族裔形容词：
  **眼窝深 + 眉弓自眉心两侧隆起 + 鼻梁自眉心处就抬起 + 鼻背直 + 鼻头窄 + 双眼皮褶深、无内眦赘皮**。
  这几条比「白人」三个字有效得多。
- **发色与族裔冲突时要显式拆开**：乌黑直发的欧洲人要写明「**黑发在这里是发色，不是族裔；
  五官必须是欧洲人的五官**」，否则黑发这一个信号就足以把整张脸拉走。
- **禁用词**（一律改写）：`八字胡`→`修剪过的髭须`、`脸盘`→`面部`、`颧骨平`→按实际骨相写、
  `暖麦色 / 浅麦褐`→`晒红的白皙`、`布鞋`→`软底尖头便鞋`、`宽袖喇叭口`→按西式形制写。
- 负向兜底（**只是兜底，不能当主力**）：
  `东亚面孔, 亚洲人, 东亚五官, 单眼皮, 内眦赘皮, 扁平鼻梁, 八字胡, 东亚服饰, 和服, 布鞋`。
- **非人类种族同样要锚**——矮人 / 侏儒 / 精灵用「面部骨相走西方奇幻一路」+ 同一组解剖特征；
  紫肤的暗夜精灵不写人种，但骨相照锚，否则同一套中文相貌词照样滑过去。

**验收只认眼睛**：出图后必须逐张看脸。张数多时做**头部拼版**（各图裁头部、拼成一张）一次看完，
比逐张读省得多，也更容易看出「这一批里哪几张不对」。

#### 22.2 每张人物卡必须有 4 秒建立视频（turntable）prompt —— 这是标准流程，**只有 character 有**

立绘定**脸**，turntable 定**体型 + 服装 + 随身物 + 站姿 + 声音**；两者一起喂 Seedance 人物 entity。
体例唯一出处：`_series/characters/c4_艾拉/c4_艾拉.md` 的 `## turntable 说明`（rule 12.5）——
16:9 · 4 s · 5-phase locked-framing single-take · 抽帧 front 0.5 s / side 2.0 s / back 3.5 s ·
路由键 `c{N}-2` · `参考: c{N}-1.png(脸的唯一标准)=>@`。
**有台词的角色**带声样段（锁定 `voice_id`，0–2 s 说那一句逐字原文，2 s 后静音）；
**零台词的角色**明写不出声并挂 `人声, 说话, 台词, 口型, 背景音乐` 负向。
场景卡与物件卡**没有** turntable——它们的多视图走 rule 4d / 4e 的正交三视图，两件事不要混。

**同构的 turntable 走生成器**（rule 4i ①）：一部片十几个角色的 turntable 只有人物那几段不同，
手写一遍就是手写十几份将来各自漂移的副本。参考实现 `tools/gen_turntables_sk2.py`。


### 23. `参考:` 行的完整构成，以及为什么它必须是**机检**的（2026-09-19，用户指出「shot1 除了一个 character 参考里没有任何的 reference 了」）

shot 的 `参考:` 行不是「挂个脸就行」，它是**这一镜要上传的全部素材清单**。完整构成与顺序：

| 次序 | 项 | handle 形态 | 备注 |
|---|---|---|---|
| 1 | **previz 白模动画** | `shot{NN}_previz.mp4(白模动画·运动与几何参考，不取长相)=>@` | **每镜必有**（rule 4h ①覆盖率 100%）；它是运镜路径、几何关系与**动作时刻表**的唯一出处 |
| 2 | 首帧承接 | `本镜首帧(上一镜末帧)=>@` | 只有 `衔接: 承接…` 的镜有 |
| 3 | **场景主体** | `bg{N}-{M}(场景主体·{视图名})=>@` | **每镜必有至少一个**；一镜跨两个地点就挂两个 |
| 4 | 人物 entity | `c{N}_{名}(Seedance 人物 entity·{装束态})=>@` 或 `c{N}-1(…)=>@` | 用户自建 entity 的走**目录名**，有立绘图的走**图片路由键** |
| 5 | 物件锚点 | `p{N}-1({物件名}锚点)=>@` | 这一镜里真正入画的重要复用物件 |
| 6 | 声样 | `{角色}声音(…·voice_id {id})=>@` | 有台词的镜才有 |

三条硬规矩：

1. **`参考:` 行必须机检，不能靠人写对。** CLAUDE.md 已有「全部 handle ⊆ 全部产物路径」这半句，
   但只写在 shot prompt 的条目里、没有做成闸门，于是 sk2 的生成器把场景写成
   `bg1_英雄谷锚点`（盘上根本没有这个名字）、还把 `bg0` 整个排除在外、previz 与物件**一个都没挂**，
   44 镜全错也没人拦到出片前。闸门要检**四件事**：①行非空 ②每个路由键在盘上真有文件
   ③previz 挂了 ④至少挂了一个场景主体。参考实现 `tools/gen_shots_sk2.py::gate_refs`。
2. **人不入画的镜，绝不许挂人物 entity。** 航拍空镜、纯环境镜的 `走位:` 写着「画面里没有人」，
   却挂着主角的脸——那是在要求模型往一个本来没有人的画面里加一个人。
   这条同样可机检：`走位:` 含「画面里没有人 / 不入画」⇔ 不得出现人物 entity handle，**双向都检**。
3. **闸门要左移进生成器**（同 rule 16.7 ③）：分镜 md 是生成器写的，参考行就该在 build 时验，
   不合格直接 raise、分镜根本生成不出来——而不是留给出片前的 reviewer 或用户去发现。


### 24. previz 要上传给生成模型，就得满足生成模型的下限（2026-09-19，用户实测 Seedance 报错）

**Seedance 拒收像素数低于 `409600`（＝640×640）的参考视频。** 注意这是 **宽 × 高 的乘积**，
不是边长——720×404 看着「有 720」，实际只有 290880，**整批 44 条 previz 全部不可用**。
previz 一律渲 **1280×720（921600）** 起步：过线、精确 16:9、留足余量。

普遍教训：**`--res` 这类参数长期被当成「渲多快」的旋钮，但它下游连着一条外部服务的硬约束。**
凡是产物要喂给外部模型的，**下限必须做成生成器里的闸门**（同 rule 16.7 ③「审计左移进生成器」）——
不达标直接 raise、**根本渲不出来**，而不是渲出一批要等到上传那一刻才发现传不上去的废片。
参考实现 `tools/previz_sk2.py::MIN_PIXELS`。


### 25. 「半写实」形容的是世界的设计，不是画面的渲染（2026-09-19，用户：「画面太像劣质游戏 CG，我们要真实感」）

§18 是 sk1「太像动画，全部重来」的产物，已经把实拍口径写全了。**sk2 照样又犯了一遍**，
因为 sk2 的 `style_guide.md` 是按「半写实 · 保形制保配色」写的，而写卡的人
**把两个互相独立的轴混成了一个**：

| 轴 | 内容 | 该不该降 |
|---|---|---|
| 一 · 世界的设计 | 形制、比例、配色（蓝顶白石是暴风城） | **保**——这才是「半写实」说的那件事 |
| 二 · 画面的真实度 | 光学、材质、大气、暗部、做旧 | **拉满**——它与轴一无关，却被一起降成了「半写实」 |

真人实拍一座按暴风城形制搭出来的实景，照样是完全写实的照片。**改编自游戏的项目尤其危险**：
`渲染样式:` 里写「忠于原作」时，原作是一款游戏——正面在要求游戏美术，
负面挂再多 `游戏引擎渲染` 也没用。**正面永远赢负面**（这是同一失效模式的第四次：
侏儒五指 → 东亚脸 → 游戏 CG）。

三条可直接检的病灶词，出现即错：
1. **`半写实` 出现在 `渲染样式:` 里** —— 它属于 `style_guide` 的世界设计一节，不属于渲染串。
2. **`材质与做旧:` 里写「干净 / 整齐 / 无风化」** —— 那是游戏资产的样子。
   设定上是新城也一样：再新的城，天天有人走就不可能干净整齐。
3. **`饱和的{颜色}`** —— 「饱和」本身就是反写实词。配色要保就写色名，别写饱和度。

**位置也是效力**：`渲染样式:` 原先排在 prompt 第 12 行，前面 11 行全在建立「这是个游戏世界」的先验。
**与先验对抗的约束必须放最前面**——把媒介声明（「这是实拍剧照，不是渲染图」）插在内容层之前。
这与 §22.1 的人种锚是同一条工程结论。

**密度比长度重要**（§18.2 的推论）。实测三版迭代：泛泛的「有磨损、有大气」模型收下就完事；
真正起作用的是 sk1 那种写法——**光比写死档数**（「受光面与阴影差三档，阴影里只留很少细节」）、
**景深点名深度链**（「最近的最实，每往深一层软一档，最远一层只剩偏蓝灰的轮廓」）、
**做旧按材质分条且每条带反例**（「石：…**不是切割整齐的新石材**；木：…**没有刨光的新板**」）。

**look 层要做成生成器**（rule 4i ①）：一部片十几张卡共用同一套 look，手改必漂。
参考实现 `tools/gen_scene_prompts_sk2.py`（场景）与 `gen_shots_sk2.py::STYLE_BASE`（分镜），
两者逐字同源。**生成器要能覆盖旧 look，不能只会「已存在就跳过」**——
幂等不等于不可更新，否则口径改了却被「已是最新」吞掉。

**出图模型的现实**（2026-09-19 实测，同一条 prompt）：`gpt-image-2` ＞ `gemini-3-pro-image`
（后者直接给 3D 渲染件、还不服从机位）；`bytedance-seedream-5-pro` 在本账号
**被默认禁用**（`model_access_denied`，需向 ElevenLabs 申请 ByteDance 模型权限），未能实测。
另记两条 API 事实：**seedream 的 prompt 硬上限是 2000 字符**（rule 4d ⑤ 那个「硬顶 2000」原来有出处），
**`quality` 参数只有 `gpt-image-*` 收**，传给别家直接 422。


### 26. 做 IP 衍生片时，先去读权利人**真正存在**的那几份文件（2026-09-20，sk2 #110 裁定）

**最容易犯的错是引用一份不存在的政策。** sk2 的 #110 原文写着「非商业同人在
**Blizzard fan content policy** 下通常可行」——实抓该公司 legal 全目录 16 条，
**没有这份文件**。与同人相关的只有三份：Video Policy（管**带游戏素材**的视频）、
Legal FAQ、Fan Art 投稿条款。而 Legal FAQ 里恰好有一问：
「Can I write novels, screenplays … **or other adaptations** based on your games?」→ **"No."**
一部有剧本有角色有台词的片子，在权利人自己的口径里更接近**被明确拒绝的 adaptation**。

**所以定性要诚实**：多数游戏 IP 的非商业同人是**「权利人明确不许可、但长期不执行」**，
不是「通常可行」。把它写成后者，后面所有决策都建在沙子上。

#### 26.1 中国法下，**回避作品名反而更危险**

直觉是「不点名 ＝ 更安全」，**反了**。《著作权法》§24(2) 的「为介绍、评论某一作品……适当引用」
抗辩，**前提句就要求「指明作者姓名或者名称、作品名称」**。不点名 ＝ 主动放弃这条抗辩。
所以「点名 / 不点名」是个**伪二选一**；真正的分界线是：

| | 可以 | 不可以 |
|---|---|---|
| **描述题材**（标题、描述、标签） | 用作品名与地名——这是**指称性使用**，也是合理使用的前提要件 | 任何暗示关联 / 背书 / 赞助的措辞 |
| **充当来源标识**（账号名、频道名、栏目名、域名） | — | **绝不含作品名**（多数权利人的 FAQ 对域名直接答 No，理由是「顾客可能以为有关联」） |

**权利人名本身不做标签**（如 `Blizzard` / `暴雪`）：涨量收益接近零，被判「关联」的概率最高。

#### 26.2 免责声明要同时干三件事，缺一条就失效

① **否认关联**（打掉商标层的混淆主张）② **指明作者与作品名**（满足中国法 §24 的前提要件）
③ **写明零素材、零原声**（让权利人的 Video Policy 与平台的 "video game content" 口径
在**事实上不适用**）。**措辞不要「精简优化」**，三件事缺一不可。
放**片头字卡 + 各站简介首屏第一段**——折叠在「展开更多」里等于没有。

#### 26.3 变现是唯一会把风险从「中」推到「高」的动作

非商业往往是这类片子**仅剩的缓冲**。注意两个陷阱：
① 权利人政策里的「平台例外」清单可能**十余年未更新**（sk2 实测：清单里四个站中有四个
已在 2013–2015 年关停），短视频站通常完全不在内；
② **已入变现计划的老频道，上传即默认带广告**，必须逐条关掉。

#### 26.4 真正的最大风险常常不是法律，是**拿存量资产去押一部 pilot**

sk2 的风险表里，「被起诉」是**低**，「老频道的变现资格被连累」是**中高**。
一次 strike 对已变现老频道的代价，远大于一部 pilot 的全部收益。
**这个痛点不需要改名来解决——换个新频道就隔离掉了，且不损失一个字的内容。**
改名（去 IP 化）通常是**既毁片又不解决问题**：具体的建筑形制仍是权利人的表达，
版权风险不降，而中国站的抗辩反而变弱。

#### 26.5 调研纪律：摘要会骗人，正文不会

本轮实测到一次：某抓取工具对一份官方 circular 的摘要声称它讲了「style 不受保护」，
提取 PDF 正文后确认**全文根本没有 style 一词**。**凡要引用的结论，必须落到正文原句**；
查不到就写「未查到权威来源」，不要用摘要顶上。

### 27. 三条工程事实（2026-09-20 实测）

1. **`hide_render` 挡得住出图，挡不住射线。** Blender 里把辅助对象设为不渲染，
   它**仍然参与光线投射**——会静默毒化任何基于 raycast 的逻辑（如 previz 的避障选机位）。
   实测后果：42 条 previz 全部废掉（相机卡在几何里或对着屋顶面），而每一条看上去都「渲成功了」。
   **凡射线 / 物理 / 碰撞类逻辑，可见性开关一律不作数**——要么把对象挪出场景，要么显式排除。
2. **BGM 不必卡在本地权重上。** `stable-audio-open-1.0` 要 torch + HuggingFace **gated** 权重
   （必须本人网页接受许可，自动化绕不过）。**ElevenLabs 有 `POST /v1/music`**，
   `force_instrumental=true` 正好对应「无词」纪律，`music_length_ms` 取 3 s–600 s。
   **坑**：`seed` **不能与 `prompt` 同用**（422），它只服务于 `composition_plan`——
   所以走简单 prompt **没有可复现的种子**。参考实现 `tools/gen_bgm_sk2.py`。
3. **异步出片要有「提交账本」，而且要能事后重建。** 视频队列可能排到十万位
   （即梦实测 `queue_idx 99384 / queue_length 332129`），**轮询超时 ≠ 没生成**——
   积分照扣、服务端照出，缺的只是「谁去取」。所以：① 提交即记 `submit_id → 资产键`；
   ② 再加一条**按 prompt 回显反查**的重建路径，这样即便当初没记账也能认回来。
   参考实现 `tools/gen_turntable_videos_sk2.py` 的 `ledger()` / `rebuild_ledger()` / `--harvest`。
4. **一个名字像「局部属性」的量，值却是「全局量」——这是最难看见的一类 bug**
   （2026-09-21，sk2 那 42 条地面 previz 的**第二个**独立根因）。
   `street_heading(bg)` 听上去是「这条街的走向」，实际返回的是
   **「上一站锚点 → 本站锚点」的城市级直线方位角**——横穿整座城，与该锚点脚下那条街无关。
   于是「沿 heading 后退 12 m」＝朝任意方向平移 12 m，街半宽只有 3–6 m，必然进旁边的房子。
   三条可迁移的教训：
   ① **找到一个说得通的根因 ≠ 找全了根因**——本条与上面第 1 条是同一批镜的**两个**独立成因，
   第 1 条修完症状没消失，才逼出第 2 条。症状没消失就继续查，别信「已经修过了」。
   ② **先怀疑名字**：`xxx_heading` / `local_*` / `*_width` 这类名字自带「我是局部量」的暗示，
   读代码的人（包括写的人）会跳过它的实现。**空间量必须从几何量出来**，
   一条街的走向只有几何知道——在锚点每 10° 打一条水平射线，
   「正反两头都通」的那条轴就是街（参考实现 `tools/previz_sk2.py` 的 `street_axis()`）。
   ③ **假设出来的距离要换成量出来的距离**：原代码把景别换算写成 `dist = SAFE_BACK + 16`，
   可 `find_clear_cam()` 有权把相机挪走，一挪景别就对不上。改成 `math.dist(cam, tgt)`。
   **配套**：静默兜底（`z += 26` 升到屋顶之上）必须**打印警告**——
   这 42 条之所以能一路错到成品，正是因为兜底不吭声，渲出来是屋顶视角、日志里一片祥和。
5. **对账用的那把尺，自己也要被对一次——「永远通过的检查」等于没有检查**
   （2026-09-21，同一次 previz 里连撞两回）。
   ① previz 打印「人占画高 ≈ x（shot md 写的是 0.19）」看着像在对账，可坐标与那个 0.19
   **都是写死的 S03 值**：换任何一镜，它报的都是一个与本镜无关的数，而且永远「对得上」。
   ② 修好之后公式仍然错——`frac = 人高 /(2·d·tan(atan(24/(2·lens))))` 把传感器高写死成 24 mm，
   而 Blender 默认 `sensor_width=36` + `sensor_fit=AUTO`，16:9 下竖向只有 36×9/16 ＝ **20.25 mm**，
   于是每个数都偏大 18%。**尺子不准，量什么都是错的，而且不会有任何报错。**
   做法：**检查项必须来自被检对象本身**（传感器高问 `cam_d.sensor_width` 与渲染宽高比要，
   期望值问镜表要），写完一条检查先**故意让它失败一次**——
   失败不了的检查，就是还没写完的检查。
6. **不要从一张投影去判三维形状——要判就去量**（2026-09-21，一次会话里连犯三回）。
   三次都是同一个错：拿**一个视角**的渲图去判「这东西长什么样」。
   ① p25 雕像的正交侧视里，剑是一根细横针、基座拖出一块平板 → 判成「模型塌了要重跑」。
      实际：**扁刃侧看本来就是一条线**，那块平板是八角基座侧看的样子。
      量一下：整个网格只有 5.0×3.6×17.9 m，根本没有任何横向突出物。
   ② S06 高机位俯拍里，近处两尊雕像「明显躺着」→ 判成朝向归一失败。
      实际：相机在 25 m 高、离它们水平只有 12 m，几乎是贴着侧面俯看，**透视压缩**而已。
      量一下：五尊全是 X/Y 5–8 m、**Z 19 m、底面 z=0**，全部站立。
   ③ 更早一次把肩甲看成兜帽尖角。
   **判据不是「这张图是不是缩略图」**——那条旧说法太窄，所以三次都没拦住。
   判据是：**你是在用一张投影推断三维事实吗？** 是，就去量
   （`glb_preview.py` 出多视角、或直接在 blend 里报包围盒），
   或者换一个**与原视角正交**的机位再看一眼。
   **极端视角（正交侧视、贴身俯拍、长焦压缩）下，「看起来是什么」与「是什么」没有关系。**
7. **一个没有约束的优化目标，最优解一定在边界外**（2026-09-21，同一个函数里踩了两次）。
   `find_clear_cam()` 本来只要「找一个通视的机位」；我把它改成「挑**最宽敞**的候选」，
   于是：① 室内镜全部贴上天花板——家具之间的眼高位置，净空当然不如头顶那片空气大；
   ② 更狠的是**屋顶外面**在空旷度上永远赢过屋里任何位置（外面净空＝探针上限），
   所以它会把相机抬出屋顶，渲出来是天花板背面。
   三条可迁移的做法：
   ① **合格的默认值不该被打分函数否决**——先判「本来这个位置行不行」，
      行就直接返回；打分只用于**不合格之后**的候选排序。
   ② **给优化量加物理上限**（头顶净空、街宽、房间高），
      否则求解器会走到你根本没打算允许的区域里去。
   ③ **代价不对称的维度要显式排序**：抬高机位是「最后手段」而不是「一个平等的选项」，
      所以按 dz 升序找、这一层找到就收手，不去更高处比空旷。
8. **previz 里必须有人**（2026-09-21）。sk2 的 44 条 previz 一度**一个人都没有**，
   而每条都在打印「人占画高 0.09 → 0.38」——那是纸上算出来的数，
   画面里没有任何可对照的东西，**景别对没对上，看片的人无从判断**。
   这违反 rule 4h ①（previz 锁的不只是机位，还有**站位与动作时刻表**）。
   被摄点上放一个 1.75 m 的替身（腿/躯干/头/朝向标）即可，代价极小；
   **比例要按所在 blend 走**（整城 0.5、内景 1:1）。
   配套：**室外可以「看街轴前方 N 米」，室内不行**——室外那是一条有纵深的街，
   室内照搬就是把镜头指到一面白墙上（实测旅店近景：满屏墙皮）。
   室内必须显式给被摄点，落在这个空间真正的视觉重心上。
9. **当一条检查逼你把产物改成与它自己的描述相反，那是检查错了，不是产物错了**
   （2026-09-22，同一条判据问错两次）。
   要判「这个景别在这个地方做不做得出来」，我先后问过三个问题：
   ① **「画幅比房间宽吗」** —— 错。画幅比房间宽**根本不是失败**：
      墙会把画框填满，那正是室内远景该有的样子（人很小、整间屋子都在画面里）。
      按这个口径，18 m 宽的王座厅被判成装不下远景，解算器于是把
      「高机位**俯瞰**」的落幅改成 0.80 近景——**数值合规、语义反了**。
   ② **「相机退得到那么远吗」** —— 物理上讲得通，但漏了现场每天都在做的事：
      **退不远就换广角**。退到 18 m 要人占 0.09 只需 57°（约 30 mm），再正常不过。
   ③ **「退到底之后还得上多广的镜头」** —— 这才是真下限。>75° 人就开始变形。
   两条可迁移的规矩：
   **A. 检查逼出的修改如果和产物自己的文字打架（「俯瞰」被改成近景、
   「拉远」被改成中景），先怀疑检查。** 语义矛盾是判据错误最可靠的警报。
   **B. 判据要落在真实的物理下限上，不要落在中间量上。**
   房间宽度、后退距离都是中间量，可以被别的手段补偿（墙填画框、换广角）；
   镜头角度才是补不了的那一个。中间量当判据 ＝ 一堆假警报，
   而假警报会驱动真破坏（我照第一版改了 13 镜 `subj_frac`，
   把景别压进窄带，**切口全没了**，生成器闸门报 9 条才拦住）。


### 28. 雕像也算「整身着装人物」——Hyper3D 的 text-only 一样做不出来（2026-09-20 实测）

rule 4h §G1 记过「整身着装人物喂进 Rodin 会塌成一团光滑坨」。**石雕人像不是例外。**

实测（sk2 英雄谷的英雄雕像，`--tier Regular --mesh-mode Quad`，prompt 写满了
盔甲 / 披风 / 拄剑 / 基座 / 单一石材）：出来的是**一具躺倒的光滑坨**——
没有盔甲分片、没有披风、剑只剩一根棍，头是个瘤子；而且**整体朝向是横躺的**
（朝向随机这条工具文档本来就写了，`whitemodel_normalize.py` 能修朝向，**修不了形状**）。

**结论：判据不是「是不是人」，是「有没有大量需要靠语义理解才能生成的分件结构」。**
盔甲片、披风褶、握剑的手——这些 text-only 给不出来，和「有界的有机小件」（器物/构件/法宝）
完全是两类东西。**雕像归人物一类，不归器物一类。**

**正解是走「图先行」（rule 4d）而不是放弃**：工具自己的文档就写着
「text-only 出的模型会丢薄长件……**image-to-3D 的正确用法是喂多视角参考图**」。
所以顺序是：**纯文字先出我们自己的三视图锚点图 → 拿那三张喂 image-to-3D**。

**⚠️ 关键约束**：**权利人的官方图不能喂给生成模型**（sk2 `divergence #108`：
参考图只进人眼、不进模型）。所以喂进去的三视图**必须是我们按锁定串自己生成的**，
不是抓来的。这恰好就是 rule 4d 「图先行、3D 在后，锚点图纯文字自由生成、零参考图」的原意——
**那条规矩不只是为了质量，在 IP 衍生项目里它同时是合规屏障。**

配套：`tools/glb_preview.py` 把任意 GLB 渲成四视图 PNG。
rule 4h §G 的「出了模型必须先渲一眼再往下做」**不能靠自觉，要有一条命令**——
生成式 3D 的失败是沉默的：文件大小正常、闸门也能过，形状已经塌了。
