...
3. **反馈→进化**：用户每条实战反馈 → surgical 更新对应 playbook / 审查 skill / `agent_refs` + 记一条"教训"（带来源镜号/反馈引用）。落点=审查 skill 准则表 + agent_refs。**流程越用越强**。
   - **能量化的反馈必须落成「可机检的闸门」，不许落成「记得注意」**（2026-09-19）。
     一条写进 playbook 的叮嘱，靠的是下一次有人记得读它；一个闸门，是下一次**做不出违规产物**。
     实测：「镜头要匀速」「把折返全去掉」「不要 180 度甩头」「别撞墙」四条反馈，
     落成四个数值判据（切向加速度 / 沿弦折返量 / 云台偏转速率 / 逐帧净空）之后，
     不合规的航线在生成阶段就 raise、根本进不了渲染队列。
   - **闸门必须测产物，不测中间变量**：从最终产物里回读（从 blend 里已打好关键帧的相机回读，
     而不是校验生成它的那个数组）。踩过：修正写在打帧之后，闸门报合格而成片撞墙。
   - **闸门要左移进生成器**（同 `ai_video.md` 16.x）：能在 build 时 raise 的，
     就不要留给出片前的 reviewer；reviewer 只兜住无法量化的那部分。
   - 不能量化的反馈（"味道不对""不够精致"）才落成 playbook 准则 + 审查 skill 的判读条目。

## 敏捷原则（2026-06-18，务必遵守）
**大方向先定，每集剧情边拍边改**——不一次性把全剧剧情写完再拍。
- 阶段 1–3 只确立**全剧大方向骨架 + 前几集细纲**；后续各集**留松**，推进到该集时（阶段 4–6）再细化。
- **不绑死**：根据实拍 feedback 随时改剧情 / 设定走向；每次改动走"每次 update 复核 + 反馈→进化"机制。
- 默认**只先做前几集**（用户指定范围），跑通"立项→出 prompt"小闭环，再逐集滚动推进。
