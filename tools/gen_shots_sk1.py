...
# STYLE_BASE / NEG_BASE / NEG_ARCH / NEG_CG 的唯一出处是 look 层生成器（rule 4i ①）——
# 主体卡与 shot prompt 共用同一套串，import 掉就不可能漂；style_guide.md 的围栏由两边的 gate 各校一次。
from gen_scene_prompts_sk1 import NEG_ARCH, NEG_BASE, NEG_CG, STYLE_BASE, SURFACE  # noqa: E402
...
SKY_DAY = ("；晴天：画面里凡露出天空都是干净通透的蓝天，凡露出水面都是碧绿通透的水，空气透亮、没有灰霾；"
           "但远景不是一样清楚——越远越淡、越远越软、越偏蓝灰（大气退远），地平线上没有奶白色的雾带")
...
# ───────────────────────────── 摄影与做旧（follow-up 014 · style_guide §11）
# 光圈按景别给：人占画高越大、景越近、光圈越大。`jb` 的两个 frac 就是量尺，不另建一套。
STOP_BANDS = ((0.50, "f2", "焦点落在人的眼睛上，背景虚成一片可辨的色块——不是全景深"),
              (0.20, "f2.8", "主体清楚，背景软下去、只剩形状与色块——不是全景深"),
              (0.00, "f4", "近处清楚，越往深处越淡、越软、越偏蓝灰（大气退远）——不是全景深"))
# 每镜写哪几面做旧，按地点挑（最多四面；再多就是堆字，会把注意力从最要紧的那几面上冲淡）
WEAR_BY_BG = {
    "bg0_汴京全城": ("瓦", "墙", "土", "水"), "bg1_虹桥": ("木", "瓦", "水", "布"),
    "bg2_东水门城门": ("墙", "木", "土", "人"), "bg3_汴河码头": ("木", "石", "水", "布"),
    "bg4_州桥御街": ("木", "石", "土", "人"), "bg5_正店酒楼": ("木", "布", "土", "人"),
    "bg6_瓦子勾栏": ("木", "布", "土", "人"), "bg7_坊巷民居": ("墙", "瓦", "木", "土"),
    "bg8_郊外清明踏青路": ("木", "土", "布", "人"), "bg9_今日州桥遗址": ("石", "土"),
    "bg10_赵太丞家": ("木", "瓦", "墙", "人"), "bg11_开封府": ("墙", "木", "石", "土"),
    "bg12_宣德楼": ("木", "石", "墙", "瓦"), "bg13_相国寺": ("木", "布", "土", "人"),
    "bg14_客店房间夜": ("木", "布", "人"), "bg15_园林雅集": ("木", "水", "布", "人"),
    "bg16_汴京全城五更": ("瓦", "墙"), "bg17_州桥夜市": ("木", "布", "土", "人"),
}


def photo_of(s: dict) -> str:
    jb = s.get("jb") or (0.0, "", 0.0, "")
    frac = max(jb[0], jb[2])
    stop, dof = next((b, d) for f, b, d in STOP_BANDS if frac >= f)
    m = re.search(r"(\d+(?:[–-]\d+)?)\s*mm", s.get("cam", ""))
    foc = m.group(1) + "mm" if m else "35mm"
    kind = "变焦" if ("–" in foc or "-" in foc) else "定焦"
    return ("ARRI ALEXA 实拍，Super-35，Cooke S4/i %s %s，光圈 %s，1/48 秒快门；%s；"
            "画面边缘略松、很淡的暗角、高光边缘极轻微色散" % (foc, kind, stop, dof))


def wear_of(s: dict) -> str:
    return "".join(SURFACE[k] for k in WEAR_BY_BG[s["bg"]])



# 光的纪律（style_guide §3 四条）：光比与暗部深度每镜必写。写「明亮 / 柔和」模型听不懂，
# 写「差几档、哪儿掉到接近黑」它才照做。档数按有没有人工光源分：白天靠太阳，夜里靠一盏油灯。
LIGHT_RATIO = {"day": "三到四档", "dawn": "三到四档", "dawn_lamp": "四档以上"}


def light_discipline(s: dict) -> str:
    return ("；受光面与阴影差%s，阴影里只有天光、没有一盏补光；屋檐下、门洞里、棚下、舱里与一切背光面"
            "掉到接近黑、只留极少细节，不抬亮、不补蓝" % LIGHT_RATIO.get(s["lt"], "五档以上"))
...
def gate_shape() -> None:
    ...
    # 2026-09-16 follow-up 009：用户选择本站不要签名开场（原来在 S02 的画外句已删），所以不再要求它出现；
    # 若将来把它放回某一镜，下面的「最多出现一次」仍然守住唯一性。
    s03 = S[2]["lines"]
...
def negatives(s: dict) -> str:
    ...
    if "p2" in s["pr"]:
        ...
        add.append("金银元宝")
    out = [NEG_BASE, NEG_ARCH, NEG_CG, NEG_TOUR] + add
    if s["lt"] in ("day", "dawn"):
...
def emit(s: dict, i: int, start: int, seams: list, facts: dict) -> tuple[str, str, int, float]:
    ...
    p += ["摄影: `%s`" % photo_of(s), "做旧: `%s`" % wear_of(s),
          "光线: `%s`" % (s["light"] + light_discipline(s) + tail + sky), "节奏: " + s["pace"], "渲染样式: " + STYLE_BASE, "比例: 16:9",
          "时长: %d秒" % s["d"]]
    pos = NL.join(p)
...
# follow-up 010：S01 / S02 一镜到底的速度口径（时空压缩），两镜 `镜头:` 共用同一句开头
RAMP = ("全程一个连续运镜、**不切**；机位始终沿着河道与街道的走向平缓转向，没有急转、没有折返、没有原地打转；"
        "速度是真无人机的量级（三十到六十米每秒），画面平稳不拖影、不跳帧、不抖")


# ═══════════════════════════════════════ 镜表
S += [
 dict(n=1, seg="序", sec="航拍长镜", d=30, bg="bg2_东水门城门", v="bg2-1",
      vx=[("bg3-1", "11–19s 汴河码头")],
      lt="day", tod=1, tier="S", ch=[], pr=[], unit="航拍", crowd=True, local=True, aerial=True,
      neg_add="明清青砖城墙, 一圈青砖城墙, 正南正北对齐的方格城市, 石拱水门, 吊桥, 铁索桥, 画面中途硬切, 跳切, 转场特效, 急转弯, 画面打转",
      facts=["gate.002", "gate.006", "route.002", "route.003", "job.001", "job.005", "boat.003", "city.021"],
      jb=(0.02, "远景", 0.02, "远景"), jbcam=("东水门外河面低空航拍", "汴河上空低飞西行"),
      tail_lock=("本镜是 shot02 的交接源。按格式契约 K26 ④ 首次生成豁免：第一次出片不挂 `本镜末帧=>@` 与 `末帧:`（末帧此时还不存在）。"
                 ...
                 "其下加一行 `末帧: 画面以末帧为结束、收束至末帧`，保证 shot02 的首帧参考不失效。末 1.5 秒微微减速、航向不变，"
                 "接缝落在一个平飞 beat 上（shouweizhen §3.3）"),
      title="【一镜到底·航拍】关厢一掠 → 穿东水门 → 仓前码头",
      ...
      plot="宣和二年清明日卯时，太阳刚离地平线一掌高：镜头贴着碧绿的汴河水面自东向西飞，两岸是护龙河外的关厢——一排排泥墙灰瓦的屋舍、门前的柳、屋后刚返青的田块，一掠而过；前方外城的夯土城墙横过来，一座跨河的城台越来越高，一扇铁裹木闸门高高吊在门洞顶上；镜头降到贴水，从闸门下面穿过门洞，洞里一暗、出洞一亮；出洞后抬升，沿着北岸掠过仓前码头：一长排灰瓦仓廒、洞开的仓门、三条平底纲船靠在石岸边，跳板上脚夫一前一后扛着布袋往仓里走；镜头继续顺汴河往西，两岸的铺面与河街掠过，末段微微减速",
      cam="%s，24–26mm：0–5.4s 离水约十四米降到七米、约 47 m/s，镜头朝西平视，关厢与田野在两岸掠过；5.4–7.6s 降到水面上四米多、放慢到约 39 m/s，从吊起的闸门下穿过门洞（闸门吊在离水六到八米、门楣在八米以上，所以必须贴水穿）；7.6–10.1s 出洞抬到七米；10.1–18.6s **机身沿河直线不转弯**、约 40 m/s，云台从正前方摇到右前方看仓前码头（仓门、跳板、靠岸纲船、脚夫队列），看完摇回航向——这个速度下转弯半径要两百米，而码头只有一百米宽，真无人机只能这样拍；18.6–30s 抬到十三四米顺汴河往西平飞、约 43 m/s，末段微微收速。全程速度 38–48 m/s（浮动一成二以内）、切向加速度 < 3 m/s²、侧向 < 1.1 g、**零折返**。画面里没有任何飞行器，也没有飞行器的影子" % RAMP,
      block="画面里没有具名人物；关厢屋前与河岸上的行人是剪影、不可辨面孔；码头跳板上脚夫扛袋上下、船上两名水手在舱棚边、门外还有一条等着进城的纲船；任一时刻可辨主体都不超过八个，所有人都不看镜头",
      act="0–5.4s 两岸关厢的屋舍、柳树与田块越来越快地向后掠，外城夯土墙与城台迎面长大、压满画面上缘；5.4–7.6s 闸门与门楣从头顶压过去，洞内一暗、出洞一亮；7.6–10.1s 河街两岸的铺面进画；10.1–18.6s 仓前码头在画面右侧展开：三条纲船靠岸，跳板上脚夫一前一后扛着布袋往仓门走，船头有人弯腰解绳，仓门里堆着麻袋，云台跟着岸线摇；18.6–30s 云台摇回航向，两岸铺面与河街掠过，河面上纲船一条接一条",
      lines=[],
      light="卯时，太阳刚离地平线一掌高、在镜头右后方的东方低空：低斜的暖光擦过河面，右岸柳树的长影一道道横铺在水上；关厢的泥墙受光面发亮、背光面是蓝天反射的冷蓝；东水门城台受光面是亮土黄、门洞里是背光的暗，洞口一圈亮边；码头石岸与跳板的木头被低斜光照出长影，仓廒的灰瓦顶反着一层冷光",
      pace="掠（关厢田野）→ 减速（城台）→ 穿（门洞）→ 掠（仓与跳板）→ 平飞（汴河）",
      spatial="一条平缓的连续航线：东水门外汴河水面自东向西 → 门洞 → 城内北岸码头 → 汴河继续往西。航向全程朝西偏北，转向都跟着河道走、曲率很小",
      contrast="首帧＝关厢屋舍夹着汴河、城墙在远处（远景 0.02）→ 末帧＝汴河上空平飞、两岸铺面（远景 0.02）。**三百五十米外起飞，六秒就进了城。**",
      moment="**8s 从吊起的闸门下穿过东水门门洞、洞内一暗出洞一亮**（15s 仓前码头卸货是中段事件）",
      post="片名与开头黑场由后期加；环境音＝河水 + 码头的号子与脚步（没有可辨字句）+ 门洞里的回声；**全镜没有任何切点与转场**",
      judge="用户 2026-09-16（follow-up 011）定调「飞得太快 / 郊区要小 / 无人机不要转弯，要流线型」。三条落法："
            "① 起飞点从虹桥（城外四公里）挪到东水门外三百五十米，六秒进城；虹桥不再进航拍，地面 shot03 / shot04 已拍足；"
            "② 全程三十到六十米每秒，删掉 follow-up 010 那段两百到三百八十米每秒的时空压缩；"
            "③ 航线只跟着河道走、没有急转与折返。码头段机位由 blend 实测坐标定（仓 x 2702–2745 / y −2550..−2502、顶高 10 m），"
            "贴北岸飞而不是沿河心飞——前一版沿河心飞，两岸都是空地。**2026-09-16 follow-up 012**：用户要求「码头缩短到 8 秒、剩下的时间给其他地方」，码头段收成 11–19s 整八秒（速度降到约三十米每秒，画面反而更看得清），省下的两秒给城内汴河段（19–30s，十一秒、约四十米每秒）"),

 dict(n=2, seg="序", sec="航拍长镜", d=30, bg="bg13_相国寺", v="bg13-1",
      vx=[("bg4-1", "26–30s 御街中轴"), ("bg12-1", "28–30s 尽头的宣德楼")],
      lt="day", tod=1, tier="S", handoff_from=1, aerial=True, xuande=True,
      ch=[], pr=[], unit="航拍", crowd=True, local=True,
      neg_add="明清青砖城墙, 一圈青砖城墙, 正南正北对齐的方格城市, 潘杨湖, 金黄琉璃瓦, 重檐, 画面中途硬切, 跳切, 转场特效, 急转弯, 画面打转",
      facts=["literati.011", "route.009", "route.010", "zhouqiao.001", "palace.002", "palace.010", "city.024", "city.030"],
      jb=(0.02, "远景", 0.02, "远景"), jbcam=("汴河上空低飞西行", "御街东侧低空四十二米近乎平视"),
      title="【一镜到底·航拍】汴河 → 相国寺大殿 → 低空顺御街望向宣德楼",
      ...
      plot="首帧是汴河上空平飞的画面；镜头沿河继续往西，两岸的铺面与河街掠过；相国寺桥从画面深处长出来，镜头顺势往北偏，从东南方斜对着寺院中轴切进去：先是灰瓦素木的大三门与泥抹刷白的寺墙，再是大殿——发暗的青灰板瓦、平直的正脊、檐下一排素木柱子、台基与几级青石台阶；大殿正脊从镜头下方过去，后面是两层的资圣门与两侧长长的廊庑，寺东门外的书市街上人影来回；出寺之后镜头贴着屋顶继续往西北飞、只从二十六米缓缓抬到四十二米，御街的中轴从画面左前方斜插进来、由近及远收拢成一条线——路心两行朱漆杈子、杈子里侧砖石御沟上新荷的小圆叶、沟岸的桃李梨杏开着白与淡粉的花；镜头几乎平视，五百多米外的中轴尽头是宣德楼：五个门洞、朱漆金钉的门扇、单檐庑殿的门楼与两侧朵楼，门楼屋面是发暗的绿琉璃瓦、不是金黄，宫城的一圈墙与墙里成片的殿宇在其后",
      cam="%s，24–26mm：0–15s 顺汴河往西直线平飞、约 39→46 m/s，从十四米缓缓抬到二十米，镜头朝西平视略俯（这一段是平直河道、两岸铺面重复，信息量最低，所以走得快）；15–17.7s 顺势北偏（转向跟着河道与寺前街走，侧向不到 0.9 g），抬到三十米，26mm，镜头对准寺院中轴；17.7–22.7s 放慢到约 33 m/s 掠过寺院：大三门、大殿正脊、资圣门依次从镜头下方过去，镜头俯角约二十五度；22.7–30s 一边往西北飞一边平稳抬到约四十二米、约 37 m/s，24mm，几乎平视（下俯不到两度），御街中轴在画面左前方收拢成一条线、宣德楼在尽头。全程速度 33–46 m/s、切向加速度 < 3.5 m/s²、侧向 < 0.9 g、**零折返**，接缝速度与 shot01 末段差一成以内。画面里没有任何飞行器，也没有飞行器的影子" % RAMP,
      block="画面里没有具名人物；相国寺书市街与御街两侧的行人都是剪影、不可辨面孔；御街路心两行朱漆杈子围出的御道里空无一人；任一时刻可辨主体都不超过八个，所有人都不看镜头",
      act="0–15s 两岸铺面与河街快速掠过，相国寺桥从画面深处长出来；15–17.7s 河道让位给寺前街，寺墙与大三门进画；17.7–22.7s 大殿的歇山屋顶迎面长大、正脊从镜头下方过去，檐下柱列、台阶与廊庑的椽头都看得清，资圣门与廊庑连成两条长带，书市街上人影来回；22.7–27s 地面平稳退开，御街中轴进画：朱漆杈子成两行、御沟水面一闪一闪、花树的白与淡粉铺在沟岸；27–30s 升势稳住，宣德楼在中轴尽头越来越清楚，宫城一圈墙与墙里成片的殿宇在其后",
      lines=[],
      light="卯时刚过，低斜的日光自东方照来：汴河两岸铺面受光面发亮、背光处是冷蓝；相国寺大殿的青灰瓦受光面发亮、檐下与廊庑背光处偏冷，台基的青石反着一层亮；御街路面被斜光照出一道长亮带，杈子与花树在路面上投下长影；宣德楼正面被东侧低光斜照，绿琉璃瓦在斜光里是哑一点的深绿",
      pace="平飞（汴河）→ 北偏 → 慢掠（大殿）→ 低空续飞（御街进画）→ 稳住（宣德楼在尽头）",
      spatial="一条平缓的连续航线：汴河上空自东向西 → 相国寺桥处顺势北偏 → 从东南斜对中轴掠过寺院 → 继续西北向低空续飞 → 御街东侧约二百五十米、离地四十二米。全程没有 90° 转弯、没有折返、没有爬升到俯视高度",
      contrast="首帧＝汴河上空平飞、两岸铺面（远景 0.02，与 shot01 末帧同一画面）→ 末帧＝四十二米低空、几乎平视，御街中轴斜插进画、宣德楼在五百多米外的尽头（远景 0.02）。**从一条河飞到这座城的中轴线上，全程贴着屋顶。**",
      moment="**30s 御街由近及远铺到宣德楼、宫城成片在其后的那一帧**（封面候选；20s 大殿正脊迎面长大是中段事件）",
      post="开头 0.3 秒不出声（承接缝静默，ai_video.md (J)）；环境音＝街市人声（没有可辨字句）+ 风声，升高后风声渐强、人声渐远；**全镜没有任何切点与转场**",
      judge="用户 2026-09-16（follow-up 011）定调一镜到底 + 不要转弯 + 别飞那么快，并在追问里选了「速度全程一致」这一版。三条落法："
            "① 桑家瓦子从航拍里去掉——它在东北，插进来必须来个九十度以上急转，和流畅直接冲突（瓦子在 shot28 / shot29 地面上有完整戏份）；"
            "② 州桥也不再飞过去，改成在爬升段随俯角变大自然进画；"
            "③ 结尾收在约三百二十米而不是两千二百米：真实爬升十到十五米每秒、六秒只能升一百米，要俯看整座城就必须把结尾放开成时空压缩加速上升，"
            "用户选了速度一致，所以画面里是御街—宣德楼—宫城这一片，不是整座城。相国寺机位由 blend 实测坐标定（大殿 x 377–423 / y 133–167、脊高 23 m），"
            "从东南斜对中轴进、看点始终压在大殿上——前两版看点指到了寺北，等于飞过去再回头看，两次都没拍到大殿。**2026-09-16 follow-up 012**：码头压到八秒后，省下的时间加在本镜——相国寺从六秒加到十秒（12–22s）、结尾爬升从六点五秒加到八秒（22–30s）；代价是城内汴河段只剩十二秒，而东水门到州桥实测两千八百米，这一段只能跑约九十五米每秒（快于真无人机）。取舍依据：这一段是平直河道、两岸铺面重复，是全镜信息量最低的一段；相国寺与御街结尾才是看点，所以时间给它们。**2026-09-16 follow-up 014**：用户定调「一镜到底不用有俯视，城的 3D 只要够拍这条航线」，末段爬升（22–30s 升到 320 m、俯角 38°）整段作废，改为低空续飞（26→42 m、俯角 < 2°）。连带两件事：① 全城层按 city_plan §8.2 改成走廊制，只建航线两侧，320 m 俯视要的整城几何不再存在；② 宣德楼在本镜只作五百多米外的远景，它的近景归 S17–S19 三个地面镜——本来就该归它们"),
...
S += [
 ...
 dict(n=35, seg="收", sec="航拍收束", d=20, bg="bg16_汴京全城五更", v="bg16-1", lt="dawn_lamp", tod=10, tier="S", aerial=True, xuande=True,
      ...
      jb=(0.02, "远景", 0.02, "远景"), jbcam=("客店屋脊低空航拍", "屋海上空一百一十米低角度航拍"),
      title="【航拍收束】五更天亮 · 退回整座城",
      summary="五更天亮，镜头从客店屋脊起飞，边退边升：屋脊下巷子里还剩两三点油灯，一点点淡下去；汴河上几条船开始撑篙；升到约一百一十米停住——**不是俯瞰全城，是贴着屋海看**：一层压一层的灰瓦屋脊往地平线退过去、越远越淡，两座高塔的塔尖先被第一道光照到。无台词；后期叠考证卡。",
      ...
      cam="一个连续运镜、不切，24mm：0–6s 离屋脊约十四米，沿屋脊缓缓向后退并抬升；6–16s 边向后退边升到约一百一十米，镜头下俯约十二度朝西北；16–20s 减速悬停，屋海一层压一层往地平线退去、越远越淡越偏蓝灰，两座高塔的剪影立在远处。**不拉到能看清全城平面的高度**——这一镜要的是屋脊的层次，不是一张城图。飞速平稳、不拖影。画面里没有任何飞行器，也没有飞行器的影子",
      block="画面里没有具名人物；早起的行人与船工只是远处剪影、不可辨面孔；可辨主体不超过八个，都不看镜头",
      act="0–6s 灰瓦屋脊在画面下方后退，屋脊下巷子里两三点油灯还亮着；6–11s 天色一点点变亮，那几点油灯在天光里越来越淡，其中一盏被人吹灭；11–16s 汴河露出来，几条船开始撑篙，船只有米粒大，河面是碧绿的、映着渐亮的蓝天；16–20s 第一道直射的暖光先照亮两座高塔的塔尖，贴着屋脊的薄雾从受光的地方开始散开，露出下面几条街巷的走向，镜头悬停；更远的屋海仍在冷蓝里，只剩一层层的轮廓",
      lines=[],
      ...
      spatial="机位＝客店屋脊上方连续后退拔高到屋海上空约一百一十米；在画主体＝屋脊与巷灯 → 屋顶群与汴河 → 一层层退远的屋海；运动方向＝向后、向上",
      ...
      judge="片尾顺序改为客店过夜 → 本镜五更航拍 → 可选的今日遗址（R5 F07 / 协调者裁定 2），镜号 36→35；主体改挂 bg16_汴京全城五更、锁定串照卡（协调者裁定 6）；有零星油灯，挂有灯负向、去掉「提亮的黑位」，不挂蜡烛组（bg16 卡判断：酒店灯烛 street.003 ✅）；高度先按 R3-26 定为约六百米；**2026-09-16 follow-up 014 再降到约一百一十米**——用户定调不要俯视、城的 3D 只建航线走廊（city_plan §8.2），六百米俯瞰要的整城几何已经不存在，而这一镜真正要的「屋脊的层次 + 第一道光打在塔尖」在一百一十米上更好看；灯熄与第一道光（R4 F19）；远处宫城门楼按宣德楼例外处理；B_city 渲 `shots/shot35/shot35_previz.*`"),
