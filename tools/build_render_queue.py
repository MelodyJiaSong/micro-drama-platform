...
Extracts every render-prompt block (起始帧 / Seedance) from each
`perf_NNNN.md` in entry order and lays them out as a top-to-bottom checklist the
user works through — copy a block, paste into Seedream / Seedance, download,
...
def build_emotion(emotion_dir: Path) -> int:
    ...
    out.append(
        ...
        "`演{编号}始`=起始帧静帧）→ 起始帧块粘进 Seedream/即梦生成静帧、检验视频块粘进任意视频模型（Seedance/…）"
