...
| 两条解说轨 | **视频原声一条 + 译配一条**（系列 follow-up 012 · divergence #20）：旅行者的台词由视频直接出声——默认 c1 林问说普通话，所以原声是中文、英文是译配；若换成说英语的候选则反过来。译配轨的做法：先把视频音轨人声分离、去掉原声、保留环境音，再按 `../2_世界观人设/casting.md` 的译配 voice_id 叠 TTS；两条轨同一套环境音与 BGM |
| 成片文件 | 英文版 `sk1_en.mp4` · 中文版 `sk1_zh.mp4` · 中文混音轨 `sk1_zh_audio.wav`（给 YouTube 附加音轨用，与母版逐帧等长）。默认 c1（普通话原声）：`sk1_zh.mp4` / `sk1_zh_audio.wav` 取视频原声，`sk1_en.mp4` 是去原声后的英文译配；选说英语的候选时反过来 |
...
1. 出中文混音轨 `sk1_zh_audio.wav`：中文解说（说普通话的旅行者＝直接取视频原声轨；说英语的旅行者＝去原声后按 `casting.md` 译配 voice_id 叠中文 TTS）+ 与英文母版**同一套环境音和 BGM**；从 `00:00` 起与母版逐帧等长，片头片尾都不能多一帧或少一帧。
...
【怎么做的】
画面是依据史料做的 AI 情景复原；旅行者是虚构人物，她的声音由 AI 生成；当地人全程不说话——没人知道 1120 年汴京街上的人怎么说话，我们就不替他们编。片里的数字都带出处，学界还在争的，片中当场说明是推测。
...
【声明】本片由 AI 生成：汴京的街景、人物和事件，是依据《东京梦华录》《清明上河图》等史料做的 AI 情景复原；旅行者林问是虚构人物，她的声音由 AI 生成；片中当地人全程不说话。
```
...
【声明】画面由 AI 生成，是依据《东京梦华录》《清明上河图》等史料做的情景复原；旅行者是虚构人物，她的声音由 AI 生成；当地人全程不说话。
...
- [ ] **旅行者全片是同一个声音**（逐镜对照 `cN-2` 声样抽听）；译配版的口型偏差可接受（画外句占多数）；换过旅行者时，本页各站文案里的名字已同步替换
