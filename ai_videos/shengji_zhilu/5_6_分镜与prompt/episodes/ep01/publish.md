...
> 语言规则：**YouTube / TikTok 用英文，抖音 / 小红书用中文**（四份文案各自创作，不是互译）。
> 本剧面向英语观众（concept G8），英文站是主站；中文站发英文原声 + 中文字幕版。
> 片外三个来源标识位（标题 / 账号名 / 封面）**不带商标**（concept F1 / F3）；免责声明进简介与片尾卡。
> 结构与字段上限的通用契约见 `.claude/agent_refs/project/ai_video.md` rule 16。
...
**字段上限**：标题 ≤100 字符 ｜ 说明 ≤5000 ｜ 标签总长 ≤500 字符 ｜ 章节 ≥3 条、首条 `00:00`、每条 ≥10s
...
An AI-generated fan work. Not affiliated with Blizzard Entertainment.
```

### 标签

```text
just enough light, fantasy series, paladin, sword and sorcery, ai film, fan film, classic fantasy, medieval fantasy, found family, underdog story, episode 1, fantasy adventure, healer, holy light, new recruit
```
...
| 字段 | 值 |
|---|---|
| 类别 | Film & Animation（影视与动画） |
| 视频语言 | English |
| 是否为儿童打造 | **否** — Not made for kids |
| 修改后的内容 | 勾「AI 生成 / 合成」披露 |
...
#fantasy #paladin #aifilm #fanfilm #foundfamily
```
...
| 字段 | 值 |
|---|---|
| 封面 | `shot01` 第 7 秒 · 一只空手 |
| 声音 | 原声（不叠平台 BGM） |
| 字幕 | 上传英文 CC；关闭自动字幕（名字与原句会被识别错） |
| 允许 | 评论 / 合拍 / 拼接 全开 |
| 地区 | 英语区为主 |
| AI 标签 | 打开「AI 生成内容」标注 |
...
- [ ] 传的是最新成片，不是 previz、不是旧版本
- [ ] 画面里**没有字幕、没有水印、没有 logo**；片头与片尾字卡是后期叠的，拼写与 §1 逐字一致
- [ ] 四站的语言对了：YouTube / TikTok 英文，抖音 / 小红书中文
- [ ] YouTube 章节 ≥3 条、首条 `00:00`、每条 ≥10s（现 12 条，最短 26s）
- [ ] 小红书标题 ≤20 字（现 11 字）
- [ ] 标题 / 账号名 / 封面里**没有任何商标字样**（concept F1）
- [ ] 各站「儿童内容 / AI 生成披露」都填了；变现关闭（concept F2）
- [ ] **改过任何 shot 的话，章节时间轴与精华版剪法要重算**（rule 16.7）
